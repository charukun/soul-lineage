...
## 主要画面の優先順位（2026-09-13）

「現在の問題」「アプリの公開ステータス」「開発状況」の順に確認できる画面を基本とする。主要3項目への移動を常に分かりやすくし、問題がない場合も取得範囲内で問題がないことを表示する。未取得・同期失敗を正常扱いしない。

スマホではアプリごとに1行を使い、その中で開発・検証・本番を3列で比較する。広い画面ではアプリカードも3列に並べる。以前のスマホでもアプリを3列に詰め込む指定を本項で更新する。タイトルと状態を省略せず読みやすくし、SHA・日時は詳細で確認する。

開発状況は作業中・統合待ち・統合済みを区別し、現在の工程・待っている理由を既存Integration情報から表示する。統合済みを公開済みとは呼ばない。RescueのWorker・Wave・ログおよび公開の技術情報は開閉可能な詳細へまとめ、確認操作・フィルタ・更新時の読んでいる位置を維持する。
