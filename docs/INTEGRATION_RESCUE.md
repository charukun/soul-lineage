...
GitHub event
  -> Fast Lane
       ...
       mechanically repairable -> Fast Repair (GitHub Actions)
          ...
          -> Fast Lane wake
       semantic conflict / source-level CI repair
...
6. 更新headをtrusted GitHub runnerでfast validationし、exact-head evidenceを作る。
...
Fast Laneがcurrent exact headの真の競合、またはsource-level修復が必要なexact-head CI failureを確定した場合、そのpass内でGitHub Issueを1件作る。
...
- PR branch更新は通常merge-forwardまたは通常Chatで検証済みの修復だけ。force push禁止
