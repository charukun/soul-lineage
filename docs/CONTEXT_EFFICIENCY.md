...
- CI logは同一セッションの共有ledgerで累積量も管理し、固有excerpt 3件または96 KiBに達したら追加log取得を停止する。別range/jobへの切替や新規セッション作成だけで上限を迂回せず、得られた証拠を要約してhandoffする。
...
- Actions調査はfailed/cancelled jobだけを対象にし、1 excerptは原因周辺を64 KiB以内へ絞る。同じsession ledgerへ出せるCI logは合計96 KiBかつ固有excerpt 3件までとし、どちらかに達したら追加range/jobのlog取得を停止する。上限到達後は現在の証拠と未解決点を短く残してIntegration/repairへhandoffし、log掘り・CI pollingを続けない。
- 同じexact headに対して既取得のPR metadata / checks / documentを無理由に再取得しない。headが変わった、状態遷移が起きた、または追加証拠が必要な場合だけ更新する。

Hard budgetは品質gateではなく取得戦略である。必要な仕様を読まずに判断するための免罪符にしない。予算を超える場合は全文投入ではなく、検索・line range・file patchへ粒度を落として必要情報を回収する。ただしCI logのsession累積上限は応答停止を避ける停止条件であり、上限後に別logを掘って迂回しない。
...
`context:excerpt` はローカル文書/patch/logをコンテキストへ出す直前に使う。セッションごとにscratchのledgerを1個用意し、同じセッションの全取得経路で共有する。ledgerには内容のSHA-256とCI logの累積byte/excerpt数だけを保存し、本文は保存しない。次の新規セッションには引き継がない。既に読んだ同じ内容の再出力は0 byteとなり、変更内容は出力する。並列writerで同じledgerを更新しない。
...
doc/patchは最大48 KiB、logは1回最大64 KiB。`--max-bytes`で縮小できる。logはさらに同じsession ledger全体で最大96 KiB・固有excerpt 3件に制限し、残予算より大きい要求は残byteまでで切る。どちらかの累積上限に達した後のlog取得は明示的に失敗し、現在の証拠を要約してhandoffする。重複excerptは0 byteとして累積予算を消費しない。巨大docは全文出力を拒否し、log/patchは明示line rangeを必須とする。logは現在のjob metadataで確認した`failure`/`cancelled`だけを対象にし、成功jobを失敗として渡さない。巨大な単一行もstreamで制限する。`truncated=true`は未読部分があるという意味で、原因を確認済みとはしない。累積予算内なら別範囲やローカル検索で追加調査できるが、上限を越えて追わない。

このコマンドは取得済みローカルファイルの出力制限であり、GitHub APIの転送量・ChatGPT側の自動注入・外部ツール呼び出しを強制制限しない。APIにrange機能があれば先に範囲を限定する。全文しか返せないlog APIは応答をモデルへ直接展開せず、実行環境に保存して絞る。context:planの48 KiBは初期全文候補の合計、excerptは1回ごとの上限に加えてCI logだけsession ledger単位の累積上限を持つ。これらをChatGPT製品全体のtoken上限と混同しない。
