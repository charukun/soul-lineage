# 100年人生 — 百年転生 Clean Rebuild

## 目的

`apps/rinne` を既存の巨大起動経路の延長ではなく、現在のゲーム仕様とRepository内の採用済み資産を正本にして再構築する。

製品タイトルは **100年人生**、サブタイトルは **百年転生**。旧実装のコード移植ではなく、ゲーム仕様、共有world契約、採用済みasset provenance、保存境界だけを引き継ぎ、起動・描画・HUD・本編sessionを新しい小さなmodule群として作る。
...
- 宝満叡智の地形・配置・建物catalogは `@soul/world/mura`、表示は `@soul/rendering/mura` の共有契約を優先する。
