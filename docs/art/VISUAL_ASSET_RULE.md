...
- 新規・置換する Production の主要3Dは `origin: artist-authored` または `origin: rinne-owned-dcc` に限る。生成・再構築・AI/3D生成サービス由来のメッシュは、外部ファイル化されていても Production asset として扱わない。
- 既存の procedural housing は `mura.housing-legacy-procedural.v1` だけを凍結例外として残す。これは `home` / `lodge` / `clanManor` の互換維持専用で、新規kindや置換assetへ流用しない。
