...
3ゲームのWeb版はブラウザ上で動作していても、管理画面・Webアプリ・軽量ブラウザゲームのフラットUIへ寄せません。`rinne` / `village` / `demon` はそれぞれの世界観に合わせた表面・輪郭・深度・選択状態をapp内で所有し、同じ情報設計を使う場合も見た目を共通の汎用カードへ統一しません。

- 常設HUDはゲーム画面を主役にし、矩形カードを並べて情報量を増やすのではなく、必要情報をメーター・紋章・フレーム・素材感のある小型コンポーネントへ圧縮する。
- モーダル・メニュー・キャラクター選択・ルート選択は、白背景＋角丸カード＋均一影＋汎用ピルボタンの組み合わせを既定にしない。作品固有の額縁、面材、縁取り、陰影、フォーカス表現を使い、画面中央のダッシュボード化を避ける。
- 主要操作はコントローラー/タッチのフォーカスを前提に、選択・押下・無効・危険・決定の状態を色だけへ依存させず、輪郭、奥行き、発光、スケール、モーションの組み合わせで区別する。
- 説明文を常時パネル内へ露出し続けず、ボタン・ヘルプ・必要時の短い補助表示へ集約する。チュートリアルはゲーム世界上の対象と操作結果を中心に見せる。
- CSSで `border-radius`、半透明背景、`box-shadow` を使う場合も、それ自体をデザインとせず、作品固有の素材感・階層・入力フィードバックを成立させるために使う。
- 新しいUIを追加する際はスマートフォン縦横・ゲームパッド想定のフォーカス・WebGL画面との重なりを確認し、hoverだけで意味が成立する操作を作らない。

回帰確認では、3appの入口・HUD・主要モーダル/メニュー・初回導線のうち変更対象を最低限確認し、汎用Webカードの再導入、世界観を跨いだ見た目の共通化、ゲーム画面を覆う説明過多を防ぎます。
