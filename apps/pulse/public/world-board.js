...
function render(data){if(!root)return;root.replaceChildren();if(!data?.available){root.append(node('p','empty','共通世界の運用状態はまだありません'));return;}
 const c=data.counts||{};const summary=node('div','world-summary');summary.append(metric('接続中',c.open??0,'ok'),metric('移行中',c.migrating??0,c.migrating?'warning':''),metric('接続端末',c.peers??0),metric('SLO超過',c.sloViolations??0,c.sloViolations?'danger':'ok'));root.append(summary);
 const rooms=node('div','world-rooms');if(!data.rooms?.length)rooms.append(node('p','empty','現在接続中の共有セッションはありません'));
 for(const room of data.rooms||[]){const card=node('article',`world-room phase-${room.phase}`);const head=node('div','world-room-head');head.append(node('strong','',room.label||'共有セッション'),node('span',`badge ${room.phase==='open'?'ok':room.phase==='migrating'?'warning':'danger'}`,room.phase==='open'?'OPEN':room.phase==='migrating'?'MIGRATING':'CLOSED'));card.append(head);
