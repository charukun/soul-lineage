...
try{
  for(const config of [{name:'desktop',width:1440,height:900},{name:'mobile',width:390,height:844},{name:'landscape',width:844,height:390}]){
    ...
    const page=await context.newPage(),errors=[],badRequests=[];page.on('pageerror',e=>errors.push(e.message));page.on('response',r=>{if(r.status()>=400&&r.url().startsWith(base))badRequests.push({url:r.url(),status:r.status()});});
    ...
    let snap=await page.evaluate(()=>window.__ASHEN__.snapshot());assert.ok(snap.damage>0,'Real autonomous combat must deal damage');assert.ok(snap.renderedActors>=4);assert.ok(snap.animations.includes('Sword'));assert.ok(snap.animations.includes('Run'));
    ...
    if(await page.locator('#bell').isEnabled()){await page.locator('#bell').click();assert.ok((await page.evaluate(()=>window.__ASHEN__.snapshot())).bellCooldown>0);}
    ...
    assert.deepEqual(errors,[],'No uncaught browser errors');assert.deepEqual(badRequests,[],'All self-hosted app assets must load');reports.push({device:config.name,viewport:config,url:base,snapshot:snap,errors,badRequests,assetCount:Object.keys(manifest.models).length,existing3DFilesAudited:manifest.exclusionAudit.trackedExisting3DFilesChecked});
    console.log('BROWSER_PASS',tag,config.name,JSON.stringify(snap));await context.close();
  }
}catch(error){failed=true;console.error('BROWSER_FAILURE',error);for(const context of browser.contexts())for(const page of context.pages())await page.screenshot({path:`evidence/${tag}-failure.png`}).catch(()=>{});reports.push({error:String(error)});}
