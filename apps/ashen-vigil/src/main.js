...
async function boot(){
  ...
  const composer=new EffectComposer(renderer);composer.addPass(new RenderPass(scene,camera));const bloom=new UnrealBloomPass(new THREE.Vector2(innerWidth,innerHeight),.19,.45,1.03);composer.addPass(bloom);composer.addPass(new OutputPass());
  ...
  const world=buildWorld(scene,assets),actors=new Actors(scene,assets),effects=new Effects($('effects')),audio=new Audio(),game=new Battle();
  let elapsed=0,last=performance.now(),modeBlend=1,frameClock=0,frameCount=0,fps=0;
  const demo=new Battle(846);demo.start();demo.units=demo.heroes;demo.pendingSpawns=0;demo.state='title';demo.drain();
  // The title is a live, orbiting view of the same authored actors, not a mock-up.
  ...
  debug.modelsLoaded=assets.loaded;debug.modelsTotal=assets.total;debug.sourceManifest=assets.manifest.sourceCommit;debug.snapshot=()=>({...game.snapshot(),fps,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,animations:[...actors.animationClips],renderedActors:actors.items.size,ready:debug.ready});
  function frame(now){
    requestAnimationFrame(frame);const realDt=Math.max(0,Math.min((now-last)/1000,.06));last=now;const paused=game.state==='paused';const dt=paused?0:realDt;elapsed+=dt;
    ...
    composer.render();effects.render(camera,units,elapsed,dt,world.flames,!title);debug.frames++;frameClock+=realDt;frameCount++;if(frameClock>=1){fps=Math.round(frameCount/frameClock);frameClock=0;frameCount=0;}
  }
