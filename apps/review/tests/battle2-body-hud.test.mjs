...
test('battle2 body HUD is a permanent six-part liquid gauge beside the player card',()=>{
 ...
 assert.match(css,/left:max\(154px,calc\(env\(safe-area-inset-left\) \+ 154px\)\)/);assert.match(css,/height:var\(--level\)/);assert.match(css,/@keyframes b2-body-water/);assert.match(css,/data-body-part=head/);assert.match(css,/data-body-part=torso/);assert.match(css,/data-hit=true/);
 assert.match(version,/BATTLE2_VERSION='2\.2\.36'/);
});
