...
test('序破急 slots accept either combo chains or basic techniques and combo cards expose their 1-3 members',()=>{
 ...
 // Explicit user exception: 連技一覧 itself is a three-column choice grid.
 assert.match(sharedCss,/\.loadout-combo-list\{display:grid!important;grid-template-columns:repeat\(3,minmax\(0,1fr\)\)!important/);assert.match(sharedCss,/\.loadout-combo-members\{display:grid!important;grid-template-columns:repeat\(3/);
});
...
test('battle2 and 百年転生 share the same captionless 心技体装 buttons and post-tap menu primitives',()=>{
 ...
 assert.match(ui,/rinnePrimaryFourMarkup/);assert.match(ui,/rinneLoadoutPanelMarkup/);assert.match(ui,/createRinneLoadoutSlot/);assert.match(ui,/createRinneLoadoutGridItem/);assert.match(ui,/HEART_LIMIT=5/);assert.match(ui,/layout:'pentagon'/);assert.match(ui,/heart:\{slots,active\}/);assert.match(ui,/@soul\/shared-ui\/rinne-loadout-menu\.css/);
