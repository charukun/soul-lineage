...
async function download(row,signal){
  let error;
  for(let attempt=0;attempt<2;attempt++){
    if(signal.aborted)throw signal.reason;
    try{
      const response=await fetch(assetUrl(row.path),{signal:AbortSignal.any([signal,AbortSignal.timeout(20000)]),cache:attempt?'reload':'default',credentials:'same-origin'});
...
export async function loadNocturneAssets({loader,signal,onProgress=()=>{}}){
  const models=new Map(),queue=[...NOCTURNE_MODELS];let byteLength=0,completed=0;
  const allowed=new Set(manifest.files.map(row=>assetUrl(row.path)));
  ...
  await Promise.all(Array.from({length:4},async()=>{
    while(queue.length){
      if(signal.aborted)throw signal.reason;
      const row=queue.shift(),bytes=await download(row,signal);
      const gltf=await withTimeout(loader.parseAsync(bytes,new URL('.',assetUrl(row.path)).href),20000,'GLB parse timed out: '+row.id);
      if(signal.aborted)throw signal.reason;
