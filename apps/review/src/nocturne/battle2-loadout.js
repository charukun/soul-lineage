...
export function createBattle2LoadoutUI({stage,onChange=()=>{}}={}){
  ...
  let value=readStored(),learnedTechniqueIds=readLearned(),section='',heartTarget=0,techniqueTarget='jo',techniqueTab='combo',bodyTarget='stance',equipmentTarget='weapon';
  ...
  return Object.freeze({
    ...
    get learnedTechniqueIds(){return learnedTechniqueIds.slice();},
    learnTechnique(id){const row=battle2LearnedTechniqueRows([id],{weapon:value.equipment.weapon})[0];if(!row||learnedTechniqueIds.includes(row.id))return false;learnedTechniqueIds=[...learnedTechniqueIds,row.id];writeLearned(learnedTechniqueIds);if(section==='technique'&&techniqueTab==='basic')renderTechnique();return true;},
    close(){section='';render();},destroy(){shell.remove();}
  });
}
