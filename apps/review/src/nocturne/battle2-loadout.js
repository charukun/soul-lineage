...
export function createBattle2LoadoutUI({stage,onChange=()=>{}}={}){
  ...
  function renderTechnique(){title.textContent='技 · 序破急';body.replaceChildren(createRinneMenuLead('序・破・急の選択枠へ、連技または基本技をセットする'));const slots=PHASES.map(([phase,label])=>{const selection=value.technique[phase];return{id:phase,label,value:battle2SelectionLabel(selection),meta:phase===techniqueTarget?'選択先':'装着中',icon:String(selection).startsWith('combo:')?'flow':rinneSkillSigilKind(selection)};});appendSlots(slots,techniqueTarget,id=>{techniqueTarget=id;renderTechnique();});appendLibrary('連技一覧','選択中の序破急スロットへセット',BATTLE2_COMBO_PRESETS.map(row=>{const selection=battle2ComboSelection(row.id);return{id:row.id,label:row.label,meta:'連技',icon:'flow',active:value.technique[techniqueTarget]===selection};}),id=>{value.technique[techniqueTarget]=battle2ComboSelection(id);emit();});appendLibrary('基本技一覧','選択中の序破急スロットへセット',BATTLE2_TECHNIQUE_CATALOG.map(row=>({id:row.id,label:row.label,meta:row.meta,active:value.technique[techniqueTarget]===row.id,icon:rinneSkillSigilKind(row.id)})),id=>{value.technique[techniqueTarget]=id;emit();});}
