...
import {BATTLE2_LOADOUT_DEFAULT,normalizeBattle2Loadout,battle2LoadoutKey} from './battle2-loadout.js';
...
export function createJohakyuP7ReviewScenario({mode='duel',duelGap=2.18,enemyLeadSeconds=0,heroStartPhase='jo',heroStartTechniqueIndex=0,checkpointSeconds=0,actorOverrides={},loadout=null,settings=null,learnedTechniqueIds=[],comboStyle='composed'}={}){
 ...
 const config=normalizeBattle2Loadout(loadout||BATTLE2_LOADOUT_DEFAULT),weapon=config.equipment.weapon,known=[...new Set(learnedTechniqueIds)],reviewSettings={techniqueMode:settings?.techniqueMode==='random'?'random':'set',inspirationRate:settings?.inspirationRate==='high'?'high':'normal'};
 ...
 let encounter=1,epoch=1,serial=1,elapsed=0,defeats=0,finishers=0,resumes=0,restartedAt=null,runtime,last=null,activity=[],history=[],replacements=[];
 ...
 function enemyRow(slot,index,id=slot){return {id,side:'enemy',kind:'enemy',hp:46,maxHp:46,stamina:100,staminaCap:100,seed:8101+index,position:{x:index===0?0:index===1?-1.75:1.75,z:.35+duelGap},equipment:{weapon:'sword',armor:'cloth',shield:false},loadout:{jo:'basic.sword',ha:'basic.sword',kyu:'basic.sword'},staminaMultiplier:.12,damageScale:.45,mind:index%2?'counter':'balanced',readyDelay:.82-enemyLeadSeconds,spawnSeconds:.7,spawnStyle:'battlebk-ground',canFinish:false,scope:'review-trial',...actorOverrides[slot]};}
 function reset(){
   runtime=createJohakyuBattleRuntime({battleId:`review-battle:${battle2LoadoutKey(config)}:${encounter}`,bounds:{minX:-4.8,maxX:4.8,minZ:-1.4,maxZ:5.4},actors:[
     {id:'hero',side:'party',self:true,kind:'hero',hp:125,maxHp:125,stamina:100,staminaCap:100,seed:73917,position:{x:0,z:.35},equipment:{weapon,armor:'heavy',shield:config.equipment.shield},loadout:heroLoadout(),staminaMultiplier:.12,damageScale:1.09,mind,stance:config.body.stance,zanshin:config.body.zanshin,nonlethal:config.heart.active.includes('skill.nonlethal'),finisherProfile:config.body.finisher,readyDelay:.82,scope:'review-trial',...actorOverrides.hero},...slots.map((slot,i)=>enemyRow(slot,i))]});
 ...
 function meta(snapshot){const hero=snapshot.actors.find(a=>a.self),actor=runtime.actor('hero'),n=actor.loadout[PHASES[actor.cursor.phaseIndex]][actor.cursor.techniqueIndex]||actor.loadout.jo[0],action=hero.action,exchange=snapshot.exchanges.find(e=>e.pair.includes('hero')),
 ...
 function step(dt=1/60,physicalContacts=[]){
   ...
   for(const event of result.events){record(event);
     ...
     if(event.type==='finisher'&&event.sourceId==='hero'){finishers++;if(!replacements.some(r=>r.id===event.targetId))replacements.push({id:event.targetId,slot:event.targetId.split('#')[0],at:elapsed+1.55});}
