...
import {classifyJohakyuParry,createJohakyuExchangeState,isDeepJohakyuExchangeHit,johakyuExchangeHudState,johakyuExchangeSnapshot,reduceJohakyuExchange} from '@soul/johakyu-combat/exchange-policy';
...
const MODES=Object.freeze({duel:'1v1',oneVsThree:'1v3'}),ENCOUNTER_SECONDS=24;
...
function advanceFootwork(battle,positions,actions,maneuvers,now,dt){
  for(const actor of battle.actors.values()){
    ...
    const movement=johakyuStageCapability(actor,{kind:'ready',footwork});if(!movement.allowed)continue;
...
export function createJohakyuP7ReviewScenario({mode='duel',duelGap=2.85,enemyLeadSeconds=0,heroStartPhase='jo',heroStartTechniqueIndex=0,resumeAtSeconds=null,actorOverrides={}}={}){
  ...
  if(resumeAtSeconds!==null&&(!Number.isFinite(resumeAtSeconds)||resumeAtSeconds<=0))throw new RangeError('Invalid resume fixture time');
  let encounter=1,epoch=1,revision=0,time=0,resumes=0,resumed=false,battle;
  let actionState=new Map(),reactionState=new Map(),reactionCooldowns=new Map(),cursors=new Map(),readyAt=new Map(),positions=new Map(),recoveries=new Map(),maneuvers=new Map(),counterWindows=new Map(),exchangeStates=new Map(),pendingNormalStarts=new Set(),lastEvents=[],trace=[],lastFrame=null,lastMeta=null,attemptSerial=0;
  ...
  function freshBattle(){
    ...
    actionState=new Map();reactionState=new Map();reactionCooldowns=new Map();cursors=new Map();readyAt=new Map();positions=new Map(Object.entries(LAYOUT).filter(([id])=>battle.actors.has(id)).map(([id,row])=>[id,{x:row.x,z:row.z}]));recoveries=new Map();maneuvers=new Map();counterWindows=new Map();exchangeStates=new Map();pendingNormalStarts=new Set();
    cursors.set('hero',{phaseIndex:PHASES.indexOf(heroStartPhase),techniqueIndex:heroStartTechniqueIndex,stageIndex:0,cycle:0});
  ...
  function updateExchange(source,target,event){
    ...
    if(after!==before&&(after.serial!==before.serial&&after.mode==='reversal'||after.mode!==before.mode&&['read','zanshin'].includes(after.mode))){
      pendingNormalStarts.add(source.id);pendingNormalStarts.add(target.id);
    }
    if(before.mode!==after.mode||before.initiativeId!==after.initiativeId||(after!==before&&['reverse','reset'].includes(after.continuity)))traceRow({type:'exchange',sourceId:source.id,targetId:target.id,mode:after.mode,initiativeId:after.initiativeId,responderId:after.responderId,continuity:after.continuity,reason:after.lastReason,phase:after.lastPhase});
    return after;
  }
  function reviewParryStrength(target,targetState,defenseNode){
    const reaction=Boolean(targetState?.reaction),fresh=target.stamina/Math.max(1,target.staminaCap)>=.55;
    return classifyJohakyuParry({authored:!reaction,counter:reaction&&fresh&&defenseNode?.phase==='kyu',phase:defenseNode?.phase??'uke'});
  }
  ...
  function setManeuver(actor,target,{reason,footwork,seconds=.4,stopDistance=null}){
    if(!actor||!target||actor.dead||actor.incapacitated)return null;
  ...
  function incomingThreat(actor,target){
    const incoming=target?combatState(target.id):null;if(!incoming||!incoming.motion.offense||incoming.impacted)return null;
  ...
  function gateAction(actor,target,node){
    ...
    if(['guard','brace','parry'].includes(kind)){
      const pair=exchangeFor(actor,target);if(pair.mode==='pressure'&&pair.initiativeId===actor.id)return true;
  ...
  function beginReaction(actor,target,kind,reason,{footwork=null,parryDirection=null}={}){
    ...
    const stageCapability=johakyuStageCapability(actor,{kind,phase:'uke',footwork:footwork??(kind==='counter'?'forward':'stay'),staminaCost:cost});
    if(!capability.canMove||(kind==='counter'&&!capability.canAttack)||!stageCapability.allowed)return null;
    if(cost>0&&!spendJohakyuStamina(actor,stageCapability.stamina.effectiveCost))return null;
  ...
  function finishReaction(actor,state,{interrupted=false}={}){
    reactionState.delete(actor.id);const target=battle.actors.get(state.targetId);
    if(state.reactionKind==='counter'&&target){counterWindows.delete(actor.id);updateExchange(actor,target,{type:'counter-complete'});pendingNormalStarts.add(actor.id);}
  ...
  function actionView(actor,state){
    ...
    return{id:state.id,targetId:state.targetId,techniqueId:node.technique.id,name:node.technique.name,phase:node.phase,step:node.stage.index,
      ...
      progress,duration:state.duration,motion:state.motion,legal:true,scope:'review-technique-composition',counterTransition:Boolean(state.counterTransition)};
  }
  function nextAction(actor){
    ...
    const stageCapability=johakyuStageCapability(actor,{weapon:'sword',phase:node.phase,...node.stage.step,staminaCost:cost});
    if((stageDamage(node.stage)>0&&!capability.canAttack)||!stageCapability.allowed){
      updateExchange(actor,target,{type:'capability-failure',phase:node.phase});
      traceRow({type:'execution-blocked',actorId:actor.id,phase:node.phase,reason:stageCapability.reason??'stamina-policy'});
      readyAt.set(actor.id,time+.24);return null;
    }
    if(cost>0&&!spendJohakyuStamina(actor,stageCapability.stamina.effectiveCost)){updateExchange(actor,target,{type:'execution-blocked',phase:node.phase});readyAt.set(actor.id,time+.32);return null;}
    ...
    const pair=exchangeFor(actor,target),window=counterWindows.get(actor.id);
    state.counterTransition=node.stage.step.kind==='counter'&&pair.mode==='reversal'&&pair.initiativeId===actor.id&&window?.against===target.id&&time<=window.until;
    if(state.counterTransition){window.claimed=true;updateExchange(actor,target,{type:'counter-start'});}
    else if(stageDamage(node.stage)>0||(node.phase==='jo'&&cursor.techniqueIndex===0&&cursor.stageIndex===0&&node.stage.step.kind==='ready')){const exchange=updateExchange(actor,target,{type:'commit',phase:node.phase});state.exchangeContinuity=exchange.continuity;}
    state.exchangeSerial=exchangeFor(actor,target).serial;
    if(actor.id==='hero')traceRow({type:'stage-start',phase:node.phase,techniqueId:node.technique.id,techniqueIndex:cursor.techniqueIndex,stageIndex:cursor.stageIndex,kind:node.stage.step.kind,attackId:state.id,distance:Number(distanceBetween(positions,actor,target).toFixed(3))});
    return state;
  }
  function breakChain(actor,state,reason){
    ...
    cursor.stageIndex=0;actionState.delete(actor.id);
  ...
  function finishAction(actor,state,{interrupted=false}={}){
    ...
    if(state.counterTransition){
      const target=battle.actors.get(state.targetId);counterWindows.delete(actor.id);
      if(target)updateExchange(actor,target,{type:'counter-complete'});
      pendingNormalStarts.add(actor.id);readyAt.set(actor.id,Math.max(readyAt.get(actor.id)||0,time+.18));return;
    }
    const targetPair=battle.actors.get(state.targetId),pair=targetPair?exchangeFor(actor,targetPair):null;
    const parryWhiff=state.node.stage.step.kind==='parry'&&state.outcome!=='parry'&&!(pair?.mode==='pressure'&&pair.initiativeId===actor.id);
    const failure=interrupted||(state.outcome==='miss'&&state.majorMiss)||parryWhiff;
    ...
    if(exchangeComplete&&target){
      updateExchange(actor,target,{type:'kyu-complete',phase:'kyu',exchangeSerial:state.exchangeSerial});
    ...
    const settle=exchangeComplete?.3:(state.motion.offense?.05:.12);
  ...
  function currentAction(actor){
    ...
    let pair=exchangeFor(actor,target);
    if(pair.mode==='zanshin'){if(time<(readyAt.get(pair.completedById)||0))return null;updateExchange(actor,target,{type:'settle'});pair=exchangeFor(actor,target);}
    let node=nodeFor(actor,cursorFor(actor));
    const counter=counterWindows.get(actor.id),willCounter=node.stage.step.kind==='counter'&&pair.mode==='reversal'&&pair.initiativeId===actor.id&&counter?.against===target.id&&time<=counter.until;
    // All old action/reaction callbacks have completed above. Only now may the
    // executor prepare a NEW normal sequence; reversal itself never writes it.
    if(stageDamage(node.stage)>0&&!willCounter&&(pendingNormalStarts.has(actor.id)||(pair.mode==='read'&&node.phase!=='jo'))){
      const cursor=cursorFor(actor);cursor.phaseIndex=0;cursor.techniqueIndex=0;cursor.stageIndex=0;cursor.cycle++;
      pendingNormalStarts.delete(actor.id);node=nodeFor(actor,cursor);
      traceRow({type:'normal-offense-start',actorId:actor.id,targetId:target.id,phase:'jo'});
    }
    // Pair-scoped AI response, not an attack legality or world-contact lock.
    // Other enemies continue their own pair, footwork and live contacts.
    if(stageDamage(node.stage)>0&&!willCounter&&['pressure','reversal'].includes(pair.mode)&&pair.initiativeId!==actor.id){
      const incoming=incomingThreat(actor,target);
      if(incoming&&pair.mode==='pressure'&&time>=(reactionCooldowns.get(actor.id)||0)){const defense=beginReaction(actor,target,'guard','opponent-pressure');if(defense){reactionCooldowns.set(actor.id,time+1.15);return actionView(actor,defense);}}
  ...
  function applyContacts(actions){
    ...
    for(const [id,action] of actions){
      ...
      if(defense){
        ...
        const parryStrength=parried?reviewParryStrength(target,targetState,defenseNode):null,strongParry=Boolean(parryStrength?.strong);
        ...
        if(strongParry){state.interrupted='strong-parry';counterWindows.set(target.id,{against:source.id,until:time+1.35,claimed:false,parryDirection});}
      ...
      const deepHit=isDeepJohakyuExchangeHit({damage:result.dealt,maxHp:target.maxHp,outcome:johakyuActorCapability(target)}),exchange=updateExchange(source,target,{type:'hit',phase:action.phase,deep:deepHit,counter:countered});state.exchangeContinuity=exchange.continuity;
      ...
      if(countered){
        ...
        if(targetAction&&!['guard','parry','brace'].includes(targetKind))targetAction.interrupted='countered';
        ...
        targetAction.interrupted='hit-before-contact';setManeuver(target,source,{reason:'hit-withdrawal',footwork:'retreat',seconds:.34,stopDistance:2.44});
      }else if(deepHit&&!targetAction){
  ...
  function facingYaw(actor){const target=selectTarget(battle,actor.id);if(!target)return LAYOUT[actor.id]?.yaw??0;const from=positionOf(positions,actor),to=positionOf(positions,target);return Math.atan2(to.x-from.x,to.z-from.z);}
  ...
  function meta(frameValue){
    ...
    const hudState=johakyuExchangeHudState(exchangeView,{actorId:'hero',phase,reaction:action?.scope==='combat-reaction',counter:action?.counterTransition});
    return freeze({mode,modeLabel:MODES[mode],phase,phaseLabel:PHASE_LABELS[phase],hudState,exchangeMode:exchangeView?.mode??'read',initiativeId:exchangeView?.initiativeId??null,completedById:exchangeView?.completedById??null,exchangeReason:exchangeView?.lastReason??'read',
  ...
  function step(dt=1/60){
    ...
    if(!resumed&&resumeAtSeconds!==null&&time>=resumeAtSeconds){const checkpoint=createJohakyuCheckpoint({battle,lifeId:'review-life',ageSeconds:28*60,encounterId:`review-${encounter}`});battle=restoreJohakyuCheckpoint(checkpoint).battle;epoch++;revision=0;resumes++;resumed=true;actionState.clear();reactionState.clear();reactionCooldowns.clear();counterWindows.clear();exchangeStates.clear();pendingNormalStarts.clear();cursors.clear();maneuvers.clear();recoveries.clear();seedReadyWindow(.08);traceRow({type:'resume',epoch});}
    if(time>=ENCOUNTER_SECONDS||battle.result){encounter++;epoch++;time=0;freshBattle();traceRow({type:'encounter-reset',epoch,encounter});}
