...
export function createJohakyuP7ReviewScenario({mode='duel',duelGap=2.18,enemyLeadSeconds=0,heroStartPhase='jo',heroStartTechniqueIndex=0,checkpointSeconds=0,actorOverrides={},loadout=null,settings=null,learnedTechniqueIds=[],comboStyle='composed',fixture=null}={}){
 ...
 let encounter=1,epoch=1,serial=1,elapsed=0,inspirationPresentationUntil=0,defeats=0,finishers=0,resumes=0,restartedAt=null,runtime,last=null,activeHeroLoadout,activity=[],history=[],replacements=[];
 ...
 function step(dt=1/60,physicalContacts=[],movement=null){
   ...
   for(const event of result.events){record(event);
     ...
     if(event.type==='player-hit'&&elapsed>=inspirationPresentationUntil&&!runtime.actor('hero')?.queuedInspiration&&!runtime.actor('hero')?.override?.inspirationPhase&&hash(event.id)<inspirationChance){
       ...
       if(picked&&definition&&targetId&&runtime.inspire('hero',{...definition,name:picked.name},targetId,phase)){
         known.push(picked.id);knownSet.add(picked.id);inspirationPresentationUntil=elapsed+FIRST_INSPIRATION_PRESENTATION.hudSeconds;config.technique[phase]=picked.id;activeHeroLoadout={...activeHeroLoadout,[phase]:picked.id};hero.loadout=compileBattleLoadout(activeHeroLoadout,weapon);
