...
function fixture(pending=null){
 const canvas=new EventTarget(),ui={pending,drawer:false},calls={pan:[],zoom:[],tap:[],preview:[],commit:0};let now=0,id=0;const frames=new Map();
 const view={pan:(x,y)=>calls.pan.push([x,y]),zoom:f=>calls.zoom.push(f),ground:(x,y)=>({x:x/10,z:y/10}),updateCamera(){},yaw:0};
 const input=installSceneInput(canvas,{view,ui,tap:(x,y)=>calls.tap.push([x,y]),preview:(x,z)=>calls.preview.push([x,z]),commit:()=>{calls.commit++;ui.pending=null;return true;},activity(){},now:()=>now,raf:fn=>{frames.set(++id,fn);return id;},caf:i=>frames.delete(i)});
