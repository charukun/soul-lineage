...
test('title entry is a lifecycle boundary before post-entry enhancement loading',async()=>{
 ...
 assert.match(enhancements,/for\(const modulePath of ORDERED_ENHANCEMENTS\)[\s\S]*const load=ENHANCEMENT_LOADERS\[modulePath\][\s\S]*await load\(\)[\s\S]*await yieldToBrowser\(\)/);
