...
function catalogDetour(ctx,event,button){
 ...
 if(card&&card.dataset.kind!==GUIDE_KIND){
  ...
  void ctx.guide.playDemo({immediate:true});return true;
 }
...
function handlePlacementButton(ctx,event,button){
 ...
 if(button.id==='cancelPlace'){
  event.preventDefault();event.stopImmediatePropagation();
  ctx.guide.setHint(stage==='drag'?'まだ置かずに、1本指で少し大きくなぞります。':'今回はボタンではなく、画面を短くタップして置きます。');
  void ctx.guide.playDemo({immediate:true});return true;
 }
...
function onClick(ctx,event){
 ...
 if(handleBuildClick(ctx,button)||handleCatalogClick(ctx,button))return;
...
function onPointerMove(ctx,event){
 const gesture=ctx.gesture;if(!gesture||event.pointerId!==gesture.id)return;
...
function handleDragRelease(ctx,event,gesture){
 if(!gesture.multi&&gesture.max>=DRAG_DISTANCE&&ctx.active&&ctx.guide.stage==='drag'&&ctx.ui.pending?.kind===GUIDE_KIND){
  accept(ctx,'place','いい位置なら、画面を短く1回タップ。');return;
 }
 if(gesture.multi||!ctx.active||ctx.guide.stage!=='drag')return;
 if(gesture.max<TAP_DISTANCE){
  // Prevent the placement action, not the pointer owner's release/cleanup.
...
function verifyPlacement(ctx){
 ...
 const placed=ctx.world.objects.find(object=>object.kind===GUIDE_KIND&&!ctx.initialGuideIds.has(object.id));
 if(placed&&!ctx.ui.pending){
  const state=ctx.world.state.onboarding?.firstRunAutoplay;
  if(state)state.placedId=placed.id;
  void ctx.village.save();
  ctx.guide.setStage('done');return;
 }
 if(ctx.ui.pending?.error){
  ctx.guide.setHint(`そこには置けません。「${ctx.ui.pending.error}」なので、少し場所をずらしてから短くタップ。`,3800);return;
 }
...
function cleanup(ctx){
 removeListeners(ctx);ctx.view.autoOrbit=ctx.previousAutoOrbit;ctx.guide.destroy();
 ctx.village.activity();ctx.village.updateTutorial();
}
async function complete(ctx,skipped=false){
 if(ctx.finishing)return;
 ctx.finishing=true;ctx.active=false;
 if(skipped){if(ctx.ui.pending)ctx.village.cancelPlacement();if(ctx.ui.drawer)ctx.village.closeDrawer();}
 markFirstRunAutoplaySeen(ctx.world.state);
 try{
  const saved=await ctx.village.save();
  ctx.canvas.dataset.firstRunTutorial=saved===false?'save-error':'seen';
...
function beginAfterEntry(ctx){
 ...
 ctx.entryListener=event=>{
  ...
  document.removeEventListener('click',ctx.entryListener,true);ctx.entryListener=null;
...
export function startFirstRunGuide({village,canvas}){
 ...
 const timing=timingFor(reduced),previousAutoOrbit=view.autoOrbit;
 ...
 const ctx={village,canvas,world,view,ui,guide,timing,previousAutoOrbit,initialGuideIds:new Set(world.objects.filter(o=>o.kind===GUIDE_KIND).map(o=>o.id)),active:true,finishing:false,gesture:null,entryListener:null,handlers:null};
 installListeners(ctx);installControls(ctx);beginAfterEntry(ctx);
 return ctx;
}
