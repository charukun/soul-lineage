...
export function installOnlineHost(container=document.getElementById('onlineHost')){
  ...
  const {world,sim,view,save}=village;
  ...
  const raidHost=new RaidHost({villageId:ownWorldId,storage:localStorage});
  ...
  const root=document.createElement('section');root.className='peer-world-panel';root.innerHTML=`
    ...
    <div class="peer-world-actions">
      <button id="peer-world-host">この村を開く</button>
      <button id="peer-world-handoff" disabled>次の端末へ世界を継ぐ</button>
    </div>
    ...
    <hr>
    <button id="peer-world-make-offer" disabled>仲間用の参加コードを発行</button>
    ...
    <button id="peer-world-accept" disabled>応答を確定</button>
    <pre id="peer-world-debug"></pre>`;
  ...
  function broadcast(message){let sent=0;for(const[id,connection]of direct){try{connection.send(message);sent++;}catch{direct.delete(id);}}if(star&&starHostId&&!direct.has(starHostId)){try{star.send(message);sent++;}catch{}}sent+=mesh?.broadcast(message)||0;return sent;}
  ...
  function phaseChanged(info){
    ...
    $('#peer-world-handoff').disabled=!info.isHost||Object.keys(node?.authority?.members||{}).filter(id=>id!==selfId&&node.authority.members[id].connected&&node.authority.members[id].eligible).length===0;
    $('#peer-world-make-offer').disabled=!info.isHost;$('#peer-world-accept').disabled=!info.isHost;
  ...
  function syncMesh(){if(!node||!mesh)return;const authority=node.authority;if(!authority)return;if(authority.hostId!==selfId)mesh.syncMembers(Object.keys(authority.members||{}),{hostId:authority.hostId});}
  ...
  async function makeOffer(){
    ...
    const tempId=`pending-${++seq}`;let connection;
    connection=await createHostOffer({RTCPeerConnection,dualChannel:true,onState:s=>{if(CLOSED.has(s)){for(const[id,c]of direct)if(c===connection){direct.delete(id);node?.hostDisconnect(id);raidHost.leave(id);}}},onMessage:(message,kind)=>{
      if(message.type==='join'){
        const peerId=String(message.playerId||'');if(!peerId)throw new Error('参加者IDがありません。');
      ...
      handleMessage(message.playerId||tempId,message,kind);
    }});
  ...
  async function joinRemote(){
    ...
    connection=await acceptHostOffer(offer,{RTCPeerConnection,dualChannel:true,onState:s=>{if(CLOSED.has(s)&&node){star=null;node.tick();}},onMessage:(message,kind)=>{
      if(message.type==='world-welcome'){
        starHostId=String(message.authority?.hostId||message.mayorId);star=connection;makeNode(message);remoteSession=true;window.__VILLAGE_REMOTE_WORLD_ACTIVE__=true;window.__VILLAGE_SIMULATION_PAUSED__=true;syncMesh();stateText('共通村の待機Hostとして接続しました。Host移譲時だけ村を復元します。');return;
      }
  ...
  $('#peer-world-host').onclick=()=>startHost().catch(e=>stateText(e.message));
  $('#peer-world-make-offer').onclick=()=>makeOffer().catch(e=>stateText(e.message));
  ...
  $('#peer-world-join').onclick=()=>joinRemote().catch(e=>stateText(e.message));
  $('#peer-world-handoff').onclick=()=>{if(node?.gracefulHandoff())stateText('次のHostへ世界を継いでいます。');};
  ...
  const timer=setInterval(()=>{
    ...
    if(node?.isHost){
      const advanced=advance(now);if(advanced){for(const peerId of Object.keys(node.authority?.members||{})){if(peerId===selfId||!node.authority.members[peerId].connected)continue;const connection=connectionFor(peerId);if(connection)transport.sendObserverSnapshot(peerId,connection,raidHost.snapshotFor(peerId),now);}}
      if(now-lastCheckpointAt>2000){lastCheckpointAt=now;try{node.publishCheckpoint(captureCheckpoint());}catch(error){stateText('Checkpointを更新できません: '+error.message);}}
    }
  ...
  const dispose=()=>{if(disposed)return;disposed=true;clearInterval(timer);if(node?.isHost)node.gracefulHandoff();mesh?.close();for(const connection of direct.values())connection.close?.();star?.close?.();node?.close();overlay.dispose();if(remoteSession){window.__VILLAGE_REMOTE_WORLD_ACTIVE__=false;window.__VILLAGE_SIMULATION_PAUSED__=false;}if(activeSession?.dispose===dispose)activeSession=null;};
  window.addEventListener('pagehide',dispose,{once:true});
  activeSession={dispose,snapshot:()=>({selfId,node:node?.snapshot()||null,mesh:mesh?.snapshot()||null,transport:transport.snapshot(),remoteSession})};
  window.__VILLAGE_PEER_HOSTED_WORLD__=activeSession;
