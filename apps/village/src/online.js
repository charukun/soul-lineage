import{createFriendAutoSignaling}from'./peer-auto-signaling.js';
...
export function installOnlineHost(){const mount=document.getElementById('onlineHost')||document.querySelector('main');if(!mount)return;const root=document.createElement('section');root.id=id;root.innerHTML=`<h2>友人を村へ招待</h2><p id="net-state">招待した時だけ、友人がこの村を見学できます。</p><button id="make-offer">友人招待をつくる</button><textarea id="invite" readonly placeholder="招待リンク"></textarea><div class="invite-actions"><button id="share-invite" disabled>招待を共有</button><button id="copy-invite" disabled>リンクをコピー</button></div><p class="muted">招待は15分で失効します。公開村一覧や魔王軍の襲撃候補には表示されません。</p><textarea id="answer" placeholder="友人から返された参加返事"></textarea><button id="accept-answer" disabled>参加返事を受け取る</button>`;mount.append(root);const $=q=>root.querySelector(q),status=t=>$('#net-state').textContent=t;
 ...
 let activeInvite=null,creating=false,disposed=false,authority=null,autoSignaling=null;
 const authorityTimer=setInterval(()=>authority?.tick(),250);
 $('#make-offer').onclick=async()=>{
  if(creating||disposed)return;creating=true;$('#make-offer').disabled=true;pending?.close?.();pending=null;activeInvite=null;authority?.dispose();authority=null;void autoSignaling?.close();autoSignaling=null;
  ...
  try{
   ...
   connection=await createHostOffer({RTCPeerConnection,onState:s=>{if(disposed)return;if(s==='open')status('友人が接続しました。参加を確認しています。');else if(['failed','closed','disconnected'].includes(s))status('接続が終了しました。');},onMessage:m=>{
    if(disposed||pending!==connection)return;
    if(m?.type==='friend-world'){authority?.receive(m);return;}
    if(m?.type!=='join'||m.role!=='visitor')return;
    try{assertFriendVillageInviteActive(invite);if(authority)throw Error('この招待は参加済みです。');authority=createFriendVisitHost({worldId:snapshot.villageId,selfId:'host-'+snapshot.villageId,send:message=>connection.send(message)});const welcome=authority.admit(m.playerId);connection.send({type:'friend-village',snapshot,welcome});status('友人が接続しました。村の外観だけを共有しています。');}catch(e){connection.close();status(e.message);}
   }});
   ...
   void createFriendAutoSignaling({invite,acceptAnswer:async answer=>{if(disposed||pending!==connection)throw Error('招待を作り直しています。');assertFriendVillageInviteActive(invite);await connection.accept(answer);},snapshot:()=>authority?.snapshot(),onStatus:text=>{if(!disposed&&pending===connection)status(text);}}).then(auto=>{if(disposed||pending!==connection){void auto.close();return;}autoSignaling=auto;invite=auto.invite;activeInvite=invite;$('#invite').value=friendVillageInviteUrl(new URL('./friend.html',location.href),invite);}).catch(()=>{if(!disposed&&pending===connection&&!authority)status('このリンクを友人へ送ってください。参加返事は手動で受け取れます。');});
 ...
 window.addEventListener('pagehide',()=>{disposed=true;clearInterval(authorityTimer);pending?.close?.();pending=null;activeInvite=null;authority?.dispose();authority=null;void autoSignaling?.close();autoSignaling=null;},{once:true});
}
