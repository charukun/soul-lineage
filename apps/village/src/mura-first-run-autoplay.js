...
function install(){
 ...
 function accept(next,message){
  ...
  clearTimeout(demoTimer);
  finger.hidden=true;ripple.hidden=true;dragStart.hidden=true;dragEnd.hidden=true;
  // Move the state machine immediately after the real input succeeds. This
  // prevents a fast second gesture from outrunning a timed confirmation pause.
  setStage(next,{message});
  layer.classList.add('mura-first-run-accepted');
  setTimeout(()=>{
   if(!active||token!==transitionRun||stage!==next)return;
   layer.classList.remove('mura-first-run-accepted');
   text.textContent=STAGES[next].text;
  },timing.confirm);
 }
 ...
 function onClick(event){
  ...
  if(stage==='catalog'){
   ...
   if(button.matches('#tabs [data-category]')&&button.dataset.category!=='住まい'){
    event.preventDefault();
    event.stopImmediatePropagation();
    setHint('最初は「住まい」のままで大丈夫です。光っている空きテントを選んでください。');
    void playDemo({immediate:true});
    return;
   }
  }
  ...
  if(stage==='build'&&button.id==='build'){
   requestAnimationFrame(()=>{
    if(active&&ui.drawer)accept('catalog','そうです。次は、実際の住まいを選びます。');
   });
   return;
  }
  ...
  if(stage==='catalog'&&button.matches('#catalog .card[data-kind="tent"]')){
   requestAnimationFrame(()=>{
    if(active&&ui.pending?.kind===GUIDE_KIND)accept('drag','配置モードに入りました。次は場所を自分の指で動かします。');
   });
   return;
  }

  if(stage==='catalog'&&(button.id==='build'||button.id==='closeDrawer')){
   requestAnimationFrame(()=>{
    if(active&&!ui.drawer)setStage('build',{message:'閉じても大丈夫です。もう一度「つくる」から試せます。'});
   });
   return;
  }
 ...
 function finishGesture(event){
  ...
  if(completed.stage==='drag'){
   if(!completed.multi&&completed.max>=DRAG_DISTANCE&&active&&stage==='drag'&&ui.pending?.kind===GUIDE_KIND){
    accept('place','場所を動かせました。最後は、画面を短くタップして置きます。');
    return;
   }
   if(!completed.multi&&completed.max<7&&active&&stage==='drag'){
    ...
    void playDemo({immediate:true});
   }else if(!completed.multi&&completed.max<DRAG_DISTANCE&&active&&stage==='drag'){
    setHint('動かし方は合っています。もう少しだけ大きく、ゆっくりなぞってみてください。');
    void playDemo({immediate:true});
   }
   return;
  }
