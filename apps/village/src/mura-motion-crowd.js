...
function install(){
 ...
 window.__MURA_MOTION_CROWD__={snapshot:()=>({version:2,active:records.size,maxSuggestion:.18,worldAuthority:false,navigationConsumerRequired:true,records:[...records].map(([id,row])=>({id,bias:row.bias,desiredVelocity:row.desiredVelocity,appliedToWorld:row.appliedToWorld,at:row.at}))})};
}
