...
export class View{
 ...
 this.canvas=canvas;this.world=world;this.scene=new T.Scene();this.scene.background=new T.Color(0x9eaaa4);this.scene.fog=new T.Fog(0x9eaaa4,112,390);
 this.renderer=new T.WebGLRenderer({canvas,antialias:true,preserveDrawingBuffer:true,alpha:false});this.renderer.setPixelRatio(Math.min(devicePixelRatio||1,1.5));this.renderer.outputColorSpace=T.SRGBColorSpace;this.renderer.toneMapping=T.ACESFilmicToneMapping;this.renderer.toneMappingExposure=1.04;
 ...
 this.sun=new T.DirectionalLight(0xffe0b2,2.65);this.sun.position.set(-45,90,60);this.sun.castShadow=true;this.sun.shadow.mapSize.set(this.softwareGPU?1024:2048,this.softwareGPU?1024:2048);Object.assign(this.sun.shadow.camera,{left:-100,right:100,top:100,bottom:-100,near:1,far:230});this.sun.shadow.bias=.00035;this.sun.shadow.normalBias=.18;this.scene.add(this.sun,this.sun.target);
 this.hemi=new T.HemisphereLight(0xbfd2d7,0x526b58,1.72);this.scene.add(this.hemi);this.hearthGlow=new T.PointLight(0xffb36b,.78,24,2);this.hearthGlow.position.set(2.5,3.4,1.5);this.scene.add(this.hearthGlow);
 ...
 this.makeTerrain();this.makePost();this.setupSelection();this.roomId=null;this.blur=.85;this.autoOrbit=innerWidth>=700;this.interacting=false;this.lastInteraction=0;this.cameraGoal=null;this.followId=null;this.observation=null;this.resize();this.rebuild();this.thumbCache={};
 }
 ...
 setTime(hour){const night=hour>=19||hour<5.5,evening=hour>=16.5&&hour<19;this.night=night;this.sun.intensity=night?.22:evening?1.65:2.65;this.sun.color.set(night?0xb1c9ea:evening?0xffbd84:0xffe0b2);this.hemi.intensity=night?.7:1.7;this.hemi.color.set(night?0x93afd2:0xdceafa);this.hemi.groundColor.set(night?0x4c6555:0x8e9f6b);const bg=night?0x1d3040:evening?0x8e8278:0x9eaaa4;this.scene.background.set(bg);this.scene.fog.color.set(bg);this.renderer.toneMappingExposure=night?.98:evening?1.01:1.04;this.hearthGlow.intensity=night?1.55:evening?1.12:.62;this.waterMat.uniforms.tint.value.set(night?0x345369:0x65a4ae);}
 ...
 this.blurMaterial.uniforms.strength.value=this.world.state.settings.tilt;const autoReset=this.renderer.info.autoReset;this.renderer.info.autoReset=false;this.renderer.info.reset();try{this.renderer.setRenderTarget(this.rt);this.renderer.render(this.scene,this.camera);this.renderer.setRenderTarget(null);this.renderer.render(this.postScene,this.postCamera);}finally{this.renderer.info.autoReset=autoReset;}
 }
