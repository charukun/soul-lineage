...
export function installPopulationHistory({world}){
 ...
 const chart=dialog.querySelector('.populationChart'),summary=dialog.querySelector('.populationHistorySummary'),back=dialog.querySelector('.populationHistoryBack'),hostContent=document.getElementById('dialogContent'),hostDialog=document.getElementById('dialog');let reopenSettings=null;

 function render(){
  const current=world.population(),cohorts=window.__MURA_SYSTEMS__?.cohortCounts?.()||{},stored=world.state.muraSystems?.demography?.history||[],fallback={year:Math.floor(world.state.clock/DAYS_YEAR)+1,population:current.people,births:0,deaths:0,departures:0,children:cohorts.children||0,youth:cohorts.youth||0,adults:cohorts.adults||current.people,elders:cohorts.elders||0},points=populationHistorySeries(stored.length?stored:[fallback]),latest=points.at(-1)||fallback;
 ...
 function open(){if(hostDialog?.open)hostDialog.close();render();if(!dialog.open)dialog.showModal();}
 ...
 const observer=hostContent?new MutationObserver(()=>{if(hostDialog?.dataset.page==='settings'&&reopenSettings)attachSettings(hostContent,reopenSettings);}):null;observer?.observe(hostContent,{childList:true,subtree:false});
