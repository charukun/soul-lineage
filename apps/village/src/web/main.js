...
function loop(now){if(stopped)return;try{const dt=document.hidden?0:Math.min(.25,(now-last)/1000);last=now;elapsed+=dt;frames++;frameSeconds+=dt;
 ...
 updateBubbles();if(elapsed-lastSave>12){lastSave=elapsed;save();}view.interacting=!!ui.pending||!!ui.drag||pointers.size>0||$('dialog').open||ui.drawer;view.render(elapsed,dt);requestAnimationFrame(loop);
