...
export function installInterface(village){
 ...
 const syncDeliveries=now=>{
  ...
  for(const receipt of fresh){const node=document.createElement('div'),entries=Object.entries(receipt.items);node.className='muraDeliveryReceipt';node.dataset.storageId=receipt.storageId;node.dataset.until=String(now+1900);node.setAttribute('aria-label',`資材庫へ搬入 ${entries.map(([k,n])=>`${RESOURCE_NAMES[k]} +${formatDeliveryAmount(n)}`).join('、')}`);node.innerHTML=entries.map(([k,n])=>`<span class="muraDeliveryItem muraTone-${k}" title="${RESOURCE_NAMES[k]}">${hudIcon(k)}<b>+${formatDeliveryAmount(n)}</b></span>`).join('');deliveries.append(node);deliveryNodes.set(receipt.revision,{node,receipt});shownDeliveryRevision=Math.max(shownDeliveryRevision,receipt.revision);}
  for(const[revision,item]of deliveryNodes){const {node,receipt}=item,storage=world.object(receipt.storageId);if(now>Number(node.dataset.until)||!storage){node.remove();deliveryNodes.delete(revision);continue;}const point=view.project(storage.x,4.8,storage.z),slot=receipt.revision%3;node.hidden=ui.entryOpen||!!ui.pending||!!view.roomId||!!view.observation||point.x<34||point.x>view.w-34||point.y<54||point.y>view.h-70;node.style.left=point.x+'px';node.style.top=(point.y-slot*15)+'px';}
 };
 ...
 function statusTick(now){
  const climate=window.__宝満叡智_V2_UI__?.currentClimate(),year=Math.floor(world.state.clock/DAYS_YEAR)+1,population=world.population();
