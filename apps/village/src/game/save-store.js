...
export function createSaveStore(platform) {
 ...
 return {
  ...
  async recover(){if(peerReadOnly)throw new Error('共通村へ接続中は個人ローカル保存を復旧できません');await tail.catch(()=>{});const[base,journalText,compact]=await Promise.all([platform.storage.read(SAVE_KEY),platform.storage.read(JOURNAL_KEY),platform.storage.read(COMPACT_KEY)]);if(base!==null||journalText!==null||compact!==null){const prefix=`${SAVE_KEY}.recovery.${platform.clock.now()}`;if(base!==null)await platform.storage.write(`${prefix}.base`,base);if(journalText!==null)await platform.storage.write(`${prefix}.journal`,journalText);if(compact!==null)await platform.storage.write(`${prefix}.compact`,compact);}await platform.storage.remove(SAVE_KEY);await platform.storage.remove(JOURNAL_KEY);await platform.storage.remove(COMPACT_KEY);blocked=false;error=null;revision=baseRevision=0;lastPayload=null;journal=null;},
  flush:()=>tail,
  diagnostics:()=>Object.freeze({revision,baseRevision,journalEntries:journal?.entries?.length||0,journalBytes:journalBytes(journal),peerReadOnly}),
