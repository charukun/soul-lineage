...
実ブラウザでの初回プレイを基準に、既存の村シミュレーションを増やすより先に「触りたいものをすぐ触れる → 行動の意味が分かる → 村が目に見えて変わる」を優先する。

- 建物や住民は表示メッシュより広いタップ判定を持ち、人物・UIとの競合時も近い候補へ吸着して原則1タップで選択できる。ドラッグ操作はタップと明確に分離する。
- 初回テントはチュートリアルから直接配置モードへ入り、推奨地点の提示と地面タップ／確定までを短い導線で完了できる。既存の任意配置、地形制約、連続配置は壊さない。
- セーブがある帰還プレイヤーは毎回のタイトル／入村操作を要求せず、現在の村へ直接復帰できる。初回入口は維持する。
- 通常時のUIは3D世界を主役にし、同時に強調する主要パネルを1系統へ抑える。チュートリアル完了後は小さな次目標へ縮退する。
- 建築、寝床増加、資源発見、施設解放、安全度変化など主要な状態変化はトーストだけで終わらせず、短い視覚フィードバックとSE／住民リアクションで因果を伝える。
- 5手順チュートリアルは「なぜ次の行動をするか」が分かる文脈を持ち、終了後も村の状態から次目標を提示する。
- 住民観察はカメラ機能だけにせず、名前・生活・住まい・仕事・気分・近況を短く読める。通常カメラ操作で即座に解除できる既存挙動を維持する。
- 通常操作中は可読性を優先してチルトシフト／被写界深度を弱め、選択対象と住民を背景から判別しやすくする。観察時のミニチュア感は維持する。
- 雨、夜、野生動物、襲撃など既存シミュレーションの変化を、住民行動・音・短い演出としてプレイヤーへ見せる。
- 回帰テストでは少なくとも「初回テント配置」「建物1タップ選択」「室内入退室」「セーブ復帰」「住民観察」「スマホ幅で横スクロールなし」を実操作で維持する。
