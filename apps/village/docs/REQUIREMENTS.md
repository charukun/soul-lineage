...
住民の年齢は実時間ではなく、既存の村世界時計 `state.clock` と `DAYS_YEAR` を正本として進めます。保存データには年齢の基準値と基準日を保持し、ロード中やフレームごとの加算誤差ではなく世界時間から現在年齢を再計算します。

- ライフステージは幼年（13歳未満）・青年（13〜17歳）・成人（18〜59歳）・老年（60歳以上）とする。
- 初期の村長・護衛には成人年齢を持たせ、新規移住者は決定論的な年齢を持って加入するため、若い住民は世界時間の経過で実際に成人する。
- 16歳未満は通常就労へ割り当てず、警備職は18歳以上のみ対象とする。既存の村長・護衛・一族プレイヤーの特別契約は維持する。
- 描画は既存の軽量住民モデルを使い続け、年齢に応じた身長比率と老年の髪色など最小限の差分だけを加える。年齢表現のために外部モデルや追加テクスチャをロードしない。
- 人物詳細には現在の年齢とライフステージを表示する。
- v4/v5の既存セーブに年齢項目がなくても読み込める。既存住民にはロード時点の世界時間を基準として安定した初期年齢を補完し、過去の経過時間を遡って一気に老化させない。
- 今回は寿命による自然死や出生システムを追加しない。既存の負傷・救助・死亡・襲撃契約を変更しない。
...
建物・庭・家具を選択した後は、配置候補のゴーストを画面中央の照準位置に保ちます。1本指スワイプは通常と同じ向きで地面と視点を動かし、その中央位置へゴーストの候補座標を連続追従させます。配置中の短いタップは、現在表示されている候補をそのまま確定します。設置不可なら確定せず、既存の配置理由を表示します。

回転はゴースト近傍の明示コントロールから行い、二本指操作を建築配置の必須操作にはしません。配置直後の「取り消す」ボタンは表示せず、通常の操作へ戻します。配置前のキャンセル、「つくる」内の取り消しと既存の `World.undo()` 契約は維持します。移動配置でも同じ中央固定・スワイプ・タップ確定を使い、確定時には現在の村状態で `canPlace` / `move` / `add` を再検証します。

配置UIの状態同期は冪等であることを必須とします。表示更新が監視対象DOMを無条件に書き換えて `MutationObserver` 自身を再発火させ続けてはならず、建物を選択した直後でもメインスレッドを占有せず入力可能な状態を維持します。
