...
回転はゴースト近傍の明示コントロールから行い、二本指操作を建築配置の必須操作にはしません。配置直後は誤配置をすぐ戻せる「取り消す」を提示し、既存の `World.undo()` 契約で直前の配置を戻せることを維持します。移動配置でも同じ中央固定・スワイプ・タップ確定を使い、確定時には現在の村状態で `canPlace` / `move` / `add` を再検証します。
