...
村人の年齢・出生・死亡・離村は、端末の実時間ではなく保存される村の世界時計を正本にします。食料・安全・住居・年齢構成に余裕がある村では世代が増え、生活基盤が崩れれば出生が止まり、死亡や離村で人口が減るため、長期の繁栄と衰退が同じ時間軸で進みます。

受入条件は次のとおりです。

- 村時間の年単位で年齢構成を更新し、子ども・成人・高齢者の比率を人口循環へ反映する。
- 出生は十分な住居・食料・安全と成人世代がそろった場合にのみ起き、人口上限を超えない。
- 高齢化・老衰と、深刻な食料不足による離村を人口減少として記録する。
- 人口履歴を村時間とともに保存し、人口・出生・死亡/離村の推移をゲーム内グラフで確認できる。
- グラフ表示のために外部チャート依存を追加せず、既存UIの軽量な描画で確認できる。

## 村施設の強化

完成済みの一般施設は、既存の建築状態・内装・配置を保ったまま段階的に強化できます。施設を選んだ場から強化へ進め、資材を投入する前に「いま何ができるか」と「次の段階で何が増えるか」を比較できることを基本とします。

受入条件は次のとおりです。

- 完成済みの一般施設（焚き火と開始時専用施設を除く）を選択すると、詳細画面まで掘らずに強化操作へ進める。
- 強化は1段階目から3段階目までとし、最大段階ではそれ以上の操作を開始しない。
- 強化開始前に必要資材・不足資材と、定員・就労枠・生産量・守り・保管など、その施設で実際に変わる効果を現在値→次段階で表示する。
- 資材不足時は不足量を示して開始できないようにする。資材がそろっている場合だけ既存の増築処理を開始する。
- 強化中は既存の建築進捗表示を利用し、施設の配置・内装・住民との紐付けを維持する。
- 既存の `level` / `upgrade` 状態と保存互換をそのまま使い、新しい並行進行データや別の強化セーブを増やさない。
