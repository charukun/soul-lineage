...
const sound=new Sound(settings.volume);const assets=new Assets();let world,effects,game,previous=performance.now(),uiElapsed=0,modal=null,modalReturn=null,pausedFrom='playing',announcerTimer,toastTimer,fpsFrames=0,fpsTime=0,lastFps=0,frameId;
...
function openPause(){if(!game||!['playing','upgrade'].includes(game.state)||modal)return;pausedFrom=game.state;game.state='paused';modal='pause';hide('upgrade-screen');show('pause-screen');sound.suspend();}
function resume(){if(game.state!=='paused')return;game.state=pausedFrom;modal=null;hide('pause-screen');if(game.state==='upgrade')show('upgrade-screen');previous=performance.now();sound.resume();}
...
function closePanel(){hide(`${modal}-screen`);const returnTo=modalReturn;modal=null;if(returnTo==='pause'){modal='pause';show('pause-screen');}else if(returnTo==='title'){show('title-screen');}else if(returnTo==='result'){show('result-screen');}else{game.state=pausedFrom;stateChanged(game.state);sound.resume();}previous=performance.now();}
function updateSettings(){$('quality-setting').value=settings.quality;$('volume-setting').value=Math.round(settings.volume*100);$('shake-setting').checked=settings.shake;}
...
function handleAction(action){
  ...
  if(action==='restart'){sound.start();game.start();}
...
function bind(){
  $('start-button').onclick=()=>{sound.start();game.start();};
  ...
  $('quality-setting').onchange=e=>{world.setQuality(e.target.value);syncSettings();};$('volume-setting').oninput=e=>{settings.volume=Number(e.target.value)/100;sound.volumeTo(settings.volume);syncSettings();};$('shake-setting').onchange=e=>{settings.shake=e.target.checked;syncSettings();};
  ...
  addEventListener('blur',()=>{keys.clear();release();openPause();sound.suspend();});
  document.addEventListener('visibilitychange',()=>{if(document.hidden){keys.clear();release();openPause();sound.suspend();}previous=performance.now();});
  addEventListener('resize',()=>{world.resize();effects.resize();});
  canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();openPause();showFatal('描画が中断されました。再読み込みしてお試しください。');});
}
function animate(now){
  frameId=requestAnimationFrame(animate);if(document.hidden){previous=now;return;}
  const dt=Math.min(.04,(now-previous)/1000);previous=now;fpsFrames++;fpsTime+=dt;if(fpsTime>=1){lastFps=Math.round(fpsFrames/fpsTime);fpsFrames=0;fpsTime=0;}
  const active=!modal&&game.state!=='paused';if(active){game.update(dt);effects.update(dt);}world.update(active?dt:0,game.hero);world.render();effects.render(game.hero,game.enemies,game.telegraphs);uiElapsed+=dt;if(uiElapsed>.08){updateHUD();uiElapsed=0;}
}
...
try{
  ...
  window.__ECLIPSE__={snapshot:()=>({...game.snapshot(),fps:lastFps,drawCalls:world.renderer.info.render.calls,triangles:world.renderer.info.render.triangles,quality:world.low?'low':'high',audio:sound.ctx?.state||'not-started',modal,loaded:assets.models.size}),version:'1.0.0',assetManifest:assets.manifest};
  if(new URLSearchParams(location.search).get('test')==='1')window.__ECLIPSE_TEST__={game,world,effects,sound,step:(seconds)=>{for(let t=0;t<seconds;t+=1/60){game.update(1/60);effects.update(1/60);if(game.state==='result')break;}world.update(.1,game.hero);world.render();effects.render(game.hero,game.enemies,game.telegraphs);updateHUD();return game.snapshot();},stopRender:()=>cancelAnimationFrame(frameId)};
