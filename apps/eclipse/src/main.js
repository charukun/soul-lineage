...
function updateSettings(){document.querySelectorAll('[data-quality]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.quality===settings.quality)));$('volume-setting').value=Math.round(settings.volume*100);$('shake-setting').setAttribute('aria-pressed',String(settings.shake));$('shake-setting').textContent=settings.shake?'ON':'OFF';}
...
function bind(){
  ...
  addEventListener('keydown',e=>{if(e.target.matches('input,button'))return;const k=e.key.toLowerCase();if(['w','a','s','d','arrowup','arrowdown','arrowleft','arrowright',' ','1','2','3','escape'].includes(k))e.preventDefault();if(k==='escape'){if(modal==='settings'||modal==='credits')closePanel();else if(modal==='pause')resume();else openPause();return;}if(k===' '){if(game.state==='title'){$('start-button').click();return;}if(modal==='pause')resume();else openPause();return;}if(['1','2','3'].includes(k)&&!e.repeat){game.skill(Number(k)-1);return;}keys.add(k);keyInput();});
