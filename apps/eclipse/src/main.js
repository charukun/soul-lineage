...
function updateSettings(){for(const button of $('quality-setting').querySelectorAll('[data-quality]'))button.setAttribute('aria-pressed',String(button.dataset.quality===settings.quality));$('volume-setting').value=Math.round(settings.volume*100);const shake=$('shake-setting');shake.setAttribute('aria-checked',String(settings.shake));shake.querySelector('span').textContent=settings.shake?'ON':'OFF';}
...
function bind(){
  ...
  $('quality-setting').onclick=e=>{const button=e.target.closest('[data-quality]');if(!button)return;settings.quality=button.dataset.quality;world.setQuality(settings.quality);syncSettings();updateSettings();};$('volume-setting').oninput=e=>{settings.volume=Number(e.target.value)/100;sound.volumeTo(settings.volume);syncSettings();};$('shake-setting').onclick=()=>{settings.shake=!settings.shake;syncSettings();updateSettings();};
  ...
  addEventListener('keydown',e=>{if(e.target.matches('input,button,a'))return;const k=e.key.toLowerCase();if(['w','a','s','d','arrowup','arrowdown','arrowleft','arrowright',' ','1','2','3','escape'].includes(k))e.preventDefault();if(k==='escape'){if(modal==='settings'||modal==='credits')closePanel();else if(modal==='pause')resume();else openPause();return;}if(k===' '){if(game.state==='title'){$('start-button').click();return;}if(modal==='pause')resume();else openPause();return;}if(['1','2','3'].includes(k)&&!e.repeat){game.skill(Number(k)-1);return;}keys.add(k);keyInput();});
