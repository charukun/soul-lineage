...
test('every acquired original is pinned, licensed and independently re-hashed',()=>{
 const acquired=JSON.parse(readFileSync(new URL('../public/models/manifest.json',import.meta.url),'utf8'));assert.equal(manifest.length,22);assert.equal(acquired.length,manifest.length);
 for(const a of acquired){const expected=manifest.find(m=>m.id===a.id);assert.ok(expected);assert.equal(a.license,'CC0-1.0');assert.equal(a.revision,expected.revision);assert.match(a.revision,/^[a-f\d]{40}$/);assert.equal(a.repository,'agentkaerf/FreeModels');assert.match(a.sha256,/^[a-f\d]{64}$/);assert.match(a.gitBlob,/^[a-f\d]{40}$/);const bytes=readFileSync(new URL('../public/'+a.file,import.meta.url));assert.equal(bytes.length,a.bytes);assert.ok(a.bytes>0);assert.equal(createHash('sha256').update(bytes).digest('hex'),a.sha256);assert.equal(createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex'),a.gitBlob);}
 const audit=JSON.parse(readFileSync(new URL('../public/models/exclusion-audit.json',import.meta.url),'utf8'));assert.deepEqual(audit.collisions,[]);assert.equal(audit.generatedModels,0);assert.ok(Object.values(audit.excludedSnapshots).every(s=>s.models.length>=100));
});
