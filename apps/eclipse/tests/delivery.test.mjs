...
const publicAsset=path=>new URL(path,root);
const sha256=b=>createHash('sha256').update(b).digest('hex');
test('compatible UAL1 is an independently hashed motion dependency, not a displayed mannequin',()=>{
 const manifest=JSON.parse(readFileSync(publicAsset('models/manifest.json'),'utf8'));
 ...
 const bytes=readFileSync(publicAsset(record.file));assert.equal(sha256(bytes),source.expectedSHA256);
...
test('texture delivery is compact, independently hashed, and leaves original source images intact',()=>{
 const delivery=JSON.parse(readFileSync(publicAsset('models/delivery.json'),'utf8'));
 ...
 for(const record of delivery.textures){
  const original=readFileSync(publicAsset(record.original.split('/').map(encodeURIComponent).join('/')));
  const delivered=readFileSync(publicAsset(record.file));
