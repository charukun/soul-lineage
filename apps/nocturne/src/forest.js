// Only clone or instance geometry from the downloaded models. No procedural solids.
...
function buildForest(){
 // The floor itself is an imported GLB, not a generated plane.
 ...
 original.traverse(o=>{if(o.isMesh){o.castShadow=false;o.receiveShadow=true;o.material=tintMaterial(o.material,'ground_grass');o.userData.assetSource='nature/ground_grass';}});scene.add(floor);
 for(let x=-2;x<=2;x++)for(let z=-2;z<=2;z++){if(Math.hypot(x,z)>2.6)continue;const p=prop('ground_pathTile',x*3.45,z*3.45,3.5,Math.floor(rand()*4)*Math.PI/2,true,-.16);p.scale.y*=.12;}
 prop('path_stoneCircle',0,-.4,6.7,0,true,-.02);
 const treeKinds=['tree_pineTallA_detailed','tree_pineTallB_detailed','tree_pineRoundC','tree_detailed_dark'];
 for(let k=0;k<treeKinds.length;k++){const placements=[];for(let i=0;i<29;i++){const a=rand()*TAU,r=randRange(11.3,30);placements.push({x:Math.cos(a)*r,z:Math.sin(a)*r,h:randRange(6.0,10.2),r:rand()*TAU});}batch(treeKinds[k],placements);}
 ...
 for(let i=0;i<7;i++)prop('path_stone',Math.sin(i)*.4,8+i*1.8,1.9,rand()*.4,true,-.03);
}
