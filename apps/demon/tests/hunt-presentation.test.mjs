...
test('feeding action stays committed long enough to complete and carries growth into capture',()=>{
...
test('motion is continuous, finite, bounded, deterministic and clamps invalid input',()=>{
 for(let i=0;i<=1000;i++){
  ...
  for(const [k,v] of Object.entries(p))if(typeof v==='number'){
   assert.ok(Number.isFinite(v));
   assert.ok(Math.abs(v)<2);
   assert.ok(Math.abs(v-prev[k])<.1,`${k} discontinuity`);
  }
 }
 assert.deepEqual(sampleDevourMotion(-1),sampleDevourMotion(0));
 assert.deepEqual(sampleDevourMotion(NaN),sampleDevourMotion(0));
