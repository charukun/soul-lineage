...
test('demon GPU-aware bridge controls render, VFX, streaming, occlusion, alpha pressure, thermal pressure and quality floor',()=>{
  for(const token of ['createGpuAwareQualityGovernor','createGpuTimer','createPerformanceRecorder','createConservativeOcclusionCuller','batchStaticMeshes','createThermalTrendGovernor','applyVisualQualityFloor','markVisualQualityPriority','auditTransparency','combineTransparencyAudits','transparentDrawCalls','transparentTriangleUpperBound','demon-first-village-v1','signature','shadowScale','applyTextureQuality','vfxScale','createVisualDistanceStreamer','presentationDistance'])assert.match(adaptive,new RegExp(token));
