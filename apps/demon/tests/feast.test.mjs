...
test('next prey prefers unknown memory and yields to return, battle and sealed sanctuary',()=>{
 const {game}=fixture();game.profile.unlocked=['traveller'];game.village.npcs=[{id:'known',role:'traveller',x:0,z:1},{id:'fresh',role:'bellkeeper',x:4,z:4},{id:'sealed',role:'smith',x:0,z:-23},{id:'eaten',role:'hunter',x:0,z:1,eaten:true}];
 const before=JSON.stringify(game.village.npcs);assert.equal(nextPrey(game,{revealed:true}).npc.id,'fresh');assert.equal(JSON.stringify(game.village.npcs),before);assert.equal(nextPrey(game,{returning:true}),null);
 game.fight={};assert.equal(nextPrey(game,{revealed:true}),null);game.fight=null;game.village.npcs[1].eaten=true;assert.equal(nextPrey(game,{revealed:true}).npc.id,'known');game.village.npcs[0].z=20;assert.equal(nextPrey(game),null);game.scent=2;assert.equal(nextPrey(game).npc.id,'known');
});
test('screen bearings agree with the real game camera right and forward axes',()=>{const p={x:0,z:0},a=.33;assert.ok(Math.abs(scentBearing(p,{x:-Math.sin(a),z:-Math.cos(a)}))<1e-12);assert.ok(Math.abs(scentBearing(p,{x:Math.cos(a),z:-Math.sin(a)})-Math.PI/2)<1e-12);});
test('real Three effects keep a fixed object budget and clear particles/light on cancel and new village',()=>{
 ...
 game.player.devourProgress=.55;game.devour={npc:game.village.npcs[0]};fx.update(game);assert.equal(fx.root.visible,true);assert.equal(fx.souls.geometry.attributes.position.count,84);assert.ok([...fx.positions].every(Number.isFinite));
 ...
 game.consume(game.village.npcs[0]);const e=consumeEvent(game);for(let i=0;i<40;i++){fx.event({...e,at:game.time});game.time+=.1;fx.update(game);assert.equal(fx.root.children.length,count);assert.ok([...fx.positions,...fx.wispPositions].every(Number.isFinite));}
 game.time+=3;fx.update(game);assert.equal(fx.root.visible,false);assert.equal(fx.light.intensity,0);fx.reset();assert.equal(fx.beacon.visible,false);fx.dispose();assert.equal(scene.children.length,0);
});
test('release opens real creature arms, keeps soles planted and yields to movement',()=>{
 for(const form of ['hollow','stalker','brute','wraith']){const g=createCreature(true),a={x:0,z:0,yaw:0,speed:0};animateCreature(g,a,0,{form});const rest=g.userData.limbs[0].arm.hand.position.clone();animateCreature(g,a,0,{form,feast:1});g.updateMatrixWorld(true);for(const {leg} of g.userData.limbs){const pos=leg.foot.getWorldPosition(new T.Vector3());assert.ok(pos.y>.065&&pos.y<.09,`${form} foot ${pos.y}`);}assert.ok(g.userData.limbs[0].arm.hand.position.distanceTo(rest)>.2);animateCreature(g,{...a,speed:1,walk:1},0,{form,feast:1});const walking=g.userData.limbs[0].arm.hand.position.clone();animateCreature(g,{...a,speed:1,walk:1},0,{form});assert.ok(g.userData.limbs[0].arm.hand.position.distanceTo(walking)<1e-12);animateCreature(g,a,0,{form,feast:0});assert.ok(g.userData.limbs[0].arm.hand.position.distanceTo(rest)<1e-12);}
});
test('audio bite accents cross each authored bite only once, including low frame rate',()=>{const a=new NightAudio(),calls=[];a.tone=(...args)=>calls.push(args);a.crunch=()=>calls.push('crunch');for(const p of [0,.29,.52,.53,.72,.74,null])a.feed(p);assert.equal(calls.filter(c=>c==='crunch').length,2);a.feed(.2);a.feed(null);assert.equal(calls.filter(c=>c==='crunch').length,2);});
