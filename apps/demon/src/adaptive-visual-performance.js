...
function apply(view,snapshot,state=governors.get(view),{forceScene=false}={}) {
  ...
  if(view.environment){
    ...
    for(const child of view.environment.children||[]){child.userData.visualQualityLevel=snapshot.level;const profileId=child.userData?.stylizedArt?.profileId;if(profileId)applyStylizedShading(child,profileId);}
  }
  if(view.actors){
...
if(typeof update==='function'&&!update.__adaptiveVisualPerformance){
 const wrapped=function adaptiveDemonUpdate(game,dt,...rest){
  ...
  if((state.occlusionFrame++%10)===0)state.occlusion.update({camera:this.camera,...state.occlusionRoots});
  return result;
 };
