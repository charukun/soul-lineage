...
function pose(bones,time,npc,motion,combat){
 ...
 const p=combat?.pose,cw=combat?.eased||0;if(p&&cw>0){bones.spine.quaternion.multiply(Q.setFromAxisAngle(AXIS_X,clamp(p.pitch||0,-.55,.55)*cw));bones.spine.quaternion.multiply(Q.setFromAxisAngle(AXIS_Y,clamp(p.twist||0,-.45,.45)*cw));bones.rightUpperArm.quaternion.multiply(Q.setFromAxisAngle(AXIS_X,-.72*cw));bones.rightLowerArm.quaternion.multiply(Q.setFromAxisAngle(AXIS_X,-.42*cw));bones.leftUpperArm.quaternion.multiply(Q.setFromAxisAngle(AXIS_X,-.28*cw));}
