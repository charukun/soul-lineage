...
import {pickRandomRaid} from './raid-routes.js';
...
import {FeastHud} from './feast-hud.js';
...
function event(e){
  ...
  feastHud?.event(e);view?.event(e);audio.event(e);
  if(e.type==='engage'&&!e.group)toast(e.npc.name+'と対峙。退けば間合いは切れる。',2.2);
  if(e.type==='learn'&&e.first)toast(e.name+'を喰らい、身体が写した。',3.5);
...
function newSession(v){
  guideState={sensed:false,lineageSeen:false};refresh();feastHud?.reset();
  game=new RaidSession(v,profile,{
    event,
    consume(role,options){
      const first=store.consume(role,options);refresh();
      if(first){
        const n=PREY[role];toast(n.power+'が、喰らった身体から刻まれた。',5.5);
...
async function claimAndEnter(v){try{pauseInput();await view.prepareCharacter(activeCharacter(store.read()).id);store.claim(v);closeSheet();newSession(v);mode='hunt';returnMode=false;$('title').hidden=true;$('hud').hidden=false;audio.start();toast('夜へ入った。喰痕を残せ。',3);last=0;acc=0;}catch(e){showError(e.message);}}
async function randomHunt(){
 try{refresh();const offers=offerVillages(store),v=pickRandomRaid(offers,profile);if(!v)throw Error('今夜の村を見つけられませんでした。');await claimAndEnter(v);}catch(e){showError(e.message||String(e));}
}
...
function settings(){
  refresh();
  let html='<p>滑らせて歩く。素早く弾けば走る。接敵後も敵からじりじり距離を取れば、戦いの輪はほどける。</p><button class="inline-action" id="sound-toggle">環境音・効果音：'+(audio.enabled?'入':'切')+'<small>風、足音、捕食、戦闘音</small></button><button class="inline-action" id="visit-log">喰痕<small>'+Object.keys(profile.visits).length+'か所</small></button><button class="inline-action" id="credits">アセットと接続状況<small>共通戦闘コード・モデル・実装範囲</small></button>';
  ...
  $('music-library').hidden=!window.__SOUL_MUSIC__;$('music-library').onclick=()=>window.__SOUL_MUSIC__?.open();$('sound-toggle').onclick=()=>{audio.start();audio.toggle();settings();};$('visit-log').onclick=visits;$('credits').onclick=credits;
...
function frame(now){
  ...
  try{
    ...
    if(running){acc=Math.min(.10,acc+dt);let steps=0;while(acc>=1/60&&steps++<6){const v=swipe.vector(.33);v.dash=swipe.dash;v.active=swipe.id!==null;game.tick(1/60,v);acc-=1/60;if(game.fight&&swipe.dash)pauseInput();if(game.finished)break;}audio.feed(game.player.devourProgress);audio.tick(dt,game.player.speed>.15&&!game.fight);}else{audio.feed(null);acc=0;}
    view.feastTarget=running?feastHud?.target(game,returnMode):null;
...
export async function boot(){
  ...
  $('begin').onclick=()=>safe(()=>void randomHunt());$('title-memory').onclick=()=>safe(lineage);$('title-settings').onclick=()=>safe(settings);$('connection').onclick=credits;
