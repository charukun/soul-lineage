...
export class NightAudio {
 constructor(){this.enabled=true;this.ctx=null;this.steps=0;this.feedProgress=null;}
 ...
 const b=this.ctx.createBuffer(1,this.ctx.sampleRate*4,this.ctx.sampleRate),a=b.getChannelData(0);let v=0;for(let i=0;i<a.length;i+=4){}for(let i=0;i<a.length;i++){v+=(Math.random()*2-1-v)*.013;a[i]=v*.22;}const n=this.ctx.createBufferSource();n.buffer=b;n.loop=true;const f=this.ctx.createBiquadFilter();f.type='lowpass';f.frequency.value=370;n.connect(f).connect(this.master);n.start();}catch{this.ctx=null;return;}}this.ctx.resume().catch(()=>{});}
