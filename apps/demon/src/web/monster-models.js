...
export const EXTERNAL_MONSTER_MODELS=Object.freeze({
 'goblin-runt':Object.freeze({path:'minion/minion-a01.glb',gitBlobSha:'f7700d62b6252f14601743f156049ef3b8454f96',bytes:170752,targetHeight:2.0,fps:24,clips:MINION_CLIPS}),
 'horn-brute':Object.freeze({path:'minion/minion-b01.glb',gitBlobSha:'b8419a471e5f2619c17b2a8905dec1a04101430b',bytes:174432,targetHeight:2.05,fps:24,clips:MINION_CLIPS}),
 'maw-stalker':Object.freeze({path:'minion/minion-c01.glb',gitBlobSha:'584b85913b3a075948e6630b6a0d48883227c6d4',bytes:151100,targetHeight:1.95,fps:24,clips:MINION_CLIPS}),
 'grave-ogre':Object.freeze({path:'minion/minion-d01.glb',gitBlobSha:'ceb7beba04fa67b9607af9c8e704d1d084dd13ee',bytes:189716,targetHeight:2.15,fps:24,clips:MINION_CLIPS}),
 'night-bat':Object.freeze({path:'animal/Bat.glb',gitBlobSha:'766de4372d17206835c491f80f6b0db367ca0012',bytes:63308,targetHeight:1.7,fps:24,clips:ANIMAL_CLIPS,hover:.7})
});
...
export async function loadExternalMonsterModel(species,renderer=null,fetchImpl=fetch){
 const spec=externalMonsterSpec(species);if(!spec)return null;
 const url=externalMonsterUrl(species),response=await fetchImpl(url,{signal:AbortSignal.timeout(15000)});
 if(!response.ok)throw new Error(`Monster model HTTP ${response.status}`);
 const declared=Number(response.headers.get('content-length'));if(Number.isFinite(declared)&&declared>512*1024)throw new Error('Monster model asset too large');
 const bytes=await response.arrayBuffer();if(bytes.byteLength!==spec.bytes)throw new Error(`Monster model size mismatch: ${species}`);
 ...
 try{
  ...
  const root=new T.Group();root.userData.externalMonster=true;root.userData.species=species;root.add(visual);
