...
function card(v,profile,imported=false,recommended=false){
 ...
 const label=`${d.label}、危険度${d.dangerLabel}、住人${d.residents}人、${prey.power}${profile.unlocked.includes(v.target)?'刻印済み':'未刻'}`;
 return `<button class="route-card raid-route${recommended?' recommended':''}${visited?' scarred':''}" ${identity} ${imported&&visited?'disabled':''} aria-label="${escape(label)}">
 <span class="tag">${visited?'喰痕 · 閉':imported?'読み込んだ村 · オフライン':weather}${recommended?'<b class="raid-recommended">推奨</b>':''}</span>
 ...
 <span class="route-bottom"><span>${escape(prey.name)}の記憶</span><span class="reward">${escape(prey.power)} · ${profile.unlocked.includes(v.target)?'刻印済み':'未刻'}</span></span>
 ${imported?`<span class="raid-note">${visited?'入村済み · 喰痕 · 閉':'読み込んだ配置を使用。住人と危険度は単独狩り用の生成値。'}</span>`:''}
 </button>`;
}
export function renderRaidRoutes(offers,profile){
 const recommended=recommendRouteId(offers,profile);
 return '<p class="raid-intro">村の規模と危険度で、今夜の襲撃先を選ぶ。<br>喰痕の残る場所は、もう闇を開かない。</p>'+
