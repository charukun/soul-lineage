...
NightView.prototype.update=function updateWithHumanCrowd(game,dt,title=false){
 ...
 for(const root of this.actors?.children||[]){const id=root?.userData?.npcId;if(id==null)continue;const npc=npcs.find(row=>String(row.id)===String(id));if(!npc||npc.dead||npc.eaten||!Number.isFinite(npc.x)||!Number.isFinite(npc.z))continue;
  ...
  const bias=personalSpaceNavigationBias({position:{x:npc.x,z:npc.z},preferredVelocity:{x:Number(npc.vx)||0,z:Number(npc.vz)||0},neighbors,radius:.84,maxSuggestion:.18});
  root.position.x=npc.x+bias.suggestion.x;root.position.z=npc.z+bias.suggestion.z;records.push({id:String(id),suggestion:bias.suggestion,accepted:false});
 }
 if(typeof window!=='undefined')window.__DEMON_MOTION_CROWD__={snapshot:()=>({version:1,maxSuggestion:.18,worldAuthority:false,records})};
 return result;
};
