...
登録済みmotion packはすべてDEV Asset Originへmaterializeしてactiveに保つ。Connectorで大きいbinaryを安全に搬送できない場合は、既存Astra方式の一時GitHub Actions Hosted Runner materializerで pinned upstream を取得し、Git blob SHAを照合してbranchへcommitする。一時workflowはmaterialization commitから削除し、第三者GitHub/Codeberg/CDNをruntime fallbackとして使わない。
