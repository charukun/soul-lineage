...
- 序破急HUDは主人公視点のExchangeに追従する。主人公がinitiativeを持つpressure中だけ序/破/急を点灯し、守勢・読み合い・reversal・counterでは左端の間合い波形、主人公自身の急完遂だけ右端の残心を点灯する。相手の完了は左波形のままとする。cursor phaseだけで攻勢表示を残さない。
...
## /battle2 Exchange regression fixture

The review fixture consumes the same shared Exchange policy as native Tidebreak/main-game integration. It retains the existing domain contact executor and its `.46` trigger; motion `contactProgress` is presentation synchronization, not a new canonical trigger. No damage/contact authority is added to Exchange.

Counter transitions retain their original action/reaction context. A pending normal-start marker is consumed only after old callbacks/recovery complete. The new normal sequence starts at 序; a stale defensive cursor cannot light 破/急. Ordinary defense and shallow incoming damage do not interrupt the running sequence.

Checkpoint restore is an explicit `resumeAtSeconds` test fixture, not an automatic mid-combo interruption in ordinary playback. `actorOverrides` supplies deterministic physiology fixtures through the shared domain validator. `/battle2` version 2.2.0 covers hero/opponent completion, stale-cursor reversal, capability exhaustion, and secondary 1v3 pressure.
