...
- 色・余白・タイポグラフィ・パネル表現は既存Visual Review Labに合わせる。

## Model behavior

初期版はLabで監査済みのSHINO実VRMを使用し、既存 `public/simulator/src/life-clock.js` の `appearanceForAge()` を唯一の年齢カーブとして使う。モデルの全体scale、頭部scale、加齢前傾、髪の白髪化、顔skinの加齢補正をレビュー専用に適用する。
