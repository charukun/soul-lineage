...
test('one player can be engaged by several Tidebreak threats at once',()=>{
  const state=combatState(33),r=1.25,front={stage:0,enemies:[enemy('a',0,r,0,300),enemy('b',Math.sin(2.1)*r,Math.cos(2.1)*r,0,300),enemy('c',Math.sin(-2.1)*r,Math.cos(-2.1)*r,0,300)],cleared:false,clearSeconds:0};state.hp=state.maxHp=500;tickFront(state,front,1/60);assert.equal(state.combat?.engine,'tidebreak');assert.ok(state.combat?.threatIds?.length>=3,'primary plus nearby threats should all stay engaged');const sources=new Set();for(let i=0;i<480&&!state.down&&!state.ended;i++)for(const event of tickFront(state,front,1/60))if(event.type==='enemy-hit')sources.add(event.sourceId);assert.ok(sources.size>=2,`multiple enemies should be able to attack the same player: sources=${[...sources].join(',')} defeats=${state.defeats} hp=${state.hp} down=${Boolean(state.down)} ended=${state.ended} enemies=${front.enemies.map(e=>`${e.id}:${e.hp}:${e.downed?'down':'up'}:${e.dead?'dead':'live'}`).join('|')}`);
});
