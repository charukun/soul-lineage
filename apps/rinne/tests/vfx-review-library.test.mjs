...
import {EFFECT_MATERIALS_SOURCE,RESOURCE_DATA_CURRENT_SOURCE,GODOT4_DEMO_SOURCE} from '../src/rebuild/review-vfx-additional-sources.js';
...
test('production VFX stays bounded while review gets distinct source originals',()=>{
  ...
  assert.equal(REVIEW_VFX_LIBRARY_COUNT,307);
...
test('real review library is a unique pinned source closure',()=>{
  ...
  for(const row of REVIEW_VFX_LIBRARY_ASSETS){
    ...
    assert.ok(row.license==='CC0-1.0'||row.license==='MIT');
  }
  ...
  assert.equal(realByRepo.get(GODOT4_DEMO_SOURCE.repository),24);
...
test('download plan preserves provenance and namespaces the real review library',()=>{
  ...
  assert.equal(EFFECT_DOWNLOADS.some(row=>row.target==='LICENSE-GODOT4-MIT.txt'),true);
  ...
  assert.equal(EFFECT_DOWNLOADS.some(row=>row.sourcePath==='Demo/effects/samples/Homing_Laser01.efkefc'),true);
  assert.equal(EFFECT_DOWNLOADS.some(row=>row.sourcePath==='Demo/effects/samples/Laser03.efkefc'),true);
  assert.equal(EFFECT_DOWNLOADS.some(row=>row.sourcePath==='Demo/effects/tests/model_instanced.efkefc'),true);
  assert.equal(EFFECT_DOWNLOADS.some(row=>row.sourcePath==='Demo/effects/tests/texture_clamped.efkefc'),true);
});

test('catalog keeps legacy compositions separate from 307 real source originals',()=>{
