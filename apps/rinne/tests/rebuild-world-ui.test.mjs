...
test('actor status is reusable and birth teaches through the mother instead of a destination waypoint',()=>{
  ...
  assert.match(birthExperience,/createActorStatus/);assert.match(birthExperience,/status\.show\('抱っこされている…'\)/);assert.match(birthExperience,/muraSharedLine\('first-outing'\)/);assert.match(birthExperience,/createSpeechBubbles/);assert.match(birthExperience,/personId:['"]Mother['"]/);assert.match(birthExperience,/say\(`\$\{state\.name\}、\$\{line\}`\)/);
