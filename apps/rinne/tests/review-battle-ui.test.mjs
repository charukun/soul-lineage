...
test('battle review keeps loop and follow implicit and exposes only the player model selector',async()=>{
  ...
  assert.match(js,/details:false/);
  assert.doesNotMatch(js,/opponentHp:|opponentMaxHp:/);
...
test('review and Jinku gameplay mount the same shared phase panel',async()=>{
  const [review,demonHtml,pkg,panelCss]=await Promise.all([
  ...
  assert.match(demonHtml,/@soul\/shared-ui\/phase-panel/);
  ...
  assert.match(panelCss,/#battle\.soul-phase-panel/);
});
