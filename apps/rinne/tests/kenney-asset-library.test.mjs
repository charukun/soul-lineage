...
test('Kenney expansion is materialized, hashed, licensed and active in review runtime',async()=>{
  ...
  for(const row of spec.records){
    ...
    assert.equal(p.gitBlobSha,row.gitBlobSha);assert.equal(p.byteLength,row.byteLength);
    assert.match(p.sha256,/^[0-9a-f]{64}$/);
    const m=manifestByPath.get(p.runtimeAssetPath);assert.ok(m,p.runtimeAssetPath);
    assert.equal(m.gitBlobSha,row.gitBlobSha);assert.equal(m.bytes,row.byteLength);assert.equal(m.sha256,p.sha256);
