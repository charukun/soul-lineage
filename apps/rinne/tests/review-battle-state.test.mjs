...
test('review camera uses the same shared Rinne and Demon combat framing contracts',()=>{
  ...
  assert.equal(impactHold.stage,'impact');assert.equal(impactHold.hitStop,true);assert.equal(impactHold.executeProgress,.54);assert.equal(impactLate.executeProgress,.54);
  assert.equal(reviewInspirationSequenceFrame(REVIEW_INSPIRATION_TIMELINE.reveal+.01).stage,'reveal');
  assert.ok(REVIEW_INSPIRATION_TIMELINE.end<1);
  ...
  assert.match(battleSource,/INSPIRATION_BULB_HOLD_MS=220/);assert.match(battleSource,/if\(cue==='stagger'\)\{showInspirationBulb\(\);battleSfx\.inspiration\('spark'\);return;\}/);assert.match(battleSource,/if\(cue==='impact'\)\{battleSfx\.impact/);
  ...
  assert.match(stageSource,/function inspirationCameraFrame\(sequence\)/);assert.match(stageSource,/heroWeaponPoint\(inspirationHandPoint\)/);assert.match(stageSource,/lock:stage==='camera'\?'weapon-focus'/);assert.match(stageSource,/fovTarget=sequence&&sequence\.stage!=='done'\?sequence\.cameraFov:40/);assert.match(stageSource,/dataset\.inspirationBeat=sequence\.stage/);assert.match(stageSource,/cue==='impact'\)presentImpact/);
