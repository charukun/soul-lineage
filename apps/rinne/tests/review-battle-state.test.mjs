...
test('review camera uses the same shared Rinne and Demon combat framing contracts',()=>{
  ...
  assert.match(stageSource,/function inspirationCameraFrame\(sequence\)/);assert.match(stageSource,/heroWeaponPoint\(inspirationHandPoint\)/);assert.match(stageSource,/nearMissVector\(target,sequence/);assert.match(stageSource,/weaponSegment/);assert.match(stageSource,/sequence\?\.targetStagger>0/);assert.match(stageSource,/sequence\.impactRecoil/);assert.match(stageSource,/blendCameraFrames\(cinematicFrame,baseFrame,sequence\.progress\)/);assert.match(stageSource,/inspirationAnchor='impact-space'/);assert.match(stageSource,/createInspirationMotionLab/);assert.match(stageSource,/createInspirationVfxLab/);assert.match(stageSource,/inspirationMotion\.applyHero/);assert.match(stageSource,/inspirationMotion\.applyEnemy/);assert.match(stageSource,/inspirationVfx\.trail/);assert.match(stageSource,/inspirationVfx\.hit/);assert.doesNotMatch(stageSource,/fxRings|inspirationFx|auraRing|fxArc|fxSpark/);assert.match(stageSource,/lock:stage==='camera'\?'weapon-focus'/);assert.match(stageSource,/fovTarget=sequence&&sequence\.stage!=='done'\?sequence\.cameraFov:40/);assert.match(stageSource,/dataset\.inspirationBeat=sequence\.stage/);assert.match(stageSource,/\['camera','spacing','stagger','silence','execute','impact','reveal','afterglow'\]/);assert.match(stageSource,/if\(cue==='impact'\)\{inspirationVfx\.hit/);
