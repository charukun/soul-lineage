...
test('review camera uses the same shared Rinne and Demon combat framing contracts',()=>{
  ...
  assert.equal(reviewInspirationSequenceFrame(REVIEW_INSPIRATION_TIMELINE.reveal+.01).cameraFov,45);
  ...
  assert.match(battleSource,/INSPIRATION_BULB_HOLD_MS=420/);assert.match(battleSource,/if\(cue==='silence'\)\{battleSfx\.inspiration\('spark'\);return;\}/);assert.match(battleSource,/setInspirationState\?\.\(\{active:true,targetId,duration,nearMissSeconds:REVIEW_INSPIRATION_TIMELINE\.camera\}\)/);assert.match(battleSource,/if\(cue==='impact'\)\{battleSfx\.impact/);
