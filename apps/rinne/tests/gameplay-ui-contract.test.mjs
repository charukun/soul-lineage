...
test('main UI exposes heart technique body equipment plus a top-right radar without direct equipment mutation',async()=>{
  const ui=await readFile(new URL('../src/gameplay-ui.js',import.meta.url),'utf8'),upgrade=await readFile(new URL('../src/gameplay-upgrade.js',import.meta.url),'utf8'),css=await readFile(new URL('../src/gameplay-ui-contract.css',import.meta.url),'utf8'),navy=await readFile(new URL('../src/dark-navy-hud.css',import.meta.url),'utf8'),index=await readFile(new URL('../index.html',import.meta.url),'utf8');
  assert.match(ui,/data-heart/);assert.match(ui,/data-techniques/);assert.match(ui,/data-body/);assert.match(ui,/data-items/);assert.match(ui,/class=\"rinne-map-radar\"/);assert.match(ui,/data-record/);assert.match(ui,/function toggle\(type,options=\{\}\)/);assert.match(ui,/ui\.heart\.onclick=\(\)=>toggle\('heart'/);assert.doesNotMatch(ui,/data-debug/);assert.doesNotMatch(ui,/state\.equipment\.(weapon|armor|shield)\s*=/);assert.doesNotMatch(upgrade,/data-debug/);assert.match(css,/\.upgrade-panel\{overflow:hidden!important/);assert.match(navy,/\.game-screen\[data-gameplay-upgrade\] \.combat-phase-indicator\[hidden\]\{display:none!important\}/);assert.match(navy,/bottom:max\(82px,calc\(env\(safe-area-inset-bottom\) \+ 78px\)\)!important/);assert.match(navy,/\.upgrade-panel>header>\[data-close\]\{display:none!important\}/);assert.match(index,/reincarnation-interface-v2\.css/);
});
