...
test('main UI exposes heart technique body equipment plus a top-right radar without direct equipment mutation',async()=>{
  ...
  assert.match(ui,/data-heart/);assert.match(ui,/data-techniques/);assert.match(loadoutUi,/SIGIL_PATHS/);assert.match(loadoutUi,/skill-sigil/);assert.doesNotMatch(loadoutUi,/setHeartSlot|HEART_SLOT_COUNT|heartSlot/);assert.match(loadoutUi,/heart-learned-list/);assert.match(detail,/HOLD_MS=430/);assert.match(detail,/pointerdown/);assert.match(detail,/detailTitle/);assert.match(detailCss,/selection-detail-view/);assert.match(detailCss,/装着枠/);assert.match(ui,/installSelectionDetail/);assert.match(ui,/decorateSelectionDetail/);assert.match(ui,/data-body/);assert.match(ui,/data-items/);assert.match(ui,/class=\"rinne-map-radar\"/);assert.match(ui,/data-record/);assert.match(ui,/data-vitals/);assert.match(ui,/data-mind/);assert.match(ui,/setContextAnchor/);assert.match(ui,/function toggle\(type,options=\{\}\)/);assert.match(ui,/ui\.heart\.onclick=\(\)=>toggle\('heart'/);assert.doesNotMatch(ui,/data-debug/);assert.doesNotMatch(ui,/state\.equipment\.(weapon|armor|shield)\s*=/);assert.doesNotMatch(upgrade,/data-debug/);assert.match(css,/\.upgrade-panel\{overflow:hidden!important/);assert.match(navy,/\.game-screen\[data-gameplay-upgrade\] \.combat-phase-indicator\[hidden\]\{display:none!important\}/);assert.match(navy,/bottom:max\(82px,calc\(env\(safe-area-inset-bottom\) \+ 78px\)\)!important/);assert.match(navy,/grid-template-columns:repeat\(5,minmax\(0,1fr\)\)!important/);assert.match(navy,/\.skill-sigil\{/);assert.match(navy,/border-radius:0!important/);assert.match(navy,/\.rinne-gameplay-upgrade \.upgrade-panel>header>\[data-close\][\s\S]*display:grid!important/);assert.match(navy,/\.rinne-context-vitals\{/);assert.match(navy,/\.rinne-mind-balance\{/);assert.match(index,/reincarnation-interface-v2\.css/);
});
