...
import {synchronizeKnownSkills} from '../src/rebuild/inspiration-state.js';
...
test('survival build lowers fatality chance while power build raises outgoing damage',()=>{
  ...
  defender.equipment.armor='heavy';defender.equipment.shield=true;power.inspiration.legacySkills.push('skill.focus','skill.edge');synchronizeKnownSkills(power);ensureCombatLoadout(power);assert.equal(setHeartSlot(power,0,'skill.focus'),true);assert.equal(setHeartSlot(power,1,'skill.edge'),true);
