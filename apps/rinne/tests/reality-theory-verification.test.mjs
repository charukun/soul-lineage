...
test('new theory suites compose without weakening Semantic Frontier impossibility boundaries',()=>{
  const packet=runCanonPacketProofSuite(),relay=runAdaptiveRelayProofSuite();assert.equal(packet.pass,true);assert.equal(packet.packetSweep.cases,5218);assert.equal(packet.sequential.pass,true);assert.equal(relay.pass,true);const combined=runRrpTheoryVerificationSuite();assert.equal(combined.pass,true);assert.equal(combined.boundaries.twoPeerNoWitnessBoundary,true);assert.equal(combined.boundaries.earlyVisibilityCounterexampleRetained,true);assert.equal(combined.boundaries.relayNotUniversal,true);assert.equal(combined.boundaries.semanticUniversalStrictDominanceStillRejected,true);
});
