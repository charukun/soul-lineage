...
test('runtime profiler separates presentation layers and cannot disable gameplay',async()=>{const text=await source();assert.match(text,/target:'pixel-fold-class-30'/);assert.match(text,/measuredHardware:false/);assert.match(text,/profileLayer\('base-motion'/);assert.match(text,/profileLayer\('selection'/);assert.match(text,/profileLayer\('weapon-contact'/);assert.match(text,/mayDisableGameplay:false/);});
