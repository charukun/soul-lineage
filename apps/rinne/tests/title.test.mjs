...
test('sound is on by default, reduced motion follows system',()=>{assert.deepEqual(normalizedSettings(null,true),{sound:true,reducedMotion:true});assert.deepEqual(normalizedSettings({reducedMotion:false},false),{sound:true,reducedMotion:false});});
test('explicit sound and motion preferences win',()=>{assert.deepEqual(normalizedSettings({sound:false,reducedMotion:false},true),{sound:false,reducedMotion:false});assert.deepEqual(normalizedSettings({sound:true,reducedMotion:false},true),{sound:true,reducedMotion:false});});
test('malformed explicit sound preference is not treated as enabled',()=>assert.deepEqual(normalizedSettings({sound:'true',reducedMotion:'false'},false),{sound:false,reducedMotion:false}));
