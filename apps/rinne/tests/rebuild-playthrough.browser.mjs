...
export async function verifyRebuildPlaythrough(browser,url,output){
  ...
  try{
    ...
    const downed=stateAt(30,47);downed.zone='frontier';downed.front=1;downed.lastDepartureCycle=6;downed.position={x:0,z:0};downed.down={elapsed:39.2,rescueSeconds:40};downed.hp=0;downed.frontState=createFront(1,downed.seed);await load(downed);await expectTone('frontier');
