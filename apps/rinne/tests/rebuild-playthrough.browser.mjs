...
export async function verifyRebuildPlaythrough(browser,url,output){
  ...
  try{
    ...
    assert.equal(await text(page.locator('#objective-badge')),'武具 7歳');await expectTone('village');await compactHud();await page.screenshot({path:join(output,'01-village-growth.png')});

    const prep=stateAt(8,42);prep.position={x:sword.x+1.35,z:sword.z};await load(prep);
    assert.equal(await text(page.locator('#life-stage')),'3/6 支度');assert.equal(await text(page.locator('#objective')),'武具を選ぶ');assert.equal(await text(page.locator('#waypoint-label')),'片手剣');assert.equal(await page.locator('#talk').isHidden(),true);await expectTone('village');
