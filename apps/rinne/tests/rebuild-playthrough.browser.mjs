...
export async function verifyRebuildPlaythrough(browser,url,output){
  ...
  async function compactHud(){
    ...
    assert.equal(await page.locator('[data-context-action]').count(),1,'only optional talk may be a contextual action');
  }
  ...
  try{
    ...
    assert.equal(await page.locator('#talk').isVisible(),true);await page.locator('#talk').click();assert.match(await text(page.locator('#dialogue-text')),/歩幅/);
    assert.equal(await text(page.locator('#move-hint')),'スワイプで移動');
    ...
    assert.equal(await text(page.locator('#objective-badge')),'武具 7歳');await compactHud();await page.screenshot({path:join(output,'01-village-growth.png')});

    const prep=stateAt(8,42);prep.position={x:sword.x+1.35,z:sword.z};await load(prep);
    assert.equal(await text(page.locator('#life-stage')),'3/6 支度');assert.equal(await text(page.locator('#objective')),'武具を選ぶ');assert.equal(await text(page.locator('#waypoint-label')),'片手剣');assert.equal(await page.locator('#talk').isHidden(),true);
    ...
    assert.match(await text(page.locator('#toast')),/片手剣 装備/);assert.equal(await page.locator('#talk').isHidden(),true);await page.screenshot({path:join(output,'02-equipment.png')});
    ...
    assert.equal(await text(page.locator('#life-stage')),'4/6 出立');assert.equal(await text(page.locator('#objective')),'港へ');assert.equal(await text(page.locator('#objective-badge')),'出航');assert.equal(await text(page.locator('#waypoint-label')),'港');assert.equal(await page.locator('#talk').isHidden(),true);
    await page.waitForFunction(()=>document.getElementById('life-stage')?.textContent.includes('第1前線'),null,{timeout:5000});
    assert.match(await text(page.locator('#objective')),/敵へ|戦闘/);assert.equal(await page.locator('#talk').isHidden(),true);await compactHud();await page.screenshot({path:join(output,'03-frontier-arrival.png')});
    ...
    assert.equal(await text(page.locator('#waypoint-label')),'次の前線');assert.equal(await text(page.locator('#objective')),'奥へ');
    ...
    assert.equal(await text(page.locator('#waypoint-label')),'帰還地点');assert.equal(await text(page.locator('#objective')),'帰還へ');await page.keyboard.down('ArrowDown');await sleep(650);await page.keyboard.up('ArrowDown');
    await page.waitForFunction(()=>document.getElementById('life-stage')?.textContent.includes('凱旋'),null,{timeout:4000});await compactHud();await page.screenshot({path:join(output,'04-homecoming.png')});
    ...
    assert.equal(await text(page.locator('#objective')),'救助待ち');assert.match(await text(page.locator('#objective-badge')),/秒/);assert.equal(await page.locator('#talk').isHidden(),true);await page.waitForFunction(()=>!document.getElementById('life-stage')?.textContent.includes('救助'),null,{timeout:4000});
    ...
    await page.locator('.life-end-dialog[open]').waitFor({state:'visible',timeout:5000});assert.match(await text(page.locator('.life-end-summary')),/12撃破/);assert.equal(await text(page.locator('.life-end-help')),'技・装備 継承');await page.screenshot({path:join(output,'05-life-end.png')});
    await page.locator('#rebirth').click();await page.waitForFunction(()=>document.getElementById('generation')?.textContent==='2代目',null,{timeout:5000});assert.equal(await text(page.locator('#life-stage')),'1/6 誕生');assert.equal(await text(page.locator('#toast')),'2代目');await page.screenshot({path:join(output,'06-rebirth.png')});
