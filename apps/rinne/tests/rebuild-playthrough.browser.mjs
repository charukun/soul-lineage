...
export async function verifyRebuildPlaythrough(browser,url,output){
  ...
  try{
    ...
    for(let i=0;i<3;i++)await page.locator('#clock-rate').click();assert.equal(await page.locator('#clock-rate').evaluate(node=>node.value),'20');await page.waitForFunction(()=>document.getElementById('life-stage')?.textContent.includes('2/6 村'),null,{timeout:18000});
    ...
    assert.equal(await text(page.locator('#life-stage')),'3/6 支度');assert.equal(await text(page.locator('#objective')),'武具を選ぶ');assert.equal(await text(page.locator('#waypoint-label')),'片手剣');assert.equal(await page.locator('#talk').count(),0);await expectTone('village');
    ...
    assert.equal(await text(page.locator('#life-stage')),'4/6 出立');assert.equal(await text(page.locator('#objective')),'港へ');assert.equal(await text(page.locator('#objective-badge')),'出航');assert.equal(await text(page.locator('#waypoint-label')),'港');assert.equal(await page.locator('#talk').count(),0);
