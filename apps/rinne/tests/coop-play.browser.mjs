...
export async function verifyCoopPlay(browser,url,output){
  ...
  try{
    ...
    await host.waitForFunction(()=>window.__RRP_CAPTURE__?.armed?.()===true);await guest.waitForFunction(()=>window.__RRP_CAPTURE__?.armed?.()===true);
    ...
    const hostCapture=await captureRaw(host),guestCapture=await captureRaw(guest);assert.ok(hostCapture&&guestCapture,'capture mode must expose both endpoint samples');assert.equal(hostCapture._capture.role,'host');assert.equal(guestCapture._capture.role,'guest');assert.equal(hostCapture._capture.worldId,guestCapture._capture.worldId);assert.equal(hostCapture._capture.windowArmed,true);assert.equal(guestCapture._capture.windowArmed,true);assert.equal(hostCapture._capture.expectedPeers,2);assert.equal(guestCapture._capture.expectedPeers,2);assert.equal(hostCapture.bandwidthSkippedBuckets,0);assert.equal(guestCapture.bandwidthSkippedBuckets,0);
