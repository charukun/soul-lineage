...
test('childhood and preparation point at automatic actions without action buttons',()=>{
  ...
  assert.equal(childGuide.stage,'2/6 遊び');assert.equal(childGuide.objective,'広場で遊ぶ');assert.equal(childGuide.target.label,'広場');assert.equal(childGuide.badge,'身体を知る');
