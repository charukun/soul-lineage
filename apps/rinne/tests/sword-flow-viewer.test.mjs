...
import {FLOW_SECONDS,FLOW_REVISION,FLOW_TIMING,FLOW_WINDOWS,flowWindow,sampleSwordFlow} from '../public/simulator/src/sword-flow.js';
...
test('flow viewer plays once, scrubs, changes modes and synchronizes its reference',async()=>{
 ...
 try{
  ...
  ids.mode.onchange({target:{value:'flow'}});ids.timeline.oninput({target:{value:'3.95'}});ids.play.onclick();tick(100);tick(100);
