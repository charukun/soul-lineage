...
const transformMatrix=(target,around)=>{if(!target)return;const transformed=around.clone().multiply(new T.Matrix4().fromArray(target));if(target.set)target.set(transformed.elements);else{for(let i=0;i<16;i++)target[i]=transformed.elements[i];}};
...
function rotateWorldPoint(point,origin,angle){const p=point3(point),dx=p.x-origin.x,dz=p.z-origin.z,cs=Math.cos(angle),sn=Math.sin(angle);return v(origin.x+dx*cs+dz*sn,p.y,origin.z-dx*sn+dz*cs);}
...
export class HumanoidRuntime extends BaseHumanoidRuntime{
 ...
 applyCombo(c,a,result){if(!a?.attack||!a._motionCombo||!result)return;const phase=this.api.progress(a),carry=localComboCarry({previous:a._motionCombo.previous,gap:a._motionCombo.gap,phase});if(Math.abs(carry)<1e-5)return;const angle=carry*.22,origin={x:c.root.position.x,y:c.root.position.y,z:c.root.position.z},around=new T.Matrix4().makeTranslation(origin.x,0,origin.z).multiply(new T.Matrix4().makeRotationY(angle)).multiply(new T.Matrix4().makeTranslation(-origin.x,0,-origin.z));c.root.rotation.y+=angle;c.root.updateMatrixWorld(true);for(const key of ['a','b','weaponBase','weaponTip'])if(result[key])writePoint(result[key],rotateWorldPoint(result[key],origin,angle));for(const key of ['sm','leftSocket','rightSocket','carry'])if(result[key])transformMatrix(result[key],around);c.lifeCombo={carry,angle};}

 recordPerception(c,a,result,lod){if(!lod.perceptualQA||!result?.b)return;c.root.updateMatrixWorld(true);const hips=this.point(c,'hips'),chest=c.bones.chest?this.point(c,'chest'):hips,body=hips.clone().lerp(chest,.55),tip=point3(result.b),base=point3(result.a),clock=a._humanoidClock||0;if(![tip.x,tip.y,tip.z,base.x,base.y,base.z].every(Number.isFinite))return;this._life.trajectory.push({t:clock,x:tip.x,y:tip.y,z:tip.z});while(this._life.trajectory.length>45)this._life.trajectory.shift();this._life.readability=localReadability({keyframes:[{id:a.attack?.kind||'pose',body:body.toArray(),weaponBase:base.toArray(),weaponTip:tip.toArray()}]});if(this._life.trajectory.length>=4){try{this._life.perceptual=localTrajectory(this._life.trajectory);}catch{}}c.lifeReadability=this._life.readability;c.lifePerceptual=this._life.perceptual;}
