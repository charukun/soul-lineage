...
export class HumanoidRuntime extends BaseHumanoidRuntime{
 ...
 sample(a,at=null,px=a.x,pz=a.z,commit=false){const result=super.sample(a,at,px,pz,commit),c=this.current;if(!result||!c)return result;const lod=localMotionLod({distance:Number.isFinite(a?._motionQualityDistance)?a._motionQualityDistance:0,isHero:!!a?.hero});this._life.lod=lod;this.applyCombo(c,a,result);this.applyGaze(c,a,commit,lod);this.applyGrip(c,a,commit,lod);this.applySecondary(c,a,commit,lod);if(commit)this.recordPerception(c,a,result,lod);if(commit){c.vrm.update(0);c.root.updateMatrixWorld(true);}return result;}
