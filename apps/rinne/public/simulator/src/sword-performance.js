...
export const PERFORMANCE_REVISION='shino-performance-1';
export const SWORD_TIMINGS=Object.freeze(Object.fromEntries(['slash','back','uppercut','thrust','heavy'].map(k=>[k,SLASH_TIMING])));
...
export function applyPerformance(actor,seconds,locomotion){
 ...
 Object.assign(actor,{x:s.x,z:s.z,yaw:s.yaw,vx:s.vx,vz:s.vz,attack:s.attack,_humanoidClock:s.time,
  _humanoidPhase:s.walk/(locomotion?.walk?.cycleDistance||1.16)+s.run/(locomotion?.run?.cycleDistance||1.8)});
 return s;
}
