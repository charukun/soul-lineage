...
import {SWORD_MOVES,SWORD_STEPS,sampleSwordPose,swordAirHeight} from './authored-sword.js';
const clamp=x=>Math.min(1,Math.max(0,x));
const smooth=x=>{x=clamp(x);return x*x*x*(10+x*(-15+6*x));};

export function createSwordSequence(kinds,{connected=true,charge=0}={}) {
 ...
 const entries=kinds.map((kind,index)=>{
  ...
  const enter=connected&&index>0?.18:0,exit=connected&&index<kinds.length-1?(['round','leap'].includes(kind)?.98:.84):1;
  const overlap=connected&&index>0?.18:0,start=end-overlap;
  const chargeTime=index===0?Math.max(0,charge):0,holdAt=.19;
  const length=(exit-enter)*move.seconds+chargeTime;
  const row={kind,index,start,end:start+length,enter,exit,overlap,seconds:move.seconds,chargeTime,holdAt,label:move.label};
  end=row.end;return Object.freeze(row);
 });
 return Object.freeze({entries:Object.freeze(entries),duration:end,connected});
}

function entryPhase(entry,time){
 const elapsed=time-entry.start,pre=(entry.holdAt-entry.enter)*entry.seconds;
 const motion=elapsed<=pre?elapsed:elapsed<pre+entry.chargeTime?pre:elapsed-entry.chargeTime;
 return {phase:Math.min(entry.exit,Math.max(entry.enter,entry.enter+motion/entry.seconds)),held:elapsed>=pre&&elapsed<pre+entry.chargeTime,before:elapsed<0};
}
...
export function swordSequenceFrame(sequence,seconds) {
 ...
 const sample=entry=>({kind:entry.kind,index:entry.index,phase:entryPhase(entry,time).phase,label:entry.label});
...
export function swordSequenceTravel(sequence,seconds,poseScale=1){
 ...
 for(const row of sequence.entries){
  const sample=entryPhase(row,time),phase=sample.before?0:sample.phase;
  const u=clamp((phase-.25)/.44),distance=smooth(u);
  const speed=!sample.held&&u>0&&u<1?30*u*u*(1-u)*(1-u)/(.44*row.seconds):0;
...
export function applySwordSequence(actor,sequence,seconds,{start=0}={}) {
 ...
 actor.air=swordSequenceAir(sequence,seconds-start);
 return frame;
}

export function createSwordTechnique(recipe){
 if(!recipe?.steps?.length||recipe.steps.length>3)throw Error('A sword technique needs one to three basic actions');
 const charge=({none:0,breath:.38,deep:.85,full:1.4})[recipe.charge||'none'];
 if(charge===undefined)throw Error('Unknown technique charge');
 return createSwordSequence(recipe.steps.map(step=>step.kind),{charge});
}
export function swordSequenceContactTime(entry,phase=.5){
 return entry.start+(phase-entry.enter)*entry.seconds+(phase>entry.holdAt?entry.chargeTime:0);
}

export function swordSequenceAir(sequence,time){
 const f=swordSequenceFrame(sequence,time),height=swordAirHeight(f.current.kind,f.current.phase);
 return f.previous?height*f.weight+swordAirHeight(f.previous.kind,f.previous.phase)*(1-f.weight):height;
}
