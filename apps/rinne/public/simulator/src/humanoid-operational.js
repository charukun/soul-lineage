...
export class HumanoidRuntime extends BaseHumanoidRuntime{
 constructor(api){super(api);this._operational={bank:null,foot:{left:[],right:[]},tracks:new Map(),semantic:[],motionState:new Map(),appliedPoseStart:null,current:null,lastClock:null};this.installImpactContactBridge();this.installSemanticBridge();}
 ...
 applyLocomotionPoseStart(a,commit){if(!commit||a?.attack||a?.recovery||a?.dead)return null;const key=String(a?.id??'hero'),speed=Math.hypot(Number(a?.vx)||0,Number(a?.vz)||0),state=speed>.08?'move':'idle',previous=this._operational.motionState.get(key);this._operational.motionState.set(key,state);if(state!=='move'||previous==='move')return null;const suggestion=this.poseStartSuggestion(a),expected=speed>2.4?'run-slow':'walk';if(!suggestion||suggestion.clipId!==expected)return null;a._humanoidPhase=suggestion.phase;this._operational.appliedPoseStart={...suggestion,actorId:key,appliedAt:Number(a?._humanoidClock)||0,presentationPhaseOnly:true};return this._operational.appliedPoseStart;}
 ...
 sample(a,at=null,px=a.x,pz=a.z,commit=false){const appliedPoseStart=this.applyLocomotionPoseStart(a,commit),result=super.sample(a,at,px,pz,commit);if(!result||!this.current)return result;if(commit){this.recordKinematics(a,result);const poseStart=this.poseStartSuggestion(a),sweptContact=this.sweptContact(a),diagnostics=this.diagnostics();this._operational.current={version:1,poseStart,appliedPoseStart,sweptContact,diagnostics,semanticEvents:this._operational.semantic.slice(-8),candidateCount:this.candidateBank().candidates.length,advisory:true,gameplayAuthority:false};this.current.operationalMotion=this._operational.current;}return result;}
