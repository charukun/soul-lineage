...
export async function retargetBank(vrm){
 ...
 for(const item of catalog){
  const anim=bank[item.id];if(!anim)continue;
  try{const clip=createVRMAnimationClip(anim,vrm);clip.name=item.id;
   ...
   clips[item.id]=clip;
  }catch(error){console.warn('VRMAリターゲットをスキップ:',item.id,error);}
 }
 return clips;
}
