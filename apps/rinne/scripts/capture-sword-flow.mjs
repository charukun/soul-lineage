...
const {FLOW_TIMING,flowWindow}=await import('../public/simulator/src/sword-flow.js');
const api={weapons:{sword:{tip:1.62,base:.21,width:.065}},clips:{flow:FLOW_TIMING,slash:{active:[.35,.64],contact:.5,launch:.34,plant:.49,chain:.86}},strikes:{slash:{}},windows:{},progress:(a,at)=>(at??a.attack?.t??0)/(a.attack?.duration||1),window:(k,p)=>k==='flow'?flowWindow(p):p>=.35&&p<=.64?0:-1};
const runtime=new HumanoidRuntime(api);await runtime.load('SHINO');return runtime;
}
import {writeFile,mkdir} from 'node:fs/promises';
import {resolve} from 'node:path';
const outDir=resolve(process.argv[2]||'/tmp/rinne-sword-flow');await mkdir(outDir,{recursive:true});


import {createReviewSword} from '../public/simulator/src/review-sword.js';
import {sampleFlowRoot} from '../public/simulator/src/sword-flow.js';
...
const actor={id:'review',hero:true,weapon:'sword',weaponDraw:1,lifeAgeYears:22,x:0,z:0,yaw:0,air:0,vx:0,vz:0,combatReady:true,_humanoidClock:0,attack:{id:'review',kind:'flow',t:0,duration:4}};
const frameCount=121,vertices=new Float32Array(frameCount*count*3),points=[],v=new T.Vector3();
for(let i=0;i<frameCount;i++){
 const p=i/(frameCount-1),t=p*4,root=sampleFlowRoot(t),scale=c.unit*c.legLength/.82;actor.attack.t=t;actor._humanoidClock=t;actor.x=root.x*scale;actor.z=root.z*scale;actor.yaw=root.yaw;
