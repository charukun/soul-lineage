...
export function mountTitle(buildInfo, options={}){
 ...
 let pointer=[0,0],drift=[0,0],gesture=false,launchAbort=null;
 ...
 function removeFrame(){
  ...
  requestToken='';assetRequests.clear();frame?.remove();frame=null;$('frame-slot').replaceChildren();
 }
 function showError(message,code='BOOT_ERROR',detail=''){
  ...
  launchAbort?.abort();frame?.remove();frame=null;requestToken='';assetRequests.clear();
 }
 async function loadFrame(){
  ...
  launchAbort=new AbortController();const launchSignal=launchAbort.signal;loading.start();
  ...
  iframe.id='simulator-frame';iframe.title='Tidebreak Expanded スキルシミュレーター';iframe.allow='autoplay; fullscreen';
  try {
   if(options.simulatorHTML){
    ...
    const config={token,environment,hostOrigin:location.origin,offline:true};
    ...
    const url=new URL('./simulator/index.html',base);url.searchParams.set('titleToken',token);url.searchParams.set('environment',environment);iframe.src=url.href;
   }
 ...
 function enter({fromHistory=false}={}){
  if(!isTitle())return;if($('settings-dialog').open)$('settings-dialog').close();gesture=true;audio.chime();updateState('enter');$('start-simulator').disabled=true;screen.dataset.phase='leaving';$('skip-intro').hidden=true;
  if(!fromHistory&&location.hash!=='#simulator'){try{history.pushState({rinneTitle:true},'','#simulator');}catch{}}
 ...
 function back({fromHistory=false}={}){
  ...
  if(!fromHistory&&location.hash==='#simulator'){try{if(history.state?.rinneTitle)history.back();else history.replaceState(null,'',location.href.split('#')[0]);}catch{}}
 ...
 listen($('start-simulator'),'click',()=>enter());listen($('back-title'),'click',()=>back());
 ...
 listen(window,'popstate',()=>{if(location.hash==='#simulator'){if(isTitle())enter({fromHistory:true});}else if(!isTitle())back({fromHistory:true});});
 ...
 syncSettings();if(!settings.reducedMotion)$('start-simulator').disabled=true;resume();if(location.hash==='#simulator'){reveal();enter({fromHistory:true});}
