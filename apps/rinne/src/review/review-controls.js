...
export function installFocusedReviewUI(){
 ...
 const drawerHead=make('header','','review-drawer-head');const title=make('div');title.append(make('strong','動作をセット'),make('small','選ぶ → 確認'));
 ...
 viewport.after(scrim,toggle,dock);dock.append(drawerHead,panel);
 ...
 for(const play of skillPage?.querySelectorAll('[data-play-select]')||[]){play.textContent='確認';play.setAttribute('title',play.getAttribute('aria-label')||'この段階まで確認');}
 ...
 const help=skillPage?.querySelector('.sequence-help');if(help)help.textContent='序・破・急の動作を選び、確認したい段の「確認」を押します。';
 const secondary=make('details','','review-secondary-disclosure');const secondarySummary=make('summary','その他の動作');secondary.append(secondarySummary);
 ...
 document.addEventListener('review-state-change',event=>{const{state,meta,problems=[]}=event.detail;
  const name=q('#motion-name'),desc=q('#motion-meta');if(name)name.textContent=state.clip||'元モデル';if(desc)desc.textContent=state.sequence?.length>1?`${state.sequence.length}段の連続再生`:(state.clip?'選択中':'静止比較');
