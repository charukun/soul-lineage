...
const MODELS=Object.freeze({
  ...
  A:{id:'model.A',label:'A',name:'AvatarSample A',path:'./simulator/assets/A_review.vrm',size:14268444,runtimeId:'A',cacheId:'A',portrait:'A',appearanceScale:[1,1,1]},
  B:{id:'model.B',label:'B',name:'AvatarSample B',path:'./simulator/assets/B_review.vrm',size:15354384,runtimeId:'B',cacheId:'B',portrait:'B',appearanceScale:[1,1,1]},
  C:{id:'model.C',label:'C',name:'AvatarSample C',path:'./simulator/assets/C_review.vrm',size:12104076,runtimeId:'C',cacheId:'C',portrait:'C',appearanceScale:[1,1,1]},
  TSUKU:{id:'model.TSUKU',label:'TSUKU',name:'Tsuku',path:'./simulator/assets/TSUKU_review.vrm',size:12688044,runtimeId:'TSUKU',cacheId:'TSUKU',portrait:'TSUKU',appearanceScale:[1,1,1]},
});
export const reviewPresets=Object.freeze(Object.values(MODELS).map(row=>({id:row.id,label:row.label,name:row.name,portrait:row.portrait,kind:'character'})));
...
// Shared combat-Humanoid definitions used by the MasterCharacter/Tidebreak inspection path.
const MASTER_TECHNIQUES=Object.freeze([
...
const MASTER_DEFENSE=Object.freeze([
...
function modelCacheKey(model){return 'model:'+(model.cacheId||model.label);}
...
async function startGlobalPreload({rootUrl,assetUrl,motionCatalog}){
  ...
  preloadPromise=(async()=>{
    let done=0,failed=0;const physicalModels=[...new Map(Object.values(MODELS).map(m=>[m.cacheId||m.label,m])).values()],motions=[...motionCatalog].sort((a,b)=>(a.bytes||Infinity)-(b.bytes||Infinity)),total=motions.length+physicalModels.length+reviewWeapons.length+1;
    const mark=label=>progressUI(`先読み: ${label}`,done,total,`${done}/${total} 完了${failed?` / ${failed}件失敗`:''}`);
    for(const item of motions){try{mark(item.label);await cachedBytes('motion:'+item.id,new URL(item.file,new URL('./simulator/',rootUrl)),{sha256:item.reviewSHA256,expectedSize:item.bytes});}catch(error){failed++;console.warn('preload motion failed',item.id,error);}done++;}
    for(const model of physicalModels){try{mark(model.name);await cachedBytes(modelCacheKey(model),new URL(model.path,rootUrl),{expectedSize:model.size,sha256:model.sha256});}catch(error){failed++;console.warn('preload model failed',model.label,error);}done++;}
    for(const weapon of reviewWeapons){if(weapon.local){done++;continue;}try{mark(weapon.label);await cachedBytes('weapon:'+weapon.id,new URL(KAYKIT_ROOT+weapon.file),{});}catch(error){failed++;console.warn('preload weapon failed',weapon.id,error);}done++;}
...
async function loadAllSources({scene,rootUrl,assetUrl,signal,onProgress,presetId,motionCatalogModule,loaderModule,vrmModule,motionsModule,vrmaModule,humanoidModule}){
  ...
  const modelBytes=await cachedBytes(modelCacheKey(model),new URL(model.path,rootUrl),{expectedSize:model.size,sha256:model.sha256,onProgress:(n,total)=>progressUI(`${model.name}`,n,total,'表示モデルを読み込み中')});signal.throwIfAborted();
  const visibleLoader=new GLTFLoader(),visible=await visibleLoader.parseAsync(modelBytes,rootUrl.href),targetBones=await visibleHumanoidBones(visible);visible.scene.name=`${model.label}ReviewVisible`;visible.scene.scale.fromArray(model.appearanceScale||[1,1,1]);visible.scene.updateMatrixWorld(true);
  ...
  const authored=motionsModule.createRetargetedClips(vrm);sourceVRMForBones.set(authored.bones,vrm);const martial=createMartialClips(vrm);authored.clips.Attack=martial.Attack;visible.scene.quaternion.copy(vrm.scene.quaternion);visible.scene.updateMatrixWorld(true);const clips=new Map(),authoredNames=['Idle','Walk','Run','Attack','Hit Reaction','T-Pose'];for(const name of authoredNames){const value=`Tidebreak / ${name}`;clips.set(value,remapClip(authored.clips[name],authored.bones,targetBones,value));}
  clips.set('通常 / 自然体',remapClip(martial.NormalIdle,authored.bones,targetBones,'通常 / 自然体'));
  ...
  let disposed=false,effects=null,externalRetarget=null,library=null,failed=0;const weapons=new Map(),pendingWeapons=new Map(),assetProblems=[];
  ...
  const rootOrigin=visible.scene.position.clone();
  const body={root:visible.scene,swordPoseScale:null,swordDisplayScale:[1,1,1],applySwordTravel(plan,time,inPlace){applyReviewSwordTravel(visible.scene,rootOrigin,plan,time,{poseScale:this.swordPoseScale,displayScale:this.swordDisplayScale,inPlace});},bones:targetBones,clipNames:[...clips.keys()],label:`${model.name} / 全モーションソース`,weaponReview:true,clipGroups:[{label:'ゲーム実装 / Tidebreak自作',names:[...clips.keys()]}],summary:'基本動作を使用可能。追加モーションを読み込み中。',assetProblems,motionMeta:new Map(),poseTransfer:'official-normalized-to-raw',sourceVRM:vrm,getClip:name=>clips.get(name)||null,afterSample(){visible.scene.updateMatrixWorld(true);},async setWeapon({enabled=false,id=null,scale=.5,x=0,y=0,z=0}={}){if(!weaponSocket.parent||!enabled){weaponSocket.visible=false;return;}const requested=id||document.querySelector('#weapon-select')?.value||'greatsword';if(requested!==activeWeapon||!weapons.has(requested))activeWeapon=await selectWeapon(requested);weaponSocket.visible=true;const selected=weapons.get(activeWeapon);if(selected)selected.scale.setScalar(Math.max(.1,Math.min(1,scale)));weaponSocket.rotation.set(x*Math.PI/180,y*Math.PI/180,z*Math.PI/180);},sampleEffects(){},dispose(){disposed=true;effects?.dispose();externalRetarget?.dispose();for(const root of weapons.values())disposeLoaded(root);disposeLoaded(library?.scene);disposeLoaded(visible.scene);vrmModule.VRMUtils.deepDispose(vrm.scene);}};
  const register=(id,meta)=>body.motionMeta.set(id,{id,source:body.label,...meta});
  ...
  setTimeout(async()=>{
    for(const item of motionCatalog){if(disposed||signal.aborted)return;try{const bytes=await cachedBytes('motion:'+item.id,new URL(item.file,simBase),{sha256:item.reviewSHA256,expectedSize:item.bytes,onProgress:(n,total)=>progressUI(`共有: ${item.label}`,n,total,`${loaded} 使用可能`)});const l=new loaderModule.GLTFLoader();l.register(p=>new vrmaModule.VRMAnimationLoaderPlugin(p));const parsed=await l.parseAsync(bytes,simBase.href),anim=parsed.userData.vrmAnimations?.[0];if(!anim)throw new Error(`VRMAを解析できません: ${item.id}`);const vendorClip=vrmaModule.createVRMAnimationClip(anim,vrm);vendorClip.name=item.id;if(['walk','run-slow'].includes(item.id))inPlaceClip(vendorClip);const value=`共有VRMA / ${item.label}`,clip=remapClip(vendorClip,authored.bones,targetBones,value);clips.set(value,clip);if(disposed||signal.aborted)return;register(value,{category:['walk','run-slow'].includes(item.id)?'move':'life',loop:true});body.clipNames.push(value);appendClip('ゲーム共通 / VRMA',value,item.label);loaded++;}catch(error){failed++;failAsset(item.id,error);console.warn(`Shared VRMA unavailable: ${item.id}`,error);}updateSummary(loaded,null,failed);}
    ...
    if(disposed||signal.aborted)return;
    if(!visible.parser.json.extensions?.VRMC_vrm?.humanoid){failAsset('external:Quaternius','この比較用リターゲッターはVRM 1のみ対応。基本動作・共有VRMAは利用可能です。');updateSummary(loaded,null,assetProblems.length);return;}
    try{
      ...
      const motionBytes=await cachedBytes('external:quaternius',new URL(`AnimationLibrary.glb?v=${row.sha256}`,assetUrl),{expectedSize:row.size,sha256:row.sha256,onProgress:(n,total)=>progressUI('外部比較 / Quaternius',n,total,`${loaded} 使用可能`)});library=await visibleLoader.parseAsync(motionBytes,assetUrl.href);body.suspendSampling=true;try{body.resetPose();visible.scene.updateMatrixWorld(true);externalRetarget=await createQuaterniusRetargeter(library,visible);}finally{body.suspendSampling=false;}effects=await createReviewEffects(scene,assetUrl);
      for(const name of externalRetarget.clipNames){const value=`外部 / ${name}`;clips.set(value,externalRetarget.getClip(name,{inPlace:true}));register(value,{category:'external',loop:true});body.clipNames.push(value);appendClip('外部Asset比較 / Quaternius',value,name);loaded++;}
    ...
    }catch(error){failed++;failAsset('external:Quaternius',error);console.error(error);progressUI('外部比較モーション読込失敗',0,1,error.message);updateSummary(loaded,null,failed);}
  },0);
  return body;
}
...
export async function installReviewExtensions({scene}){
  ...
  const [loaderModule,vrmModule,motionsModule,vrmaModule,motionCatalogSource,humanoidModule]=await Promise.all([
  ...
  const motionCatalogModule={default:motionCatalogSource.default.map(row=>({...row,...REVIEW_MOTION_INTEGRITY[row.id]}))};
