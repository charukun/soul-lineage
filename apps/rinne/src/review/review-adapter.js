...
async function loadBytes(url, {signal,onProgress,expectedSize,sha256} = {}) {
  ...
  for (;;) {
    ...
    if (length > (expectedSize || 25 * 1024 * 1024)) { await reader.cancel(); throw new Error('Asset exceeds its allowed byte count'); }
    parts.push(value); onProgress?.(length,expectedSize || Number(response.headers.get('content-length')) || 0);
  }
  ...
  if (sha256) {
    ...
    if (digest !== sha256) throw new Error('Asset SHA-256 mismatch');
  }
  signal?.throwIfAborted(); return result.buffer;
}
  ...
  return {
    async loadPreset({signal,onProgress}) {
      ...
      try {
        ...
        if (manifest.revision !== REVIEW_ASSET_REVISION) throw new Error('Asset revision mismatch; reload this preview');
        const modelBytes = await loadBytes(new URL(SHINO_REVIEW.publicPath,rootUrl),{signal,expectedSize:SHINO_REVIEW.size,sha256:SHINO_REVIEW.sha256,onProgress:(n,total) => onProgress(`Shino ${Math.round(n/total*100)}%`)});
        character = await loader.parseAsync(modelBytes,rootUrl.href); signal.throwIfAborted();
        const row = manifest.files.find(file => file.id === 'animation.quaternius.library');
        if (!row?.sha256 || !row.size) throw new Error('Animation integrity record is missing');
        ...
        const own = {
          ...
          sampleEffects(age) {
            retarget.bones.rightHand.getWorldPosition(impact); impact.z += .25;
