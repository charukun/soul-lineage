...
export async function mountStory(win,environment,{signal}={}){
  ...
  const hud=doc.createElement('section');hud.id='story-hud';hud.setAttribute('aria-label','本編の進行');hud.innerHTML=`
    <div class="story-heading"><span>輪廻転焦</span><strong id="story-zone"></strong></div>
    <p id="story-summary"></p><div class="story-health"><i id="story-hp"></i></div>
    <p id="story-objective"></p><button type="button" id="story-records">暮らしの記録</button>
    <p id="story-storage" role="status"></p>`;
  const panel=doc.createElement('section');panel.id='story-actions';panel.setAttribute('aria-label','近くでできること');panel.innerHTML='<strong id="story-place"></strong><p id="story-message" role="status" aria-live="polite"></p><div id="story-buttons"></div>';
  doc.getElementById('stage').append(hud,panel);
  ...
  doc.body.append(dialog,death);const $=id=>doc.getElementById(id);let signature='',saveElapsed=0,uiElapsed=0,modalPaused=false,lastNotice='';
  function notice(text){lastNotice=text;$('story-message').textContent=text;}
  function save(){if(!active||disposed)return false;
  ...
  function endScreen(){if(death.open)return;port.pause(true);const select=$('story-memento');select.replaceChildren();
  ...
  function render(force=false){
    ...
    $('story-summary').textContent=`第${s.generation}生 · ${f.life.age}歳 · 世界暦${Math.floor(f.life.worldSeconds/60)}年`;
    ...
    $('story-objective').textContent=f.hero.dead?`救助を待っています。あと${Math.ceil(Math.max(0,(s.zone==='village'?12:40)-s.downedSeconds))}秒`:
      s.phase==='birth'?'母と村を歩き、贈り物を選ぼう。4歳で独り立ち。':s.zone==='frontier'?(s.cleared?'前線を突破。先へ進むか、帰還船へ。':'村人を救助し、前線を突破しよう。'):
      f.life.ageYears<7?'施設の近くで暮らしを学ぼう。7歳で武具庫が開く。':f.life.ageYears<15?'村で経験を重ねよう。15歳から船に乗れる。':story.canDepart(f.life)?'出航の年。船着き場から前線へ向かえる。':`次の出航：世界暦${story.nextDeparture(f.life)}年。村で暮らし続けてもよい。`;
    ...
    if(force||signature!==sig){signature=sig;const buttons=$('story-buttons');buttons.replaceChildren();const add=(text,id,enabled=true)=>buttons.append(button(text,id,enabled));
      if(s.phase==='birth'){for(const [id,name] of Object.entries(GIFTS))add(`${s.gifts.includes(id)?'受取済み：':''}${name}`,`gift:${id}`,s.gifts.length<2&&!s.gifts.includes(id));add('母に話しかける','talk');add('地面に降りる','release');}
        ...
        add(s.resting?'休息を終える':'休息する','rest');if(s.pendingDiscoveries.length)add('閃いた技を受け取る','discover');
      }
    }
    if(s.activity)$('story-message').textContent=`${EXPERIENCES[s.activity.kind]}を続けています… ${Math.ceil(s.activity.remaining)}秒`;
    else $('story-message').textContent=lastNotice||s.events[0]||'画面をスワイプして移動。短いフリックで走り続けます。技と視点は右下から。';
    if(s.phase==='ended')endScreen();sync();
  }
  on($('story-buttons'),'click',event=>{const b=event.target.closest('[data-action]');if(!b||b.disabled)return;const [kind,id,place]=b.dataset.action.split(':'),f=port.snapshot();port.stopMove();let result=false;
    if(kind==='gift')result=story.gift(id);
    if(kind==='talk'){notice('母「焦らなくていい。人と暮らして、あなたの生き方を見つけてね。」');return;}
    if(kind==='release')result=story.release();
    ...
    if(result){sync();save();}render(true);
  });
  ...
  on($('story-records'),'click',()=>{modalPaused=port.snapshot().paused;port.pause(true);records();dialog.showModal();});
  on($('story-close'),'click',()=>dialog.close());on(dialog,'keydown',e=>e.stopPropagation());on(death,'keydown',e=>e.stopPropagation());
  on(dialog,'close',()=>{if(story.state.phase!=='ended')port.pause(modalPaused);save();});
  ...
  on($('story-file'),'change',async()=>{const file=$('story-file').files[0];if(!file)return;
    try{if(file.size>2000000)throw Error('保存ファイルが大きすぎます。');const imported=readStorySave(JSON.parse(await file.text()));await port.restore(imported.runtime);layout=imported.world;story=new Story(imported.story,layout);sync();save();records();render(true);notice('保存した人生を読み込みました。');}
  ...
  on($('story-map-file'),'change',async()=>{const file=$('story-map-file').files[0];if(!file)return;
    try{if(file.size>8000000)throw Error('村の配置が大きすぎます。');const value=JSON.parse(await file.text()),next=value.schemaVersion===1&&value.terrainId?validateMuraLayout(value):projectMuraLayout(value.payload||value);applyWorld(next,true);records();paragraph($('story-dialog-content'),'村の配置を読み込みました。村の元データは変更しません。');}
  ...
  }sync();saveElapsed+=seconds;uiElapsed+=seconds;if(story.state.phase==='ended'||saveElapsed>=2){saveElapsed=0;save();}if(uiElapsed>=.15||story.state.phase==='ended'){uiElapsed=0;render();}});
  ...
  const dispose=()=>{if(disposed)return;save();disposed=true;active=false;lease.release();port.pause(true);unsubscribe();unsubscribeWorld();abort.abort();scene.dispose();hud.remove();panel.remove();dialog.remove();death.remove();style.remove();delete doc.body.dataset.story;};
