...
export async function mountStory(win,environment,{signal,onExit,onMusic}={}){
  ...
  function render(force=false){
    ...
    const canDepart=story.canDepart(f.life),guidance=gameplayGuidance({state:s,frame:f,canDepart,nextDeparture:story.nextDeparture(f.life)}),healthRatio=Math.max(0,f.hero.hp/f.hero.maxhp);
    ...
    if(guidance.target&&guidance.target!==lastGuidanceTarget){lastGuidanceTarget=guidance.target;ui.selectTarget(guidance.target);}
    ...
    const sig=JSON.stringify([s.phase,s.zone,near?place.id:null,f.hero.dead,Math.floor(f.life.ageYears)>=7,canDepart,!!s.activity,s.resting,s.gifts,s.pendingDiscoveries,s.cleared,s.front,s.rescue?.status,s.rescue&&story.near(s.rescue,f.hero),story.near({x:-5,z:3},f.hero)]);
    if(force||signature!==sig){signature=sig;const buttons=[];const add=(text,id,enabled=true)=>buttons.push(button(text,id,enabled));
      if(s.phase==='birth'){for(const [id,name] of Object.entries(GIFTS))add(`${s.gifts.includes(id)?'受取済み：':''}${name}`,`gift:${id}`,s.gifts.length<2&&!s.gifts.includes(id));add('母に話す','talk');add('自分の足で歩く','release');}
        ...
        if(s.activity)add('行動をやめる','cancel');add(s.resting?'休息を終える':'休息する','rest');
      }
  ...
  const dispose=()=>{if(disposed)return;save();disposed=true;active=false;lease.release();if(ui.canPause())port.pause(true);else port.blockInput(true);unsubscribe();unsubscribeWorld();abort.abort();win.clearInterval(uiTimer);win.clearTimeout(noticeTimer);scene.dispose();ui.destroy();dialog.remove();death.remove();style.remove();delete doc.body.dataset.story;};
