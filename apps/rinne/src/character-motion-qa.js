...
export function createWorkshopMotionQA({review,scene,camera,orbit,canvas,refresh,draw}) {
  ...
  async function start(){if(loading)return;loading=true;el('qa-start').disabled=true;notify('既存モーションを準備中…');
    try{if(!bank){bank=await loadWorkshopMotionSource({
      ...
      active=true;playing=true;time=0;pose=sample(0);lastFrame=null;snapshotPair={before:null,after:null};review.configure({view:'single',rotate:false,paused:false});
  ...
  function finishActor(actor,index,{ui=true}={}){if(!active)return;const qa=adapter(actor);let quality;if(corrected)quality=qa.correct();else{qa.reset();quality=qa.inspect();}
    ...
    const attachment=bank.attachments[Math.min(bank.attachments.length-1,Math.round(time*60))];const ageScale=appearanceForCharacter(review.records[index]).scale;
    ...
    if(ui&&index===review.settings.selected){const evaluated=captureNormalizedMotion(actor.bones,qa.rest);const sanity=inspectPose(evaluated,{mode:'warn',style:'expressive'}).issues;const transition=lastFrame&&time>lastFrame.time?inspectTransition(lastFrame.pose,evaluated,time-lastFrame.time):[];lastFrame={time,pose:evaluated};diagnostics=[...quality.issues,...sanity,...transition];observed=quality.corrections;sync();}
  }
