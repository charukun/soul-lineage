...
export function createWorkshopMotionQA({review,scene,camera,orbit,canvas,refresh,draw}) {
  ...
  let report=null,diagnostics=[],observed=[],snapshotPair={before:null,after:null},lastFrame=null,weaponSnapshot=null;
  ...
  async function start(){if(loading)return;loading=true;el('qa-start').disabled=true;notify('既存モーションを準備中…');
    try{if(!bank){bank=await loadWorkshopMotionSource({
      ...
      active=true;playing=true;time=0;speed=1;loopRange=null;el('qa-speed').value='1';el('qa-loop').checked=false;pose=sample(0);lastFrame=null;snapshotPair={before:null,after:null};review.configure({rotate:false,paused:false});
  ...
  function seek(value){if(!bank||!active)return;if(!Number.isFinite(value))throw new Error('Invalid QA seek');time=Math.max(0,Math.min(30,value));if(el('qa-loop').checked){const row=qaSequenceAt(time);loopRange=[row.start,row.end];}playing=false;lastFrame=null;pose=sample(time);cleanupWeapons();refresh();aim();sync();draw();}
  ...
  const api={
    ...
    finish(actor,index){if(!active)return;const qa=adapter(actor);let quality=corrected?continuousCorrection(actor,index,qa):qa.inspect();
    ...
    snapshot(){return {time,frame:qaSequenceAt(time).frame,camera:cameraId,cameraPosition:camera.position.toArray(),cameraTarget:orbit.target.toArray(),corrected,diagnostics,corrections:observed,weapon:weaponSnapshot,playback:{speed,loopRange},visualRequired:['skinning / weight','clothing','hair','silhouette'],conditions:conditions()};},
