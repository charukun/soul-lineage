...
export function createWorkshopMotionQA({review,scene,camera,orbit,canvas,refresh,draw}) {
  ...
  function sync(){const row=qaSequenceAt(time);el('qa-time').value=String(time);el('qa-motion').value=row.id;el('qa-frame').textContent=`${row.label} · ${time.toFixed(2)}秒 · frame ${row.frame} · 基盤 ${corrected?'ON':'OFF'}`;
    el('qa-play').textContent=playing?'Ⅱ 一時停止':'▶ 再生';el('qa-before').textContent=corrected?'基盤 ON':'基盤 OFF';el('qa-before').setAttribute('aria-pressed',String(corrected));el('qa-before').dataset.mode=corrected?'on':'off';
  ...
  async function start(){if(loading)return;loading=true;el('qa-start').disabled=true;notify('既存モーションを準備中…');
    try{if(!bank){bank=await loadWorkshopMotionSource({
      ...
      if(!report)freshReport();pose=sample(time);refresh();aim('front');sync();notify('再生しながら基盤ON/OFFを切り替えて見比べられます。');
    }catch(error){active=false;playing=false;notify(`準備できませんでした: ${error.message}`);}finally{loading=false;el('qa-start').disabled=false;}
  }
  function seek(value){if(!bank||!active)return;time=Math.max(0,Math.min(30,value));playing=false;redrawCurrent();}
  ...
  function streamlineUI(){const root=el('motion-qa'),heading=root.querySelector('h2'),quick=el('qa-start').parentElement,step=el('qa-prev').parentElement,diagnostic=root.querySelector('details'),tourLabel=el('qa-tour').closest('label'),countRow=root.querySelector('[data-qa-count]')?.parentElement,motionLabel=el('qa-motion').parentElement;
    if(heading)heading.textContent='モーション比較';if(el('tab-qa'))el('tab-qa').textContent='動き比較';el('qa-start').textContent='▶ 30秒を開始';el('qa-stop').textContent='終了';quick.classList.add('qa-quickbar');quick.append(el('qa-before'));step.classList.add('qa-stepbar');if(motionLabel?.firstChild)motionLabel.firstChild.textContent='区間へ移動';
    ...
    const more=document.createElement('details');more.className='qa-more';const summary=document.createElement('summary');summary.textContent='表示オプション';more.append(summary);if(tourLabel)more.append(tourLabel);if(countRow)more.append(countRow);el('qa-cameras').after(more);
  ...
  streamlineUI();
  const style=document.createElement('style');style.dataset.motionQaAb='true';style.textContent=`#motion-qa>h2{font-size:1rem;margin-bottom:.3rem}#qa-status{margin:.2rem 0 .55rem;color:#b9c9c8}.qa-quickbar{position:sticky;top:-9px;z-index:4;display:grid!important;grid-template-columns:1.05fr 1.05fr .7fr;gap:6px;padding:8px 0 9px;background:linear-gradient(#18272c 78%,rgba(24,39,44,0));backdrop-filter:blur(5px)}.qa-quickbar #qa-start{grid-column:1/-1;min-height:46px}.qa-quickbar #qa-play,.qa-quickbar #qa-before,.qa-quickbar #qa-stop{min-height:44px}.qa-quickbar #qa-before{font-weight:750}.qa-quickbar #qa-before[data-mode="on"]{border-color:#d8be83;background:#39382e;color:#ffecc1}.qa-quickbar #qa-before[data-mode="off"]{border-color:#d98f75;background:#442c27;color:#ffd4c7}.qa-stepbar{display:grid!important;grid-template-columns:1fr 1fr;gap:6px;margin:.35rem 0}.qa-section-title{margin:.7rem 0 .3rem!important}.qa-more,.qa-diagnostics-panel{margin:.65rem 0;border-top:1px solid rgba(255,255,255,.08);padding-top:.25rem}.qa-more summary,.qa-diagnostics-panel summary{min-height:42px;padding:9px 2px;cursor:pointer;color:#d7ddd8}#qa-ab{margin:.75rem 0;padding:.7rem;border:1px solid rgba(220,196,147,.38);border-radius:10px;background:rgba(10,20,23,.4)}#qa-ab .qa-ab-head{display:flex;align-items:flex-start;justify-content:space-between;gap:.5rem;flex-wrap:wrap}#qa-ab .qa-ab-head h3{margin:0;color:#efe4c8;font-size:.82rem}#qa-ab .qa-ab-actions{display:flex;gap:.4rem;flex-wrap:wrap}#qa-ab .qa-ab-meta{display:block;margin:.4rem 0;font-size:.72rem;opacity:.82}#qa-ab[data-stale="true"] .qa-ab-meta::after{content:' · 現在の条件と異なります';color:#efcf8f}#qa-ab .qa-ab-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.45rem}#qa-ab figure{min-width:0;margin:0;border:1px solid rgba(255,255,255,.12);border-radius:8px;overflow:hidden;background:#0b1316}#qa-ab figcaption{padding:.38rem .45rem;font-size:.76rem;font-weight:700}#qa-ab img{display:block;width:100%;min-height:110px;max-height:260px;object-fit:contain;background:#081013}#qa-ab figure a{display:block;padding:.35rem .45rem;font-size:.7rem}#qa-ab-summary{margin:.5rem 0 0;white-space:pre-wrap;font-size:.72rem;line-height:1.45}@media(max-width:420px){.qa-quickbar{grid-template-columns:1fr 1fr .62fr}.qa-quickbar button{padding-left:5px;padding-right:5px;font-size:.72rem}#qa-ab .qa-ab-actions{display:grid;grid-template-columns:1fr 1fr;width:100%}#qa-ab .qa-ab-actions button{font-size:.7rem;padding:6px}}`;document.head.append(style);
  const ab=document.createElement('section');ab.id='qa-ab';ab.dataset.stale='false';const abHead=document.createElement('div');abHead.className='qa-ab-head';const abTitle=document.createElement('h3');abTitle.textContent='A/B 同条件比較';const abActions=document.createElement('div');abActions.className='qa-ab-actions';const abKnown=document.createElement('button');abKnown.id='qa-ab-known';abKnown.type='button';abKnown.textContent='既知差 17.475秒';const abRefresh=document.createElement('button');abRefresh.id='qa-ab-refresh';abRefresh.type='button';abRefresh.textContent='今の位置で比較';abActions.append(abKnown,abRefresh);abHead.append(abTitle,abActions);const abMeta=document.createElement('output');abMeta.id='qa-ab-meta';abMeta.className='qa-ab-meta';abMeta.textContent='左右に同じフレームの基盤OFF / ONを固定します。';const abGrid=document.createElement('div');abGrid.className='qa-ab-grid';
  ...
  abGrid.append(makeFigure('before','A · 基盤OFF','A画像を保存'),makeFigure('after','B · 基盤ON','B画像を保存'));const abSummary=document.createElement('pre');abSummary.id='qa-ab-summary';abSummary.textContent='自己交差・対象ボーン・適用補正を補助表示します。数値だけで合格にはしません。';ab.append(abHead,abMeta,abGrid,abSummary);const more=el('motion-qa').querySelector('.qa-more');el('motion-qa').insertBefore(ab,more??el('motion-qa').querySelector('details'));abKnown.onclick=()=>void compareKnown();abRefresh.onclick=()=>void compareCurrent();
