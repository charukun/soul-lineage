...
const entry=row=>Object.freeze({...row,effects:Object.freeze(row.effects),tags:Object.freeze(row.tags),affinities:Object.freeze(row.affinities??[]),cues:Object.freeze(row.cues??[])});
...
const CORE_EFFECTS=[
  ...
  entry({id:'original-light',label:'Light・原本',category:'finisher',kind:'original',mode:'raw',context:'finisher',effects:['finisher'],tags:['原本','光','大技','オーラ'],affinities:['light'],cues:[cue('finisher','impact',{lifetime:2,priority:3})]}),
  ...
  entry({id:'original-water',label:'ToonWater・原本',category:'elemental',kind:'original',mode:'raw',context:'finisher',effects:['water'],tags:['原本','水','属性','魔法'],affinities:['water'],cues:[cue('water','impact',{lifetime:1.7,priority:3})]}),
...
function libraryCategory(effect){
  if(effect.affinities?.length)return'elemental';
...
function libraryEntry(effect){
  ...
  return entry({
    ...
    realSource:true,sourcePath:effect.sourcePath,author:effect.author,effects:[effect.id],affinities:effect.affinities||[],
    tags:['実素材','原本',effect.license||'license-pinned',effect.author,group,category,...(effect.affinities||[])],
