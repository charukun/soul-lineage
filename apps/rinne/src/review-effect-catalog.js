...
const entry=row=>Object.freeze({...row,effects:Object.freeze(row.effects),tags:Object.freeze(row.tags),attributes:Object.freeze(row.attributes??[]),cues:Object.freeze(row.cues??[])});
...
const CORE_EFFECTS=[
  ...
  entry({id:'original-light',label:'Light・原本',category:'finisher',kind:'original',mode:'raw',context:'finisher',effects:['finisher'],tags:['原本','光','大技','オーラ'],attributes:['light'],cues:[cue('finisher','impact',{lifetime:2,priority:3})]}),
  ...
  entry({id:'original-water',label:'ToonWater・原本',category:'elemental',kind:'original',mode:'raw',context:'finisher',effects:['water'],tags:['原本','水','属性','魔法'],attributes:['water'],cues:[cue('water','impact',{lifetime:1.7,priority:3})]}),
...
function libraryCategory(effect){
  if(effect.attributes?.length)return'elemental';
...
function libraryLabel(effect){
  ...
  return name.replace(/^(?:AndrewFM01|NextSoft01|Pierre01|Pierre02|Suzuki01|Tktk01|Tktk02|Tktk03|Basic)_/,'').replace(/^MxS_/,'').replaceAll('_',' ');
}
function libraryEntry(effect){
  ...
  return entry({
    ...
    realSource:true,sourcePath:effect.sourcePath,author:effect.author,effects:[effect.id],attributes:effect.attributes||[],
    tags:['実素材','原本',effect.license||'license-pinned',effect.author,group,category,...(effect.attributes||[])],
