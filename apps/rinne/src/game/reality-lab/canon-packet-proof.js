...
function proposalFor({epoch,revision,parentRoot,operationId,canon,recovery,priorDedupe={}}){
...
function validProposal(proposal){
  if(!proposal||!Number.isInteger(proposal.epoch)||proposal.epoch<1||!Number.isInteger(proposal.revision)||proposal.revision<1||!validId(proposal.operationId)||typeof proposal.recovery?.operationDedupe!=='object')return false;
  const payloadKey=canonical({operationId:proposal.operationId,canon:proposal.canon,recovery:proposal.recovery.application});
  return proposal.payloadKey===payloadKey&&proposal.recovery.operationDedupe[proposal.operationId]===payloadKey&&proposal.proofKey===canonical({epoch:proposal.epoch,revision:proposal.revision,parentRoot:proposal.parentRoot,operationId:proposal.operationId,canon:proposal.canon,recovery:proposal.recovery})&&proposal.root===digest({epoch:proposal.epoch,revision:proposal.revision,parentRoot:proposal.parentRoot,operationId:proposal.operationId,canon:proposal.canon,recovery:proposal.recovery});
}
...
export function createCanonPacketProtocol({failures=1,members=null,leaderId=null,allowUnsafeProof=false}={}){
  ...
  const live=()=>members.filter(id=>nodes.get(id).alive),liveCount=()=>live().length;
  ...
  function store(node,proposal,state='prepared',{repair=false}={}){
    if(!validProposal(proposal))return false;if(!repair&&proposal.epoch<node.maxEpoch){rejectedStale++;return false;}
    const key=entryKey(proposal),prior=node.entries.get(key);if(prior&&prior.proposal.proofKey!==proposal.proofKey){rejectedConflict++;return false;}
    node.entries.set(key,{proposal:clone(proposal),state:state==='committed'||prior?.state==='committed'?'committed':'prepared'});if(!repair)node.maxEpoch=Math.max(node.maxEpoch,proposal.epoch);return true;
  }
  ...
  function receipt(proposal,{recovered=false,replayOf=null}={}){return Object.freeze({epoch:proposal.epoch,revision:proposal.revision,parentRoot:proposal.parentRoot,operationId:proposal.operationId,root:proposal.root,recovered,replayOf});}
  function begin({operationId,canon,recovery,expectedEpoch=epoch}={}){
    ...
    if(priorKey){
      ...
      if(committed.operationId===operationId&&!visible.some(row=>row.root===committed.root)){const replay=receipt(committed,{recovered:true,replayOf:operationId});visible.push({...replay,durableAtVisibility:holders(committed).length,replayed:true});return{replay:true,receipt:clone(replay),proposal:clone(committed)};}
    ...
    const proposal=proposalFor({epoch,revision:nextRevision++,parentRoot:committed?.root??'genesis',operationId,canon,recovery,priorDedupe:committed?.recovery?.operationDedupe??{}});
    if(!store(nodes.get(leader),proposal,'prepared'))throw Error('Leader could not durably prepare Canon');active={proposal,acks:new Set([leader]),visible:false};for(const id of members)if(id!==leader)enqueue(PACKET_TYPE.PREPARE,leader,id,proposal);return{replay:false,proposal:clone(proposal)};
  }
  ...
  function publish(){if(!quorumReady())throw Error('Canon durable quorum unavailable');if(phase!==PACKET_PHASE.OPEN||!leader||!nodes.get(leader).alive)throw Error('Canon authority is not open');active.visible=true;committed=clone(active.proposal);store(nodes.get(leader),active.proposal,'committed',{repair:true});const durableAtVisibility=holders(active.proposal).length,row={...receipt(active.proposal),durableAtVisibility};visible.push(row);for(const id of members)if(id!==leader)enqueue(PACKET_TYPE.COMMIT,leader,id,active.proposal);return clone(row);}
  ...
  function recover({candidateId=live()[0]}={}){
    ...
    const chosen=bestSurvivingProposal();if(chosen===false){leader=null;phase=PACKET_PHASE.CLOSED;return false;}const leaderChanged=!leader||!nodes.get(leader)?.alive;if(chosen){for(const id of survivors)store(nodes.get(id),chosen,'committed',{repair:true});committed=clone(chosen);nextRevision=Math.max(nextRevision,chosen.revision+1);}else if(visible.length){leader=null;phase=PACKET_PHASE.CLOSED;return false;}
  ...
  function safety(){const uniqueVisible=new Map(),conflicts=[];let parent='genesis';for(const row of visible){const key=`${row.epoch}:${row.revision}`,prior=uniqueVisible.get(key);if(prior&&prior!==row.root)conflicts.push(key);else uniqueVisible.set(key,row.root);if(!row.replayed&&row.parentRoot!==parent)conflicts.push(`parent:${key}`);if(!row.replayed)parent=row.root;}const quorumVisibility=visible.every(row=>row.durableAtVisibility>=config.quorum),openDurability=phase!==PACKET_PHASE.OPEN||!committed||holders(committed).length>=config.quorum;return Object.freeze({pass:conflicts.length===0&&quorumVisibility&&openDurability,conflicts,quorumVisibility,openDurability});}
...
export function proveSequentialEpochAndDedupe(){
  ...
  const c=protocol.begin({operationId:'life:C',canon:{generation:3},recovery:{tick:30}});deliverPrepare(protocol,'n2');deliverAck(protocol,'n2');const cr=protocol.publish();
  const stale={type:PACKET_TYPE.PREPARE,from:'n0',to:'n1',epoch:b.proposal.epoch,revision:b.proposal.revision,root:b.proposal.root,proofKey:b.proposal.proofKey,proposal:b.proposal};protocol.inject(stale);const staleAccepted=protocol.deliverWhere(row=>row.type===PACKET_TYPE.PREPARE&&row.epoch===b.proposal.epoch&&row.revision===b.proposal.revision),final=protocol.snapshot();
  const roots=final.visible.map(row=>row.root),parents=final.visible.filter(row=>!row.replayed).map(row=>row.parentRoot);
  return{pass:recovered&&afterRecovery.committed?.root===b.proposal.root&&replayB.replay&&replayA.replay&&revisionAfterB===b.proposal.revision&&conflictRejected&&cr.parentRoot===b.proposal.root&&ar.parentRoot==='genesis'&&staleAccepted===false&&final.rejectedStale>=1&&roots.length===3&&roots[0]===a.proposal.root&&roots[1]===b.proposal.root&&roots[2]===c.proposal.root&&parents[0]==='genesis'&&parents[1]===b.proposal.root&&final.safety.pass,recovered,replayB:replayB.replay,replayOlderOperation:replayA.replay,conflictRejected,staleAccepted,visible:final.visible,safety:final.safety};
}
