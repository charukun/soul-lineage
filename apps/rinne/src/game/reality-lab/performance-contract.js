...
// Absolute thresholds are limited to targets already established elsewhere in the repository.
...
// These metrics need a compatible physical-multipeer baseline before an absolute product SLO is asserted.
...
export const PHYSICAL_MULTIPEER_REQUIRED_METRICS=Object.freeze([...new Set([
  ...
  'hostLossDetectionP95Ms','hostReopenP95Ms','frameP95Ms',
])]);
...
function normalizeMetric(key,value){
  ...
  const number=Number(value),isRate=key.endsWith('Rate');
...
function finiteSamples(values){return Array.isArray(values)?values.map(Number).filter(Number.isFinite):[];}
function average(values){const rows=finiteSamples(values);return rows.length?rows.reduce((sum,value)=>sum+value,0)/rows.length:null;}
function peak(values){const rows=finiteSamples(values);return rows.length?Math.max(...rows):null;}
...
export function aggregateRrpPerformanceSamples(samples={}){
  ...
  const rollback=finiteSamples(samples.rollbackMs),durationMinutes=Number(samples.durationMinutes??0);
  ...
  return Object.freeze({
    ...
    stateFreshnessP95Ms:pct(samples.stateFreshnessMs,.95),stateFreshnessP99Ms:pct(samples.stateFreshnessMs,.99),
    positionErrorP95M:pct(samples.positionErrorM,.95),positionErrorMaxM:peak(samples.positionErrorM),
    ...
    rollbackPerMinute:durationMinutes>0?rollback.length/durationMinutes:null,rollbackP95Ms:rollback.length?percentile(rollback,.95):null,rollbackMaxMs:rollback.length?Math.max(...rollback):null,
...
export function realityResultToPerformanceEvidence(result,{scenario='steady',canonSafety=null}={}){
  ...
  const elapsedSeconds=Math.max(.001,Number(result.elapsedMs||0)/1000),messages=Math.max(0,Number(result.messages||0));
  const missing=Math.max(0,Number(result.missingSamples||0)),measured=Math.max(0,Number(result.errorSamples||0));
  ...
  return normalizeRrpPerformanceEvidence({
    ...
    samples:messages,
    metrics:{
      hostUplinkAverageKbps:numberOrNull(result.primaryKbps),peerUplinkAverageKbps:numberOrNull(result.maxPeerKbps),
      modelDeliveryP95Ms:numberOrNull(result.modelDeliveryP95Ms),modelMaxQueue:numberOrNull(result.maxQueue),
      modelDarkMs:numberOrNull(result.darkMs),modelRollbackMs:numberOrNull(result.rollbackMs),positionErrorMaxM:numberOrNull(result.maxPositionError),
...
export function compareRrpPerformanceEvidence(baselineInput,currentInput,{ratios=RRP_RATCHET}={}){
  ...
  return Object.freeze({pass:contract.status===PERFORMANCE_STATUS.PASS&&regressions.length===0,compatibility,contract,checks,regressions,improvements});
}
...
function completePhysicalFixture(overrides={}){
  ...
  const metrics={
    ...
    stateFreshnessP95Ms:180,positionErrorP95M:.35,rollbackP95Ms:80,connectionSuccessRate:.99,frameP95Ms:30,
    ...(overrides.metrics||{}),
  };
  return {evidenceClass:EVIDENCE_CLASS.PHYSICAL_MULTIPEER,samples:300,safety:{...safety,...(overrides.safety||{})},metrics,provenance:{buildRevision:'proof',runtime:'browser',deviceClass:'physical-mobile',deviceModel:'fixture',peers:3,networkProfile:'fixture',...(overrides.provenance||{})}};
}

export function runRrpPerformanceContractProofSuite(){
  const safe=evaluateRrpPerformanceContract(completePhysicalFixture());
  const unsafe=evaluateRrpPerformanceContract(completePhysicalFixture({metrics:{inputToDisplayP95Ms:1,hostUplinkP95Kbps:1},safety:{committedCanonRollbackEvents:1}}));
  const slow=evaluateRrpPerformanceContract(completePhysicalFixture({metrics:{hostLossDetectionP95Ms:4501}}));
  const missing=completePhysicalFixture();delete missing.metrics.inputToDisplayP95Ms;
  const calibration=evaluateRrpPerformanceContract(missing);
  ...
  const baseline=completePhysicalFixture(),regressed=completePhysicalFixture({metrics:{inputToDisplayP95Ms:220}}),ratchet=compareRrpPerformanceEvidence(baseline,regressed);
  const checks={
    completePhysicalPasses:safe.status===PERFORMANCE_STATUS.PASS&&safe.physicalCertificationEligible,
    ...
    ratchetRejectsRegression:ratchet.pass===false&&ratchet.regressions.some(row=>row.metric==='inputToDisplayP95Ms'),
