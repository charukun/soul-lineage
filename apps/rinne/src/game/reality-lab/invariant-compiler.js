...
function uniqueRegisterWitness(resource,a,b){
  if(!a||!b||!Object.hasOwn(a,'assign')||!Object.hasOwn(b,'assign'))return null;
  if(a.assign===b.assign)return null;
  return{resourceId:resource.id,type:resource.type,base:null,left:a.assign,right:b.assign,merged:'conflict'};
}
function tokenWitness(resource,a,b){
  if(a?.consume===true&&b?.consume===true)return{resourceId:resource.id,type:resource.type,base:'available',left:'consumed',right:'consumed',merged:'double-consume'};
  return null;
}
function conflictWitness(resource,a,b){
  if(resource.type==='grow-only-set')return null;
  if(resource.type==='bounded-counter')return boundedCounterWitness(resource,a,b);
  if(resource.type==='unique-register')return uniqueRegisterWitness(resource,a,b);
  if(resource.type==='single-use-token')return tokenWitness(resource,a,b);
  throw Error('Unknown invariant resource');
}
...
export function compileInvariantCoordination({resources=[],operations=[]}={}){
  ...
  for(let i=0;i<ops.length;i++)for(let j=i+1;j<ops.length;j++){
    ...
    if(witnesses.length)conflicts.push({key:effectKey(a,b),left:a.id,right:b.id,resources:shared,witnesses});
    else safePairs.push({key:effectKey(a,b),left:a.id,right:b.id,resources:shared});
  }
...
export function verifyInvariantCoordinationCertificate(certificate){
  if(certificate?.schema!=='rrp-invariant-kernel/1'||typeof certificate.root!=='string')return false;
  try{
    const rebuilt=compileInvariantCoordination({resources:certificate.resources,operations:certificate.operations});return rebuilt.certificate.root===certificate.root;
  }catch{return false;}
}

export function proveInvariantCompilerKernel(){
  const compiled=compileInvariantCoordination({resources:[
    {id:'stock',type:'bounded-counter',min:0,max:2},
    {id:'lineage-name',type:'unique-register'},
    {id:'rebirth-token',type:'single-use-token'},
    {id:'discoveries',type:'grow-only-set'},
  ],operations:[
    {id:'reserve-a',effects:{stock:{delta:-1}}},
    {id:'reserve-b',effects:{stock:{delta:-1}}},
    {id:'name-a',effects:{'lineage-name':{assign:'A'}}},
    {id:'name-b',effects:{'lineage-name':{assign:'B'}}},
    {id:'rebirth-a',effects:{'rebirth-token':{consume:true}},irreversible:true,requiresCrashSurvival:true,kind:OP_KIND.CANON},
    {id:'rebirth-b',effects:{'rebirth-token':{consume:true}},irreversible:true,requiresCrashSurvival:true,kind:OP_KIND.CANON},
    {id:'discover-a',effects:{discoveries:{add:'x'}},mergeable:true},
    {id:'discover-b',effects:{discoveries:{add:'y'}},mergeable:true},
  ]});
  const edges=new Set(compiled.conflicts.map(row=>row.key)),safe=new Set(compiled.safePairs.map(row=>row.key));
  const expectedConflicts=['reserve-a::reserve-b','name-a::name-b','rebirth-a::rebirth-b'];
  const witnessMinimal=expectedConflicts.every(key=>edges.has(key)&&compiled.conflicts.find(row=>row.key===key)?.witnesses.length>0);
  const growOnlySafe=safe.has('discover-a::discover-b');
  const reserveOps=compiled.semanticOperations.filter(op=>op.id.startsWith('reserve-')),discoverOps=compiled.semanticOperations.filter(op=>op.id.startsWith('discover-'));
  const conservativeEscalation=reserveOps.every(op=>op.requiresTotalOrder&&!op.rollbackAllowed)&&discoverOps.every(op=>!op.requiresTotalOrder&&op.rollbackAllowed);
  const certificateValid=verifyInvariantCoordinationCertificate(compiled.certificate),tampered=clone(compiled.certificate);tampered.conflicts=[];
  const tamperRejected=verifyInvariantCoordinationCertificate(tampered)===false;
...
export function proveCompiledSemanticFrontier(){
  const compiled=compileInvariantCoordination({resources:[{id:'stock',type:'bounded-counter',min:0,max:1},{id:'discoveries',type:'grow-only-set'}],operations:[
    {id:'reserve-a',effects:{stock:{delta:-1}},bytes:64,recipients:2},
    {id:'reserve-b',effects:{stock:{delta:-1}},bytes:64,recipients:2},
    {id:'discover',effects:{discoveries:{add:'x'}},bytes:48,recipients:4,mergeable:true},
  ]});
  const result=semanticFrontier(compiled.semanticOperations,{players:30,trustedAuthority:true,crashReplicas:3,rttMs:80,jitterMs:20});
  const hasPlan=result.frontier.length>0;
  ...
  return{pass:hasPlan&&unsafeNeverWeak&&mergeableCanStayWeak,hasPlan,unsafeNeverWeak,mergeableCanStayWeak,frontierPoints:result.frontier.length,compiled,result};
}
