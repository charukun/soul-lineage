...
export function relayPresencePlan({players=30,relays=5,redundancy=1,payloadBytes=1,hopLatencyMs=0}={}){
  ...
  for(let i=0;i<nonRelayIds.length;i++){
    const chosen=[];for(let copy=0;copy<redundancy;copy++){const relay=relayIds[(i*redundancy+copy)%relays];chosen.push(relay);relayLoads[relay]++;}assignments[nonRelayIds[i]]=chosen;
  }
...
export function relayFanoutLowerBound({players=30,relays=5,redundancy=1}={}){
  integer(players,'players',2);integer(relays,'relays',1);integer(redundancy,'redundancy',1);const recipients=players-1;if(relays>recipients||redundancy>relays)throw Error('Invalid relay lower-bound input');
  const nonRelays=recipients-relays,minRelayFanout=Math.ceil(redundancy*nonRelays/relays);return Object.freeze({hostFanout:relays,minRelayFanout,minMaxSenderFanout:Math.max(relays,minRelayFanout)});
}
...
export function optimizeRelayPresence({players=30,redundancy=1,payloadBytes=1,hopLatencyMs=0}={}){
  ...
  for(let relays=redundancy;relays<=recipients;relays++){
    const plan=relayPresencePlan({players,relays,redundancy,payloadBytes,hopLatencyMs});if(!best||plan.maxSenderFanout<best.maxSenderFanout||plan.maxSenderFanout===best.maxSenderFanout&&plan.hostFanout<best.hostFanout||plan.maxSenderFanout===best.maxSenderFanout&&plan.hostFanout===best.hostFanout&&plan.aggregateBytes<best.aggregateBytes)best=plan;
  }
  return best;
}
...
export function relayFailureCoverage(plan,{failures=1}={}){
  ...
  const scenarios=[];for(const failed of combinations(plan.relayIds,failures)){
    const uncovered=plan.nonRelayIds.filter(id=>plan.assignments[id].every(relay=>failed.includes(relay)));scenarios.push({failed,uncovered});
  }
  return{pass:scenarios.every(row=>row.uncovered.length===0),failures,scenarios,noIntermediaryDependency:false};
}

function validatePresenceOperation(operation={}){
  if(operation.replaceable!==true||operation.irreversible||operation.requiresTotalOrder||operation.requiresCrashSurvival||operation.requiresByzantine||operation.requiresExternalOrder)throw Error('Adaptive relay is only valid for replaceable presence');return true;
}
...
export function chooseAdaptivePresence({players=30,payloadBytes=1,hopLatencyMs=0,maxSenderFanout=Infinity,maxExtraHops=1,requiredRelayFailures=0,operation={replaceable:true}}={}){
  ...
  feasible.sort((a,b)=>a.hostUplinkBytes-b.hostUplinkBytes||a.maxSenderFanout-b.maxSenderFanout||a.aggregateBytes-b.aggregateBytes||a.maxPathHops-b.maxPathHops);
...
export function proveRelayConstructionOptimality({maxPlayers=64,maxRedundancy=2}={}){
  ...
  for(let players=3;players<=maxPlayers;players++)for(let redundancy=1;redundancy<=Math.min(maxRedundancy,players-1);redundancy++){
    const best=optimizeRelayPresence({players,redundancy});let theoretical=Infinity;
    for(let relays=redundancy;relays<=players-1;relays++)theoretical=Math.min(theoretical,relayFanoutLowerBound({players,relays,redundancy}).minMaxSenderFanout);
...
export function proveDenseAdaptiveRelay(){
  const players=30,payloadBytes=1024,star=directPresenceStar({players,payloadBytes}),r1=optimizeRelayPresence({players,redundancy:1,payloadBytes}),r2=optimizeRelayPresence({players,redundancy:2,payloadBytes});
  const r1Failure=relayFailureCoverage(r1,{failures:1}),r2Failure=relayFailureCoverage(r2,{failures:1});
