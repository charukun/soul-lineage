...
export function runArchitectureProofLoop(){
  const canon=runCanonNucleusProofSuite(),semantic=runSemanticFrontierProofSuite(),cases=[];
  ...
  const boundaries={denseCounterexampleRetained:Boolean(dense&&!dense.presence.cellFanoutWin),spreadBenefitRetained:Boolean(spread&&spread.presence.cellFanoutWin),hostRecoveryObserved:Boolean(host&&host.results.every(row=>row.phase==='open'&&row.migrations.length===1&&row.freezeViolations===0)),corruptionRepairObserved:Boolean(corruption&&corruption.results.every(row=>row.invalid===1&&row.repairs>=1)),universalStrictDominanceRejected:semantic.maximal.strictUniversalDominancePossible===false,semanticPolicyClosure:semantic.maximal.weakPolicyClosure===true};
  const pass=canon.pass&&semantic.pass&&cases.every(row=>row.pass)&&Object.values(boundaries).every(Boolean);
  return{format:'rrp-architecture-proof/2',pass,canon,semantic,cases,boundaries,limits:['model wire does not certify SCTP/NAT/TURN','Semantic Frontier is a policy closure, not a claim of physically impossible universal strict dominance','CAP/FLP and crash/BFT lower bounds remain hard constraints','cross-policy state translation beyond declared invariants/commutativity still needs app-specific proof','replaceable realtime state may roll back to the last accepted recovery boundary']};
}
