...
export function proveSemanticProtocolComposition(){
  ...
  const quorum=crashQuorumConfig(1);
  const checks={presencePolicyFeasible:presencePolicy.feasible===true,canonPolicyFeasible:canonPolicy.feasible===true&&canonPolicy.guarantees?.recoveryComplete===true,packetShapeMatchesCanonPolicy:quorum.members===3&&quorum.quorum===2,relayOnlyRefinesPresence:relay.pass&&relay.selected?.kind==='adaptive-relay'&&canonRelayRejected,byzantineEscalatesOutOfNucleus:adversarialNucleus.feasible===false&&adversarialPbft.feasible===true};
...
export function runRrpTheoryVerificationSuite({semantic=null}={}){
  const packets=runCanonPacketProofSuite(),relay=runAdaptiveRelayProofSuite(),semanticProof=semantic??runSemanticFrontierProofSuite(),composition=proveSemanticProtocolComposition();
  const boundaries={
