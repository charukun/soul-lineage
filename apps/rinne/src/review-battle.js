...
const phasePanel=createPhasePanel(q('battle-phase'),{skin:'rinne',brand:'輪廻転焦',details:true});
...
function renderPhase(core){
  ...
  phasePanel.render({
    phase:state.heroPhase||state.phase,
    opponent:core?.enemy?.name||'相手',
    skill:state.skill||core?.hero?.attack||'間合いを測る',
    opponentHp:core?.enemy?.hp,
    opponentMaxHp:core?.enemy?.maxhp,
