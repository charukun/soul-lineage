...
function runtimeLoadout(engine,weapon){
  const current=engine.loadout(),templates=engine.templates().filter(row=>row.weapon===weapon&&row.type!=='reaction');
  const result={uke:current.uke};
  for(const [index,phase] of ['jo','ha','kyu'].entries()){
    result[phase]=runtimeRecipe(learnedSlots[phase],phase,weapon)||structuredClone(templates[index%Math.max(1,templates.length)]||current[phase]);
  }
  return result;
}
...
function handleInspirationCue(cue,payload={}){
  ...
  if(cue==='spark'){inspirationSequenceActive=true;hideInspirationBanner();battleSfx.draw();return;}
...
function resetBattle(){
  lastCore=null;finishedAt=0;hideReviewSign();inspirationSequenceActive=false;lastSequenceAction='';lastSequencePhase='';lastAudioAttacks.clear();phaseHistory?.replaceChildren();battleStage?.resetRound();battleSfx.reset();if(battleSfx.unlocked)battleSfx.draw();
  ...
  runtime.configure({
    weapon,loadout,opponent:group?'group':'duel',hp:230,maxhp:230,enemyHp:180,enemyWeapon:'sword',enemyStyle:'balanced',mindset:'balanced',positions,
    ...(group?{enemies:[{weapon:'sword',hp:180},{weapon:'spear',hp:165},{weapon:'axe',hp:195}]}:{})
  });
...
function advanceBattle(dt){
  ...
  lastCore=runtime.step(dt);
}
function frame(now){
  const dt=Math.min(.05,Math.max(0,(now-last)/1000));last=now;
  const finished=Boolean(lastCore?.done);
  ...
  if(reviewBattleLoopDue({loopEnabled,playing:true,finished:nextFinished,finishedAt,now}))resetBattle();
...
phasePanel.dataset.skin='rinne';syncSoundButton();renderLearnedSlots();renderInsightHistory();resetBattle();void ensureBattleStage();requestAnimationFrame(frame);
