...
export function installOnlinePlayer({mount=document.querySelector('main'),environment='local'}={}){const PLAYER_KEY=`soul.${environment}.rinne.player.v1`;let peer,x=0,z=0;const playerId=localStorage.getItem(PLAYER_KEY)||crypto.randomUUID();localStorage.setItem(PLAYER_KEY,playerId);const root=document.createElement('section');root.innerHTML=`<h2>村へ参加</h2><p id="peer-state">村長の参加コードを貼り付けます</p><textarea id="host-offer"></textarea><button id="join-peer">応答コードを作る</button><textarea id="peer-answer" readonly></textarea><div><button data-move="0,-1">↑</button><button data-move="-1,0">←</button><button data-move="1,0">→</button><button data-move="0,1">↓</button></div><pre id="raid-state"></pre>`;mount.append(root);style(root);const $=q=>root.querySelector(q),status=t=>$('#peer-state').textContent=t;
 ...
 root.querySelectorAll('[data-move]').forEach(b=>b.onclick=()=>{if(!peer)return;const[a,c]=b.dataset.move.split(',').map(Number);x+=a*2;z+=c*2;try{peer.send({type:'state',x,z});}catch(e){status(e.message);}});return()=>{peer?.close?.();root.remove();};}
