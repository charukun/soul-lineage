...
const asset=name=>`${import.meta.env.BASE_URL}ui/storybook/${name}.webp`;
...
const categoryName={stance:'構え',style:'戦法',zanshin:'残心',weapon:'武器',armor:'防具',shield:'盾'};

/** Production controller: consumes existing life-owned state and uses only canonical setters. */
export function installStorybookUI(ui,{gameScreen,audio,requestEquip}){
  ...
  const views=Object.keys(BOOK_PAGES),selection=Object.fromEntries(views.map(key=>[key,{slot:key==='heart'?0:key==='technique'?'jo':key==='body'?'stance':'weapon',category:'all',filter:'all',query:'',sort:'learned',id:null,page:0}]));
  let state=null,disposed=false,renderKey='',status='',help=false;
  ...
  function mutate(fn,message){const model=currentModel();if(model.readonly){update(model.readonly);return;}const result=fn();if(result===false){update('この設定は適用できません。');return;}audio?.item?.();gameScreen.dispatchEvent(new CustomEvent('rinne:loadout-changed',{detail:{page:model.page}}));update(message);}
  function choose(id){const model=currentModel(),s=selection[model.page];s.id=id;status='';render();}
  ...
  function mind(model){let cursor=0;const colors=['#bd886d','#a5bb97','#89aeca','#c0a3c7','#8bb9ae','#d6bf7d'];const stops=model.mind.map((v,i)=>{const a=cursor;cursor+=v.value*100;return `${colors[i]} ${a}% ${cursor}%`;});return `<div class="rb-mind" aria-label="現在の意識バランス"><div role="img" aria-label="意識バランス" style="background:conic-gradient(${stops.join(',')})"></div><dl>${model.mind.map(v=>`<div><dt>${v.label}</dt><dd>${Math.round(v.value*100)}%</dd></div>`).join('')}</dl></div>`;}
  function bodyChart(model){const axes=BOOK_TRAITS.map(([key,label],i)=>{const a=(i*72-90)*Math.PI/180,r=Math.max(.35,Math.min(1.25,Number(model.body[key])||1))*.7;return {label,value:model.body[key]||1,x:50+40*Math.cos(a)*r,y:50+40*Math.sin(a)*r,tx:50+46*Math.cos(a),ty:50+46*Math.sin(a)};});return `<section class="rb-body-chart"><h3>現在の身体の傾向</h3><div><svg viewBox="0 0 100 100" role="img" aria-label="身体の傾向。生来の標準を100として表示"><path d="M50 10L88 38L73 82L27 82L12 38Z M50 30L69 44L61 66L39 66L31 44Z" fill="none" stroke="#b89e75" stroke-width=".7"/><polygon points="${axes.map(v=>`${v.x},${v.y}`).join(' ')}" fill="#bba9d6aa" stroke="#9682bb" stroke-width="1"/>${axes.map(v=>`<circle cx="${v.x}" cy="${v.y}" r="1.5" fill="#8772ad"/>`).join('')}</svg><dl>${axes.map(v=>`<div><dt>${v.label}</dt><dd>${Math.round(v.value*100)}</dd></div>`).join('')}</dl></div><small>生来の傾向 · 標準100</small></section>`;}
  ...
  function selectedDetail(model,row){
    if(!row)return `<article class="rb-detail rb-empty-detail">${art(model.page+'-hero','rb-detail-art')}<h2>これからの旅で</h2><p>まだ習得したものがありません。</p><p>暮らし、見学、稽古で得た経験が、この人生の心得や技になります。</p>${action('signs','兆しを確かめる',false,'rb-primary')}</article>`;
    ...
    let details='';
    if(model.page==='heart')details=`<h3>現在の意識バランス</h3>${mind(model)}`;
    if(model.page==='body')details=`<h3>身法の働き</h3><dl class="rb-facts">${row.facts.map(v=>`<div><dt>${esc(v.label)}</dt><dd>${esc(v.value)}</dd></div>`).join('')}</dl>`;
    ...
    return `<article class="rb-detail"><div class="rb-detail-picture">${art(model.page+'-hero','rb-detail-art')}<span class="rb-ribbon">${equipped?'設定中':esc(row.status)}</span></div><div class="rb-detail-copy"><h2>${esc(row.name)}</h2><p>${esc(row.purpose)}</p>${row.tradeoff?`<p class="rb-tradeoff">${esc(row.tradeoff)}</p>`:''}${details}<details class="rb-origin"><summary>この人生で生まれた理由</summary><p>${esc(row.story)}</p>${row.provenance.map(p=>`<p>${esc(p.text)}</p>`).join('')}</details></div><footer><p class="rb-lock" ${lock?'':'hidden'}>${esc(lock)}</p>${['heart','technique','items'].includes(model.page)?action('remove','はずす',Boolean(model.readonly),'rb-secondary'):''}${action('equip',esc(call)+' ›',Boolean(lock),'rb-primary')}</footer></article>`;
  }
  function render(){
    ...
    const row=model.allRows.find(r=>r.id===s.id),pages=Math.max(1,Math.ceil(model.rows.length/9));s.page=Math.min(s.page,pages-1);
    const subset=model.rows.slice(s.page*9,s.page*9+9),categories=model.page==='body'?BOOK_BODY.map(([k,v])=>[k,v]):model.page==='items'?[['weapon','武器'],['armor','防具'],['shield','盾']]:[];
  ...
  ui.open=(type,options={})=>{if(!views.includes(type))cleanup();previous.open(type,options);if(views.includes(type)){status='';if(options.skillId)selection[type].id=options.skillId;renderKey='';render();}};
  ui.close=()=>{cleanup();previous.close();};
  ui.bindState=next=>{state=next;previous.bindState(next);if(!next)return;const key=JSON.stringify([next.id,next.inspiration?.revision,next.knownSkills,next.combatLoadout,next.equipment,Math.floor(next.ageYears||0),Boolean(next.combat),next.down,next.ended]);if(key!==renderKey){renderKey=key;render();}else if(isCore()&&!ui.body.querySelector('.rb-page'))render();};
  ...
  ui.dispose=()=>{disposed=true;cleanup();previous.dispose();};
  return ui;
}
