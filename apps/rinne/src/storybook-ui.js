...
export function installStorybookUI(ui,{gameScreen,audio,requestEquip}){
  ...
  function selectedDetail(model,row){
    ...
    if(model.page==='body')details=`<h3>身法の働き</h3><dl class="rb-facts">${row.facts.map(v=>{const old=target.row?.facts?.find(f=>f.key===v.key);return `<div><dt>${esc(v.label)}</dt><dd>${old&&old.value!==v.value?`<s>${esc(old.value)}</s> → `:''}${esc(v.value)}</dd></div>`;}).join('')}</dl><small class="rb-basis-note">100%は基準値。0%は「できない」ではなく、この身法からの追加補正がない状態です。</small>`;
  ...
  function helpMarkup(model){return `<aside class="rb-help-dialog" role="dialog" aria-label="${model.glyph}の解説"><h2>${model.glyph}・${model.title}</h2><p>${model.hint}</p><p>心は「何を優先するか」、技は「何をするか」、体は「どう動くか」、装は「何を持つか」。上の枠を選び、候補を見比べ、確定ボタンで反映します。候補を見ただけでは変更されません。</p>${model.page==='technique'?`<div class="rb-extra">${action('add-combo','連を追加',model.combos.length>=MAX_COMBOS||Boolean(model.readonly))}${action('remove-combo','この連を外す',model.combos.length<2||Boolean(model.readonly))}${['jo','ha','kyu'].map(p=>action('favored-'+p,({jo:'序',ha:'破',kyu:'急'})[p]+'を得意技にする',Boolean(model.readonly))).join('')}<label>手動奥義<select data-book-one-motion ${model.readonly?'disabled':''}><option value="">使わない</option>${learnedTechniqueSkills(state,{oneMotion:true}).map(id=>`<option value="${esc(id)}" ${id===model.oneMotion?'selected':''}>${esc(techniqueName(id,state))}</option>`).join('')}</select></label></div>`:''}${action('help','閉じる',false,'rb-primary')}</aside>`;}
