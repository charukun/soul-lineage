...
export function installCoopMenu({container,buildInfo,getPrepared,getName,onPlay,onLeave,performanceProbeFactory=null}){
  ...
  async function host(resume){
    ...
    try{
      ...
      session=await createCoopHost({world,contentVersion,save:persist,RTCPeerConnection,onChange:changed,performanceProbe:makeProbe('host'),semanticShadowState,semanticShadowCoverage,semanticShadowPersistence});$('coop-invite').hidden=false;$('coop-cancel').hidden=false;changed();
  ...
  $('coop-join').onclick=run(async()=>{
    ...
    session=await joinCoopHost({invite,name:getName(),contentVersion,RTCPeerConnection,resume,onChange:changed,performanceProbe:makeProbe('peer'),remember:ticket=>{void storage().write(`coop-ticket:${invite.worldId}`,JSON.stringify(ticket)).catch(error=>status(`再接続の記録に失敗: ${error.message}`));}});
