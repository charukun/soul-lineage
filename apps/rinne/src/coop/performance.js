...
export function createCoopPerformanceProbe({role='peer',now=()=>performance.now(),maxSamples=4096}={}){
  ...
  const raw={inputToDisplayMs:[],canonCommitMs:[],hostLossDetectionMs:[],hostReopenMs:[],peerUplinkKbps:[],hostUplinkKbps:[],reliableBufferedAmountBytes:[],presenceBufferedAmountBytes:[],stateFreshnessMs:[],positionErrorM:[],rollbackMs:[],frameMs:[],gpuMs:[],memoryMb:[],batteryPctPerHour:[]};
  ...
  function flushTx(force=false){const at=now(),elapsed=at-txWindowStartedAt;if(elapsed<1000&&!force)return false;if(elapsed>0&&txWindowBytes>0)push(role==='host'?'hostUplinkKbps':'peerUplinkKbps',txWindowBytes*8/elapsed);txWindowStartedAt=at;txWindowBytes=0;return true;}
  function recordSend({payloadBytes=0,reliableBufferedAmount=null,presenceBufferedAmount=null}={}){flushTx(false);if(finite(Number(payloadBytes)))txWindowBytes+=Number(payloadBytes);push('reliableBufferedAmountBytes',Number(reliableBufferedAmount));push('presenceBufferedAmountBytes',Number(presenceBufferedAmount));}
  ...
  function inputAcknowledged(seq){
    seq=clampSeq(seq);if(seq==null)return false;const at=now(),start=inputStarted.get(seq);for(const pending of [...inputStarted.keys()])if(pending<=seq)inputStarted.delete(pending);if(start==null)return false;push('inputToDisplayMs',Math.max(0,at-start));return true;
  }
  ...
  async function connectionOpen(connection){
    ...
    try{const report=await pc.getStats();let selected=null;report.forEach(row=>{if(row.type==='transport'&&row.selectedCandidatePairId)selected=row.selectedCandidatePairId;});let pair=selected?report.get(selected):null;if(!pair)report.forEach(row=>{if(!pair&&row.type==='candidate-pair'&&row.nominated&&row.state==='succeeded')pair=row;});const local=pair?.localCandidateId?report.get(pair.localCandidateId):null,remote=pair?.remoteCandidateId?report.get(pair.remoteCandidateId):null;if(local?.candidateType==='relay'||remote?.candidateType==='relay')turnRelayConnections++;}catch{/* stats support is optional */}
    return true;
  }
  ...
  function snapshot({flush=false}={}){if(flush)flushTx(true);else flushTx(false);return structuredClone({...raw,durationMinutes:Math.max(0,(now()-startedAt)/60000),connectionAttempts,connectionSuccesses,connectedPeers:connectionSuccesses,turnRelayConnections,pendingInputs:inputStarted.size,pendingCanon:canonStarted.size});}
