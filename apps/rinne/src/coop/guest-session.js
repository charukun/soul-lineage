...
export async function joinCoopHost({invite,name,contentVersion,RTCPeerConnection,resume=null,remember=()=>{},onChange=()=>{},now=()=>performance.now(),performanceProbe=null}){
  ...
  const probe=performanceProbe||createCoopPerformanceProbe({role:'peer',now});
  ...
  const wire=createRoomWire(message=>{
    ...
    if(message?.type==='rebirth-result'&&rebirthRequest?.lifeId===message.lifeId){
      if(message.error)failRequest(String(message.error));else{probe.canonCommitted(message.lifeId);rebirthRequest.resolve();rebirthRequest=null;}return;
    }
    ...
    if(view.ackInputSeq!=null&&(!Number.isSafeInteger(view.ackInputSeq)||view.ackInputSeq<0))return;
  ...
  connection=await acceptHostOffer(invite.offer,{RTCPeerConnection,dualChannel:true,onMessage:m=>wire.receive(m),onState:state=>{
    if(state==='open'){void probe.connectionOpen(connection);send({type:'hello',name:String(name||'旅人').slice(0,12),contentVersion,resume});}
  ...
  function rebirth(villageId){
    ...
    const lifeId=latest.me.id;rebirthRequest={lifeId,promise,resolve,reject};probe.canonIntent(lifeId);
  ...
  return{role:'guest',worldId:invite.worldId,get selfId(){return selfId;},get layout(){return layout;},answerCode:connection.code,
    ...
    snapshot:()=>({phase,error,view:latest}),performance:()=>probe.snapshot({flush:true}),dispose:()=>{if(disposed)return;disposed=true;phase='closed';clearInterval(timer);failRequest('村を離れました。');connection.close();}};
}