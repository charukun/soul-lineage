...
export function installRrpPerformanceCapture({getSession,buildInfo=null,now=()=>performance.now(),intervalMs=250,expectedPeers=null,setIntervalFn=setInterval,clearIntervalFn=clearInterval,windowRef=globalThis.window,documentRef=globalThis.document}={}){
  ...
  let segments=[],pendingBootstrapPayload=null,variantBootstrapped=false,lastProvisionalAt=null,variantSequence=0;const measurementKeys=new Set();
  ...
  function semanticMeasurement(sample){
    if(!sample||typeof sample!=='object')return false;if(sample.warmStart){pendingBootstrapPayload=safeClone(sample.checkpointPayload);if(measurementStarted)runBootstrap();return true;}if(!measurementStarted||variant==='shadow-only')return false;runBootstrap();
    const events=Number(sample.eventCount||0),protectedCommit=Number.isSafeInteger(events)&&events>0;
  ...
  function performanceProbeFactory(role){
    if(activeProbe&&role!==activeRole){segments=[];measurementStarted=false;measurementStartedAt=null;outageStartedAt=null;pendingBootstrapPayload=null;variantBootstrapped=false;lastProvisionalAt=null;variantSequence=0;for(const key of Object.keys(variantRaw))variantRaw[key].length=0;clearMeasurementStorage();}
    else stashProbe();
  ...
  function resetMeasurement(){activeProbe?.resetWindow({keepConnections:true});segments=[];measurementStarted=false;measurementStartedAt=null;outageStartedAt=null;windowArmed=false;lastRaw=null;lastMeta=null;lastDiagnostics=null;variantBootstrapped=false;lastProvisionalAt=null;variantSequence=0;for(const key of Object.keys(variantRaw))variantRaw[key].length=0;clearMeasurementStorage();}
  function armWindow(session){
    if(!activeProbe||windowArmed||!readyToArm(session))return false;
    if(!measurementStarted){activeProbe.resetWindow({keepConnections:true});segments=[];measurementStarted=true;measurementStartedAt=new Date().toISOString();runBootstrap();}
    windowArmed=true;
