...
const known=state=>new Set(Array.isArray(state?.knownSkills)?state.knownSkills:[]);
const isLearned=(state,id)=>id===basicSkill(state)||known(state).has(id);
const isAction=(state,id)=>isLearned(state,id)&&(id===basicSkill(state)||SKILL_BY_ID[id]?.type==='action');
const supportIds=state=>SUPPORT_SKILLS.map(row=>row.id).filter(id=>known(state).has(id));
const actionIds=state=>[basicSkill(state),...ACTION_SKILLS.map(row=>row.id).filter(id=>known(state).has(id))];
...
function optionUnlocked(state,option){const requires=option?.requiresAny||[];return requires.length===0||requires.some(id=>known(state).has(id));}
...
export function ensureCombatLoadout(state){
  if(!state)return null;const existing=state.combatLoadout&&typeof state.combatLoadout==='object'?state.combatLoadout:{},heart=existing.heart&&typeof existing.heart==='object'?existing.heart:{},availableHeart=supportIds(state);
  ...
  if(!isAction(state,technique.oneMotion)||SKILL_BY_ID[technique.oneMotion]?.type!=='action')technique.oneMotion=null;
...
export function setOneMotion(state,skill){const loadout=ensureCombatLoadout(state);if(skill===null){loadout.technique.oneMotion=null;return true;}if(SKILL_BY_ID[skill]?.type!=='action'||!isLearned(state,skill))return false;loadout.technique.oneMotion=skill;return true;}
...
export function requestOneMotion(state){const loadout=ensureCombatLoadout(state),skill=loadout.technique.oneMotion;if(!state?.combat||state.down||state.ended||!skill||!isLearned(state,skill))return null;state.combat.oneMotionQueued={skill,ttl:1.15};return skill;}
