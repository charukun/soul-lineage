...
export function combatSkillForPhase(state,combat,phase){const combo=comboById(state,combat?.comboId)||activeCombo(state);return isAction(state,combo?.slots?.[phase])&&weaponCompatible(state,combo?.slots?.[phase])?combo.slots[phase]:basicSkill(state);}
