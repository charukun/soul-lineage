import { BASE_APPEARANCE_PARTS, VISUAL_ROLES, CHARACTER_REFERENCE_ARCHETYPES, characterReferenceArchetype,
  visualIdentityForCharacter, visualIdentityForReferenceArchetype, compareVisualIdentities } from '@soul/characters';
const defaults = { version: 1, mode: 'enhanced', context: 'village', role: 'mixed', archetype: 'mixed', reference: true, marked: [] };
...
export function qualitySettings(input = {}, ids = null) {
  ...
  check(value.archetype === 'mixed' || Object.hasOwn(CHARACTER_REFERENCE_ARCHETYPES,value.archetype), 'Invalid reference archetype');
  ...
  return { ...value, reference: value.archetype === 'mixed' ? value.reference : false, marked: [...value.marked] };
}
export function qualityIdentity(record, index, input, manual = null) {
  ...
  if (quality.archetype !== 'mixed') return visualIdentityForReferenceArchetype(record, quality.archetype, manual);
...
export function qualityReport(records, input, profiles = new Map(), indices = null) {
  ...
  return { ...compareVisualIdentities(rows), referenceCount: records.length - rows.length, mode:quality.mode,
    archetype: quality.archetype, target: target ? { id:target.id,label:target.label,age:target.age,role:target.role,assetFile:target.assetFile,coverage:target.coverage } : null };
}
