...
function renderHeart(model,focusId=null){
  ...
  model.ui.body.append(sectionIntro('会得した心得','身につけた心得を確認し、戦いへ持ち込むものを選ぶ。'));
  const ids=learnedHeartSkills(state),list=document.createElement('div');list.className='heart-list';
  if(!ids.length){list.innerHTML='<p class="loadout-empty">まだ心得を会得していない。</p>';model.ui.body.append(list);return;}
...
function renderFavored(model,state,combo,phase){
  const button=document.createElement('button');button.type='button';button.className='favored-tag';button.dataset.active=String(Boolean(combo.favored?.[phase]));button.textContent=combo.favored?.[phase]?'得意技':'得意にする';button.onclick=()=>{toggleFavored(state,combo.id,phase);model.audio.ui();haptic(8);renderTechnique(model);};return button;
}
...
function renderFocusedSpark(model,state,combo){
  const id=model.focusSkill;if(!id||SKILL_BY_ID[id]?.type!=='action')return null;
  const box=document.createElement('div');box.className='technique-focus-banner';box.innerHTML='<span>新しい技</span><strong></strong><div></div>';box.querySelector('strong').textContent=techniqueName(id);const actions=box.querySelector('div');
  for(const [phase,label] of PHASES){const button=document.createElement('button');button.type='button';button.textContent=`${label}へ`;button.onclick=()=>{setComboSkill(state,combo.id,phase,id);model.focusSkill=null;model.audio.item();haptic(18);renderTechnique(model);};actions.append(button);}return box;
}
function renderPicker(model,state){
  if(!model.picker)return null;const one=model.picker.kind==='oneMotion',box=document.createElement('section');box.className='technique-picker';box.append(sectionIntro(one?'ワンモーションを選ぶ':'技を選ぶ',one?'手動発動は強い代わりに消耗と隙が大きい。':'選んだ技をこの序破急スロットへ刻む。'));
...
function renderTechnique(model,focusId=null){
  ...
  model.ui.title.textContent='技 · 序破急';model.ui.body.innerHTML='';model.ui.body.append(sectionIntro('連技を組む','序 → 破 → 急を1連のコンボとして自動戦闘へ持ち込む。'),renderComboTabs(model,state));
...
function renderBody(model){
  ...
  model.ui.body.append(renderBodyGroup(model,state,'stance','構えモーション','戦闘態勢で取る身体の形。'),renderBodyGroup(model,state,'style','心構え / 戦闘スタイル','間合い・接近・離脱・回り込みの癖を決める。'),renderBodyGroup(model,state,'zanshin','残心モーション','一撃のあと、どう次へつなぐかを決める。'));
}
