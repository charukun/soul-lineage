...
function renderCombatTabs(model,active){
  const wrap=document.createElement('nav');wrap.className='combat-plan-tabs';wrap.setAttribute('aria-label','戦技の分類');
  for(const [id,label] of [['heart','心'],['technique','技'],['body','体']]){const button=document.createElement('button');button.type='button';button.dataset.active=String(id===active);button.textContent=label;button.onclick=()=>{model.section=id;model.picker=null;model.audio.ui();if(id==='heart')renderHeart(model);else if(id==='technique')renderTechnique(model);else renderBody(model);};wrap.append(button);}return wrap;
}
function pager(model,key,total,rerender){
  const pages=Math.max(1,Math.ceil(total/PAGE_SIZE)),current=Math.min(Math.max(0,model.pages[key]||0),pages-1);model.pages[key]=current;
  if(pages<=1)return null;
  const nav=document.createElement('nav');nav.className='panel-pager';const prev=document.createElement('button'),meta=document.createElement('span'),next=document.createElement('button');prev.type=next.type='button';prev.textContent='‹';next.textContent='›';prev.disabled=current<=0;next.disabled=current>=pages-1;meta.textContent=`${current+1} / ${pages}`;prev.onclick=()=>{model.pages[key]=current-1;model.audio.ui();rerender();};next.onclick=()=>{model.pages[key]=current+1;model.audio.ui();rerender();};nav.append(prev,meta,next);return nav;
}
function pageRows(model,key,rows,rerender){
  const pages=Math.max(1,Math.ceil(rows.length/PAGE_SIZE)),current=Math.min(Math.max(0,model.pages[key]||0),pages-1);model.pages[key]=current;return{rows:rows.slice(current*PAGE_SIZE,current*PAGE_SIZE+PAGE_SIZE),pager:pager(model,key,rows.length,rerender)};
}

function renderHeart(model,focusId=null){
  const state=model.getState();if(!state)return;ensureCombatLoadout(state);model.section='heart';model.tracker.consume('heart');model.ui.title.textContent='戦技 · 心';model.ui.body.innerHTML='';
  const ids=learnedHeartSkills(state),active=new Set(state.combatLoadout.heart.active);if(focusId){const index=ids.indexOf(focusId);if(index>=0)model.pages.heart=Math.floor(index/PAGE_SIZE);}
  model.ui.body.append(renderCombatTabs(model,'heart'),sectionIntro('心得を選ぶ','この人生で使える心得から、今の戦いへ持ち込む判断傾向を選ぶ。'));
  const list=document.createElement('div');list.className='heart-list';const page=pageRows(model,'heart',ids,()=>renderHeart(model,focusId));
  for(const id of page.rows)list.append(skillButton(id,{active:active.has(id),highlight:id===focusId,onClick:()=>{setHeartActive(state,id,!new Set(state.combatLoadout.heart.active).has(id));model.audio.item();haptic(10);renderHeart(model,id);}}));
  if(!page.rows.length){const empty=document.createElement('p');empty.className='loadout-empty';empty.textContent='現在使える心得はありません。';list.append(empty);}model.ui.body.append(list);if(page.pager)model.ui.body.append(page.pager);
}
...
function renderPicker(model,state){
  ...
  const ids=learnedTechniqueSkills(state,{oneMotion:one}),list=document.createElement('div');list.className='technique-picker-list',page=pageRows(model,'picker',ids,()=>renderTechnique(model));
  for(const id of page.rows){const button=skillButton(id,{active:one?state.combatLoadout.technique.oneMotion===id:comboById(state,model.picker.comboId)?.slots?.[model.picker.phase]===id,onClick:()=>{if(one)setOneMotion(state,id);else setComboSkill(state,model.picker.comboId,model.picker.phase,id);model.picker=null;model.pages.picker=0;model.audio.item();haptic(16);renderTechnique(model);}});list.append(button);}box.append(list);if(page.pager)box.append(page.pager);const cancel=document.createElement('button');cancel.type='button';cancel.className='picker-cancel';cancel.textContent='戻る';cancel.onclick=()=>{model.picker=null;model.audio.ui();renderTechnique(model);};box.append(cancel);return box;
}
function renderOneMotion(model,state){
  const box=document.createElement('section');box.className='one-motion-card';const current=state.combatLoadout.technique.oneMotion;box.innerHTML='<div><span>奥義</span><strong></strong><small>戦闘態勢中だけ手動発動 · 大消耗 / 大きな隙</small></div><div class="one-motion-actions"></div>';box.querySelector('strong').textContent=current?techniqueName(current):'未設定';const actions=box.querySelector('.one-motion-actions');
  const choose=document.createElement('button');choose.type='button';choose.textContent=current?'変更':'セット';choose.onclick=()=>{model.picker={kind:'oneMotion'};model.pages.picker=0;model.audio.ui();renderTechnique(model);};actions.append(choose);
...
function renderTechnique(model,focusId=null){
  ...
  model.ui.title.textContent='戦技 · 技';model.ui.body.innerHTML='';model.ui.body.append(renderCombatTabs(model,'technique'));
  const picker=renderPicker(model,state);if(picker){model.ui.body.append(picker);return;}
  model.ui.body.append(sectionIntro('連技を組む','序 → 破 → 急の3手を組み、戦闘中の自動連技を決める。'),renderComboTabs(model,state));
...
function renderBodyTabs(model){const wrap=document.createElement('nav');wrap.className='body-kind-tabs';for(const [id,label] of [['stance','構え'],['style','戦法'],['zanshin','残心']]){const button=document.createElement('button');button.type='button';button.dataset.active=String(model.bodyKind===id);button.textContent=label;button.onclick=()=>{model.bodyKind=id;model.audio.ui();renderBody(model);};wrap.append(button);}return wrap;}
...
function renderBody(model){
  const state=model.getState();if(!state)return;ensureCombatLoadout(state);model.section='body';model.bodyKind=model.bodyKind||'stance';model.ui.title.textContent='戦技 · 体';model.ui.body.innerHTML='';model.ui.body.append(renderCombatTabs(model,'body'),renderBodyTabs(model));
  if(model.bodyKind==='stance')model.ui.body.append(renderBodyGroup(model,state,'stance','構えモーション','戦闘態勢で取る身体の形。'));
  else if(model.bodyKind==='style')model.ui.body.append(renderBodyGroup(model,state,'style','心構え / 戦闘スタイル','間合い・接近・離脱・回り込みの方針を決める。'));
  else model.ui.body.append(renderBodyGroup(model,state,'zanshin','残心モーション','一撃のあと、どう次へつなぐかを決める。'));
}
...
export function createHeartTechniqueBodyUI({ui,audio,getState,tracker}){
  const model={ui,audio,getState,tracker,comboId:null,picker:null,focusSkill:null,section:'technique',bodyKind:'stance',pages:{heart:0,picker:0}};
  ...
  return{renderHeart:focus=>renderHeart(model,focus),renderTechnique:focus=>renderTechnique(model,focus),renderBody:()=>renderBody(model),syncCombat:state=>syncCombat(model,state),reset(){model.comboId=null;model.picker=null;model.focusSkill=null;model.section='technique';model.bodyKind='stance';model.pages={heart:0,picker:0};},dispose(){}};
}
