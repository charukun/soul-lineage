...
const topSlotRow=swapMode=>createRinneLoadoutSlotRow({swapMode});
...
function renderHeart(model,focusId=null){
  ...
  const ids=learnedHeartSkills(state),active=state.combatLoadout.heart.active;model.heartTarget=Math.max(0,Math.min(2,model.heartTarget||0));if(focusId)model.heartPreview=focusId;if(!model.heartPreview)model.heartPreview=active[model.heartTarget]||ids[0]||null;
  const swapActive=model.swap?.section==='heart',slots=topSlotRow(swapActive);
  for(let index=0;index<3;index++){const id=active[index]||null;slots.append(slot('心得 '+(index+1),id?techniqueName(id):'空き',{selected:model.heartTarget===index,empty:!id,icon:rinneSkillSigilKind(id,SKILL_BY_ID[id]?.effects),swapMode:swapActive,swapSource:swapActive&&model.swap.source===index,detail:id?skillDetail(id,'心得','意識中'):null,onClick:()=>{if(swapActive){const source=model.swap.source,sourceId=active[source];if(source!==index&&sourceId)setHeartSlot(state,index,sourceId);clearSwap(model);model.heartTarget=index;model.audio.item();haptic(18);renderHeart(model,model.heartPreview);return;}model.heartTarget=index;model.heartPreview=id||model.heartPreview;model.audio.ui();renderHeart(model,model.heartPreview);},onLongPress:id?()=>{model.swap={section:'heart',source:index};model.audio.ui();haptic([18]);renderHeart(model,model.heartPreview);}:null}));}
  model.ui.body.append(slots);if(swapActive)model.ui.body.append(createRinneSwapHint('入れ替える心得枠を選択'));
  const preview=model.heartPreview;if(preview){const current=active[model.heartTarget]===preview,row=skillDetail(preview,'心得',active.includes(preview)?'意識中':'習得済み');model.ui.body.append(detailSection({icon:rinneSkillSigilKind(preview,SKILL_BY_ID[preview]?.effects),...row,actionLabel:current?'設定済み':`心得${model.heartTarget+1}にセット`,actionDisabled:current,onAction:()=>{setHeartSlot(state,model.heartTarget,preview);model.audio.item();haptic(12);renderHeart(model,preview);}}));}
  ...
  for(const id of page.rows){const item=gridItem(techniqueName(id),effectSummary(id),{active:active[model.heartTarget]===id,focus:id===preview,icon:rinneSkillSigilKind(id,SKILL_BY_ID[id]?.effects),detail:skillDetail(id,'心得',active.includes(id)?'意識中':'習得済み'),onClick:()=>{model.heartPreview=id;model.audio.ui();renderHeart(model,id);}});item.setAttribute('aria-label',techniqueName(id)+'。詳細を表示');list.append(item);}
...
function renderComboList(model,state){
  ...
  const rows=state.combatLoadout.technique.combos,selection=state.combatLoadout.technique.phaseSelections?.[model.techniqueTarget],library=gridSection('連技一覧','候補を選ぶと詳細を表示'),list=library.querySelector('.loadout-grid');
...
function renderBody(model){
  const state=model.getState();if(!state)return;ensureCombatLoadout(state);model.section='body';model.bodyKind=model.bodyKind||'stance';model.ui.title.textContent='体 · 身法';model.ui.body.innerHTML='';model.ui.body.append(createRinneMenuLead('いまの身体に合う型を選ぶ'));const kinds=[['stance','構え','戦闘態勢の形'],['style','戦法','間合いと動き'],['zanshin','残心','攻撃後の戻り']],slots=topSlotRow(false);
  for(const [kind,label,meta] of kinds){const option=unlockedBodyOptions(state,kind).find(row=>row.id===state.combatLoadout.body[kind]);slots.append(slot(label,option?.label||'未設定',{selected:model.bodyKind===kind,icon:'stance',detail:{kicker:`${label}の装着枠`,title:option?.label||'未設定',summary:option?.description||meta,status:model.bodyKind===kind?'選択先':'装着中'},onClick:()=>{model.bodyKind=kind;model.bodyPreview=state.combatLoadout.body[kind]||null;model.pages.body=0;model.audio.ui();renderBody(model);}}));}model.ui.body.append(slots);
