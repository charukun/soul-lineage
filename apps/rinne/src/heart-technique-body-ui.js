...
function techniqueDetail(state,selection,status='習得済み'){
  ...
  return{...shared,note:'この人生で身につけた技',icon:shared.sigil||rinneSkillSigilKind(selection,SKILL_BY_ID[selection]?.effects)};
}
const techniqueIcon=(state,id)=>sharedTechniqueDetail(id,{weapon:state?.equipment?.weapon||'sword',name:techniqueName(id,state)}).sigil||rinneSkillSigilKind(id,SKILL_BY_ID[id]?.effects);
