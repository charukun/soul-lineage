...
function resetActor(actor){actor.x=actor.spawnX;actor.z=actor.spawnZ;actor.hp=actor.maxHp;actor.dead=false;actor.respawn=0;actor.cooldown=.55;if(Object.hasOwn(actor,'playerCooldown'))actor.playerCooldown=.35;actor.flash=0;actor.moving=false;}
...
function tickRespawns(skirmish,dt){
  for(const row of [...skirmish.guards,...skirmish.hostiles]){row.flash=Math.max(0,row.flash-dt*4);if(!row.dead){row.cooldown-=dt;if(Number.isFinite(row.playerCooldown))row.playerCooldown-=dt;continue;}row.respawn-=dt;if(row.respawn<=0)resetActor(row);}
}
...
export function villageFatalityChance(state){
  const effects=skillEffects(state),armor=ARMORS[state.equipment.armor]||ARMORS.cloth,shield=state.equipment.shield?.12:0;
  const survival=armor.guard+shield+effects.mitigation+effects.evasion*.4+effects.recovery*.25;
  return clamp(.78-survival*.9,.16,.78);
}
...
function tickPlayer(state,skirmish,dt,events){
  ...
  for(const hostile of threats){
    if(hostile.playerCooldown>0||state.hp<=0)continue;const roll=hash01(`${state.seed}:${hostile.id}:evade:${Math.floor(state.ageSeconds*4)}:${state.defeats}`);hostile.playerCooldown=.68+hash01(`${hostile.id}:player-cooldown`)*.28;
    ...
    if(state.hp<=0){
      const fatalChance=villageFatalityChance(state),fatalRoll=hash01(`${state.seed}:${hostile.id}:fatal:${Math.floor(state.ageSeconds)}:${state.defeats}`);
      if(fatalRoll<fatalChance){endLifeEarly(state,`${hostile.label}との戦い`);events.push({type:'life-end',cause:'combat',fatalChance});}
      else{state.down={elapsed:0,rescueSeconds:12,village:true};state.combat=null;events.push({type:'downed',fatalChance});}
      break;
    }
  }
}
