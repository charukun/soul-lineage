export function createCombatProjectilePresentation({THREE,frontRoot,mat}){
  ...
  function make(id){const mesh=new THREE.Mesh(new THREE.SphereGeometry(.075,8,6),mat(0xa9e8f0,{emissive:0x4b9dab,emissiveIntensity:1.1,roughness:.35,metalness:.05}));mesh.name=`CombatProjectile:${id}`;root.add(mesh);meshes.set(id,mesh);return mesh;}
