import {AFFINITY_EFFECT_KEYS} from './authored-effect-manifest.js';
...
const affinityEffect=(state,event)=>AFFINITY_EFFECT_KEYS[state?.inspiration?.records?.[event?.techniqueId]?.effectAffinity]||null;
...
export function combatEffectCues(events, {state, front, hostiles=[], anchors={}}={}) {
  ...
  for(const event of events){
    ...
    const heavy=outgoing&&(manual||event.manual===true||event.phase==='one'||event.phase==='kyu'),affinity=outgoing?affinityEffect(state,event):null;
    const rotation={x:0,y:Math.atan2(to.x-from.x,to.z-from.z),z:0};
    const color=affinity?[255,174,92,255]:(outgoing?[255,236,196,255]:[255,126,96,255]);
    // Presentation affinity replaces the generic contact burst only; it never changes hit authority or damage.
    cues.push({effect:affinity||(heavy?'finisher':'impact'),position:{...contact},rotation,scale:affinity?(heavy?1.08:.92):(heavy?1.15:1),
      lifetime:affinity?(heavy?1.55:1.25):(heavy?1.8:1.2),color,priority:heavy?3:2,kind:affinity?'affinity-contact':(heavy?'finisher':'contact')});
