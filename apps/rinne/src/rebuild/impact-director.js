...
export function createImpactDirector({mobile=false,reducedMotion=false}={}){
  ...
  function applyPoseLag(state,front,realDt){const scale=timeScale(),actors=[{key:'hero',holder:state.combat,field:'tidebreakPose'},...(front?.enemies||[]).map(enemy=>({key:`enemy:${enemy.id}`,holder:enemy,field:'tidebreakPose'}))],restore=[];for(const row of actors){if(!row.holder)continue;const current=row.holder[row.field];if(!current){poseDisplay.delete(row.key);continue;}const previous=poseDisplay.get(row.key),alpha=clamp(realDt*(scale<.99?8+18*scale:45),0,1),display=blendFrame(previous,current,alpha,scale<.99);poseDisplay.set(row.key,clone(display));if(scale<.995){restore.push([row.holder,row.field,current]);row.holder[row.field]=display;}}return()=>{for(const [holder,field,value] of restore)holder[field]=value;};}
  function frame(realDt,{level=qualityLevel,reduced:nextReduced=reduced,hidden:isHidden=hidden}={}){setMode({level,reduced:nextReduced,isHidden});realDt=Math.max(0,Math.min(.08,Number(realDt)||0));if(stop>0)stop=Math.max(0,stop-realDt);else if(slow>0)slow=Math.max(0,slow-realDt);if(slow<=0){slowScale=1;slowDuration=0;}camera.strength*=Math.exp(-realDt*16);camera.fov*=Math.exp(-realDt*13);for(const row of reactions)row.remaining=Math.max(0,row.remaining-realDt);reactions=reactions.filter(row=>row.remaining>0);return snapshot();}
