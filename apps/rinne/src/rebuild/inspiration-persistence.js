...
export function ensureInspiration(state,{fresh=false}={}){
  ...
  if(state.inspiration){if(state.inspiration.version!==INSPIRATION_VERSION)throw Error('閃きの保存形式が違います。');state.inspiration.talents??=[];state.inspiration.talentDetails??={};return state.inspiration;}
  ...
  state.inspiration={version:INSPIRATION_VERSION,revision:0,clock:0,serial:0,lastNamed:-INSPIRATION_LIMITS.namedGap,body,heritage:inheritedFrom(state),traces:[],seen:[],questions:{},records:{},legacySkills:fresh?[]:selectedLegacy(state),talents:[],talentDetails:{},pending:null,execution:null,sequence:[],signs:[],lastLifeId:state.id||''};
...
export function validateInspiration(state){
  ...
  s.talents=unique(s.talents.map(id=>id==='gifted'?'tenyo':id==='prodigy'?'sui':id).filter(id=>['tenyo','sui'].includes(id)));if(!s.talentDetails||Array.isArray(s.talentDetails)||typeof s.talentDetails!=='object')s.talentDetails={};
  if(s.talentDetails.gifted&&!s.talentDetails.tenyo)s.talentDetails.tenyo={...s.talentDetails.gifted,id:'tenyo',label:'天与'};delete s.talentDetails.gifted;
  if(s.talentDetails.prodigy&&!s.talentDetails.sui)s.talentDetails.sui={...s.talentDetails.prodigy,id:'sui',label:'彗',origin:'legacy-prodigy'};delete s.talentDetails.prodigy;
  ...
  for(const [id,row] of Object.entries(s.records)){
    ...
    row.specialEffects=Array.isArray(row.specialEffects)?row.specialEffects.slice(0,4).filter(effect=>effect&&['cosmetic','status','rule'].includes(effect.impact)).map(effect=>({id:safe(effect.id),label:safe(effect.label),impact:effect.impact,rarity:['common','rare','singular'].includes(effect.rarity)?effect.rarity:'common',unlockCondition:safe(effect.unlockCondition)})):[];
    if(isGeneratedTechniqueId(id)){const naming=generatedTechniqueNaming(definition,{seed:state.seed??0,motifs:unique([...(definition.motifs||[]),...s.heritage.map(h=>h.motif)]),specialEffects:row.specialEffects});row.name=naming.displayName;row.naming={style:naming.style,grade:naming.grade,baseName:naming.baseName,signature:naming.signature};}
  ...
  s.seen=unique(s.seen.map(safe));s.legacySkills=unique(s.legacySkills.filter(id=>/^(skill|action)\.[a-z-]+$/.test(id)));for(const id of s.talents){const detail=s.talentDetails[id]||{};s.talentDetails[id]=Object.fromEntries(Object.entries(detail).slice(0,12).map(([k,v])=>[safe(k),typeof v==='number'?v:safe(v)]));}
...
export function inspirationImprint(state){
  ...
  return {version:1,body:{...s.body},talents:[...s.talents],motifs:[...weights].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0])).slice(0,5).map(([id,strength])=>({id,strength})),heritage:s.heritage.map(row=>({...row})),techniques:Object.values(s.records).map(r=>({id:r.answerId,name:r.name||inspirationTechniqueName(answer(r.answerId)),family:r.family,kind:r.kind,age:r.age,stable:r.stable,origin:r.provenance.slice(0,2).map(p=>p.text)}))};
}
