import { CAUSAL_ANSWER_BY_ID, INSPIRATION_QUESTIONS, inspirationTechniqueName, isEffectAffinity } from '@soul/game-data';
...
export function validateInspiration(state){
  ...
  for(const [id,row] of Object.entries(s.records)){
    ...
    row.family=definition.family;row.kind=definition.kind;row.motifs=[...definition.motifs];row.archived=Boolean(row.archived);row.stable=Boolean(row.stable);if(row.effectAffinity!=null&&!isEffectAffinity(row.effectAffinity))delete row.effectAffinity;finiteField(row.age,0,100,'会得年齢');
