...
function applyEnemyCollateral(states,front,result){
  const byId=new Map(states.map(state=>[state.id,state])),seen=new Set();for(const [primaryId,rows]of result){for(const event of [...rows]){if(event.type!=='enemy-hit'||event.multiTarget||!(event.damage>0))continue;const attacker=sourceEnemy(front,event.sourceId),key=`${primaryId}:${event.sourceId}`;if(!attacker||seen.has(key))continue;seen.add(key);for(const victim of enemySweepTargets(attacker,states,primaryId,front)){const defense=directionalDefenseFor(victim,attacker),damage=Math.min(victim.hp,event.damage*.72*defense.damageScale);if(!(damage>0))continue;victim.hp=Math.max(0,victim.hp-damage);const targetRows=result.get(victim.id)||[];targetRows.push({type:'enemy-hit',sourceId:attacker.id,damage,sector:defense.sector,awareness:defense.awareness,multiTarget:true,engine:'world-contact'});result.set(victim.id,targetRows);resolveCollateralLethal(victim,attacker,front,targetRows);}}}return result;
}
