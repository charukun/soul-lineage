import { WEAPONS, endLifeEarly } from './domain.js';
...
function resolveCollateralLethal(state,attacker,front,events){
  if(state.hp>0||state.down||state.ended)return;const fatalChance=frontierFatalityChance(state),fatalRoll=hash01(`${state.seed}:${attacker.id}:sweep-fatal:${front.stage}:${Math.floor(state.ageSeconds)}:${state.defeats}`);
  if(fatalRoll<fatalChance){endLifeEarly(state,`第${front.stage+1}前線の巻き込み`);events.push({type:'life-end',cause:'combat',fatalChance,sourceId:attacker.id,multiTarget:true,engine:'world-contact'});}else{state.down={elapsed:0,rescueSeconds:40,frontier:true};state.combat=null;events.push({type:'downed',fatalChance,sourceId:attacker.id,multiTarget:true,engine:'world-contact'});}
}
function applyEnemyCollateral(states,front,result){
  const seen=new Set();for(const [primaryId,rows]of result){for(const event of [...rows]){if(event.type!=='enemy-hit'||event.multiTarget||!(event.damage>0))continue;const attacker=sourceEnemy(front,event.sourceId),key=`${primaryId}:${event.sourceId}`;if(!attacker||seen.has(key))continue;seen.add(key);for(const victim of enemySweepTargets(attacker,states,primaryId,front)){const defense=directionalDefenseFor(victim,attacker),damage=Math.min(victim.hp,event.damage*.72*defense.damageScale);if(!(damage>0))continue;victim.hp=Math.max(0,victim.hp-damage);const targetRows=result.get(victim.id)||[];targetRows.push({type:'enemy-hit',sourceId:attacker.id,damage,sector:defense.sector,awareness:defense.awareness,multiTarget:true,engine:'world-contact'});result.set(victim.id,targetRows);resolveCollateralLethal(victim,attacker,front,targetRows);}}}return result;
}
...
function processCombatEvents(state,front,events){
  ...
  for(const event of original){
    if(event.type==='player-hit'&&event.damage>0){
      ...
      noteTechniqueUse(state,event.skill,{phase:event.phase,hit:event.damage>0});if(target){const response=recordEnemyPattern(target,state.id,event.skill);if(response==='counter')skillEvents(recordCombatLesson(state,'repeatedPattern',{amount:.35,cooldown:4}),events);}if(event.damage>0){const extras=applyMultiTargetContact(state,front,event,events);for(const extra of extras){const secondary=targetEnemy(front,extra.targetId);if(secondary)recordEnemyPattern(secondary,state.id,extra.skill);}}
    }
