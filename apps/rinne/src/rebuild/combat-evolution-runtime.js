...
import { applyMultiTargetContact,ensureCombatTerrain,terrainMovementScale,tickRangedProjectiles } from './combat-world-contact.js';
...
function applyMovementConsequences(state,front,before,effects){
  const intended={x:state.position.x,z:state.position.z},terrain=terrainMovementScale(front,before,intended,.3),scale=terrain*effects.movementScale;state.position.x=before.x+(intended.x-before.x)*scale;state.position.z=before.z+(intended.z-before.z)*scale;if(state.combat)state.combat.injuryEffects=effects;
}
...
function processCombatEvents(state,front,events){
  ...
  for(const event of original){
    if(event.type==='player-hit'&&event.damage>0){
      const target=targetEnemy(front,event.targetId);if(!event.projectile&&event.engine!=='world-contact'&&effects.attackScale<.999&&target){const restore=event.damage*(1-effects.attackScale);target.hp=Math.min(target.maxHp,target.hp+restore);event.damage*=effects.attackScale;}
      noteTechniqueUse(state,event.skill,{phase:event.phase,hit:true});if(target)recordEnemyPattern(target,state.id,event.skill);applyMultiTargetContact(state,front,event,events);
    }
...
export function tickEvolvedFront(state,front,dt,options={}){
  ensureCombatTerrain(front);const effects=prepareState(state),before={x:state.position.x,z:state.position.z},enemyBefore=positions(front),events=[];assignSquadRoles(front,[state]);tickRangedProjectiles(state,front,dt,events);
  events.push(...tickCoreFront(state,front,dt,options));processCombatEvents(state,front,events);applyMovementConsequences(state,front,before,effects);applySquadTactics(front,[state],dt*.55);applyEnemyTerrain(front,enemyBefore);recordCombatReplay(state,front,dt,events,{force:events.length>0});finalizeIfNeeded(state,front);return events;
}
