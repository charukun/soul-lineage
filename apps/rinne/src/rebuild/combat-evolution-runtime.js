...
function markExtraDown(state,target,events){if(target.dead||target.downed||target.hp>.001)return;target.hp=0;target.downed=true;target.downedElapsed=0;target.moving=false;target.attacking=false;events.push({type:'enemy-downed',targetId:target.id,engine:'combat-position'});}
function undoCoreDown(state,target,events){if(!target?.downed||!(target.hp>0))return;target.downed=false;target.downedElapsed=0;target.moving=false;target.attacking=false;for(let i=events.length-1;i>=0;i--)if(events[i].type==='enemy-downed'&&events[i].targetId===target.id)events.splice(i,1);}
