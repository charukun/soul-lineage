...
import { applySkillComponents, combatBodyPerformance, directionalDefenseFor, staminaPolicyFor, tidebreakMindVectorFor, tidebreakMindsetFromVector } from './combat-tactics.js';
...
function phaseRecipe(state,phase,weapon,combo,target){
  const performance=combatBodyPerformance(state);
  ...
  if(causal){
    ...
    return{id:`rinne-${phase}-${causal.id}`,name:causal.name,type:'normal',weapon,element:'steel',rhythm:'flow',tempo:Math.max(.7,Math.min(1.3,staminaPolicyFor(state).tempoScale*performance.techniqueTempoScale)),aura:'none',steps};
  }
  ...
  return{id:`rinne-${phase}-${skill}`,name:techniqueName(skill,state),type:'normal',weapon,element:'steel',rhythm:raw.rhythm,tempo:Math.max(.7,Math.min(1.3,raw.tempo*performance.techniqueTempoScale)),aura:'none',steps:kinds.map((kind,index)=>({kind,footwork:raw.feet[index]||'forward',charge:raw.charges[index]||'none'}))};
}
...
function reactionRecipe(state,weapon,target){const defense=target?directionalDefenseFor(state,target):{receive:'basic'},receive=defense.receive,counter=['backcounter','turncounter','thrustcounter','rising'].includes(receive),performance=combatBodyPerformance(state);return{id:`rinne-uke-${receive}`,name:'受け',type:'reaction',weapon,element:'steel',rhythm:'flow',tempo:Math.max(.75,performance.techniqueTempoScale),aura:'none',receive,steps:[{kind:counter?adaptKind('thrust',weapon):receive==='none'?'ready':'brace',footwork:counter?'chase':'stay',charge:'none'},{kind:counter?'guard':'retreat',footwork:'retreat',charge:'none'},{kind:'ready',footwork:'stay',charge:'none'}]};}
...
export function defensiveLoadoutFor(state,target){const weapon=tidebreakWeaponFor(state.equipment.weapon),uke=reactionRecipe(state,weapon,target),policy=staminaPolicyFor(state),performance=combatBodyPerformance(state),kind=policy.band==='critical'?'retreat':'guard',hold={id:'rinne-threat-guard',name:'受勢',type:'normal',weapon,element:'steel',rhythm:'flow',tempo:Math.max(.7,Math.min(1.3,policy.tempoScale*performance.techniqueTempoScale)),aura:'none',steps:[{kind,footwork:kind==='retreat'?'retreat':'stay',charge:'none'},{kind:'brace',footwork:'stay',charge:'none'},{kind:'ready',footwork:'stay',charge:'none'}]};return{jo:hold,ha:hold,kyu:hold,uke};}
