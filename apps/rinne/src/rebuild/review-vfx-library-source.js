import {normalizeEffectAffinities} from '@soul/game-data';
...
const AFFINITY_RULES=Object.freeze([
  ...
  ['thunder',/thunder|lightning|electric/i],
  ['earth',/soil|earth|rock|gohlem|sand/i],
  ['light',/light(?!ning)|holy|sacred|benediction|moonlight/i],
  ['dark',/dark|shadow/i],
]);
export const inferVfxAffinities=sourcePath=>Object.freeze(normalizeEffectAffinities(AFFINITY_RULES.filter(([,pattern])=>pattern.test(String(sourcePath||''))).map(([id])=>id)));
...
export const reviewVfxEffect=(id,sourcePath,author)=>Object.freeze({
  ...
  scale:scaleFor(sourcePath),lifetime:2,affinities:inferVfxAffinities(sourcePath),reviewOnly:true
});
