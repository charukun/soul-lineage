...
export function tidebreakFrameFromSnapshot(actor,{targetId=null,intent='attack',sector='front'}={}){
  if(!actor)return null;return{execution:actor.execution?structuredClone(actor.execution):null,weapon:actor.weapon||null,transition:Boolean(actor.transition),reaction:Boolean(actor.reaction),attack:actor.attack||null,progress:clamp(safe(actor.progress),0,1),slot:actor.slot||null,skill:actor.skill||null,guarding:Boolean(actor.guarding),stun:safe(actor.stun),pose:actor.pose||null,targetId,intent,sector};
}
