...
function lineIntersectsRect(a,b,r){
  const steps=Math.max(3,Math.ceil(Math.hypot(b.x-a.x,b.z-a.z)/.22));for(let i=0;i<=steps;i++){const t=i/steps,x=a.x+(b.x-a.x)*t,z=a.z+(b.z-a.z)*t;if(Math.abs(x-r.x)<=r.w/2&&Math.abs(z-r.z)<=r.d/2)return true;}return false;
}
...
export function worldContactCandidates(state,front,event){
  ...
  return front.enemies.filter(enemy=>!enemy.dead&&enemy.id!==event.targetId&&dist(origin,enemy)<=radius).filter(enemy=>{
    const angle=Math.atan2(enemy.x-origin.x,enemy.z-origin.z);if(Math.abs(wrap(angle-profile.yaw))>halfArc)return false;if(lineBlocked(front,origin,enemy))return false;return true;
...
function firstProjectileHit(front,a,b){
  let best=null,bestD=Infinity;for(const enemy of front.enemies){if(enemy.dead)continue;const d=segmentDistance(a.x,a.z,b.x,b.z,enemy.x,enemy.z);if(d>.42)continue;const along=dist(a,enemy);if(along<bestD){best=enemy;bestD=along;}}return best;
}
...
export function tickRangedProjectiles(state,front,dt,events){
  ...
  // The staff is the current ranged archetype: strong at distance, finite catalyst charges, melee fallback when empty or pressured.
