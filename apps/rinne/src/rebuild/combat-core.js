...
import { bodyRuntime, combatFinisherRuntime, comboById, selectCombatCombo, techniqueName } from '../combat-loadout.js';
...
import {applyChoreographyImpact,combatBodyOutcome} from './combat-choreography.js';
...
function createSession(state,target,front,signature,{secondary=false,enemyCanHit=true,comboIdOverride=null}={}){
  ...
  const session={id:`${state.id}:${state.generation}:${front.stage}:${ordinal}`,runtime,loadout,sourceId:state.id,targetId:target.id,signature,secondary,enemyCanHit,heroHpScale:hScale,enemyHpScale:eScale,last:snapshot,lastAttackKey:snapshot.hero.execution?String(snapshot.hero.execution.attackId):null,oneMotionArmed:null,invalid:false,exchange:createJohakyuExchangeState({sourceId:state.id,targetId:target.id})};sessionMap(state).set(target.id,session);return session;
}
...
const FINISHER_RANGE=1.9,FINISHER_DURATION=.92,FINISHER_IMPACT=.58,DOWNED_RECOVERY_SECONDS=7.5;
...
function recoverDownedEnemies(state,front,events){if(!combatFinisherRuntime(state).nonlethal)return;for(const target of front.enemies){if(target.dead||!target.downed||(Number(target.downedElapsed)||0)<DOWNED_RECOVERY_SECONDS)continue;target.downed=false;target.downedElapsed=0;target.hp=Math.max(1,target.maxHp*.28);target.cooldown=Math.max(.9,Number(target.cooldown)||0);target.flash=.2;target.attentionTargetId=null;events.push({type:'enemy-recovered',targetId:target.id,hp:target.hp,engine:'tidebreak'});}}
function finisherFrame(progress,targetId,souenLabel='介錯'){const p=clamp(progress,0,1),swing=Math.sin(p*Math.PI),commit=clamp((p-.3)/.42,0,1);return{attack:'heavy',progress:p,slot:'kyu',skill:'トドメ',souenLabel,guarding:false,stun:0,targetId,intent:'finisher',sector:'front',pose:{pitch:-.08-.34*swing,twist:.22-.68*commit,roll:-.08*swing,pelvisYaw:.18-.32*commit,headPitch:-.08,headLag:.08,crouch:-.16*swing,hand:[.34,1.08,.12],tip:[.18+.18*commit,.52+.26*(1-commit),1.02-.48*commit],left:[-.26,1.14,.06]}};}
function finishEnemy(state,target,events,run=null){if(target.dead)return;target.dead=true;target.downed=false;target.downedElapsed=0;target.moving=false;target.attacking=false;target.tidebreakPose=null;events.push({type:'enemy-down',targetId:target.id,engine:'tidebreak',finisher:true,souenId:run?.souenId??null,souenLabel:run?.souenLabel??null});}
function startFinisher(state,target,events){const policy=combatFinisherRuntime(state);if(!policy.execute)return false;const duration=FINISHER_DURATION*(Number(policy.finisher.durationScale)||1),impactAt=Number.isFinite(policy.finisher.impactAt)?policy.finisher.impactAt:FINISHER_IMPACT;state.finisher={targetId:target.id,elapsed:0,duration,impactAt,committed:false,souenId:policy.finisher.id,souenLabel:policy.finisher.label};state.combat={targetId:target.id,phase:'kyu',engine:'finisher',attackCooldown:duration,tidebreakPose:finisherFrame(0,target.id,policy.finisher.label)};state.attacking=true;state.moving=false;events.push({type:'finisher-start',targetId:target.id,action:'トドメ',souenId:policy.finisher.id,souenLabel:policy.finisher.label,duration,impactAt,engine:'tidebreak'});return true;}
function advanceFinisher(state,front,dt,events){const run=state.finisher;if(!run)return false;const target=front.enemies.find(enemy=>enemy.id===run.targetId);if(!target||target.dead||!target.downed){state.finisher=null;if(state.combat?.engine==='finisher')state.combat=null;state.attacking=false;return false;}run.elapsed+=dt;const progress=clamp(run.elapsed/run.duration,0,1);state.combat={targetId:target.id,phase:'kyu',engine:'finisher',attackCooldown:Math.max(0,run.duration-run.elapsed),tidebreakPose:finisherFrame(progress,target.id,run.souenLabel)};state.yaw=turnToward(state.yaw,angleTo(state.position,target),Math.max(.08,dt*9));state.attacking=true;state.moving=false;if(!run.committed&&progress>=run.impactAt){run.committed=true;events.push({type:'finisher',targetId:target.id,phase:'finisher',action:'トドメ',souenId:run.souenId,souenLabel:run.souenLabel,damage:Math.max(1,target.maxHp*.4),manual:true,engine:'tidebreak'});}if(progress>=1){finishEnemy(state,target,events,run);state.finisher=null;state.combat=null;state.attacking=false;}return true;}
function finisherThreatRadius(state){const mind=tidebreakMindVectorFor(state);return clamp(2.15+mind.survival*.8+mind.guard*.55+mind.spacing*.25-mind.attack*.65,1.75,3.65);}
function maybeStartFinisher(state,front,events){if(state.finisher||state.down||state.ended)return false;const policy=combatFinisherRuntime(state);if(!policy.execute)return false;const active=front.enemies.filter(enemy=>!enemy.dead&&!enemy.downed);if(active.some(enemy=>dist(state.position,enemy)<=finisherThreatRadius(state)))return false;const downed=front.enemies.filter(enemy=>enemy.downed&&!enemy.dead);if(!downed.length)return false;const nearest=nearestEnemy(state.position,downed);if(!nearest.enemy||nearest.distance>FINISHER_RANGE)return false;return startFinisher(state,nearest.enemy,events);}
...
function sessionExchangeSnapshot(session,row){
  if(!row)return createJohakyuExchangeState({sourceId:session.sourceId,targetId:session.targetId});
  const id=value=>value==null?null:String(value)===String(session.last.hero.id)?session.sourceId:String(value)===String(session.last.enemy.id)?session.targetId:String(value);
  return johakyuExchangeSnapshot({...row,initiativeId:id(row.initiativeId),responderId:id(row.responderId),completedBy:id(row.completedBy),pair:row.pair.map(id)});
}
function updateSessionExchange(session,event){
  const id=value=>value===session.sourceId?session.last.hero.id:value===session.targetId?session.last.enemy.id:value;
  const observed=session.runtime.observeExchange({...event,sourceId:id(event.sourceId),targetId:id(event.targetId)});
  if(observed)session.exchange=sessionExchangeSnapshot(session,observed.exchange);
  return session.exchange;
}
function deepExchangeHit(damage,maxHp,impact,outcome,previousOutcome){return isDeepJohakyuExchangeHit({damage,maxHp,impact,outcome,previousOutcome});}
function projectPrimaryExchange(state,session,target){if(state.combat?.targetId===target?.id)state.combat.exchange=johakyuExchangeSnapshot(session.exchange);}
function applyTidebreakStep(state,target,front,dt,events,{primary=false,bodyAuthority=false,enemyPositionAuthority=false,enemyCanHit=true,intent='attack',sharedParticipantCount=1,comboIdOverride=null}={}){
  const session=sessionFor(state,target,front,{secondary:!primary,enemyCanHit,comboIdOverride});armOneMotion(state,session,dt);const manual=primary&&state.moving?.67:0;session.runtime.input(Math.sin(state.yaw),Math.cos(state.yaw),manual,0);
  ...
  const previousEnemyBody=dealt>.001?combatBodyOutcome(target):null,previousHeroBody=taken>.001?combatBodyOutcome(state):null;
  ...
  if(dealt>.001)updateSessionExchange(session,{type:'hit',sourceId:state.id,targetId:target.id,phase:armed?'one':(slot||state.combat?.phase||'jo'),deep:deepExchangeHit(dealt,target.maxHp,outgoingImpact,enemyBody?.outcome,previousEnemyBody)});
  if(taken>.001)updateSessionExchange(session,{type:'hit',sourceId:target.id,targetId:state.id,phase:next.enemy?.slot||'enemy',deep:deepExchangeHit(taken,state.maxHp,incomingImpact,heroBody?.outcome,previousHeroBody)});
  ...
  projectPrimaryExchange(state,session,target);
...
function tickFrontStep(state,front,dt,{advanceEnemies=true,allowedEnemyIds=null,advanceFormation=true,heroPositionAuthority=true,enemyPositionAuthority=true,attentionStates=null,sharedParticipantCount=1}={}){
  ...
  if(state.zone!=='frontier'||state.ended)return events;if(advanceEnemies)advanceEnemyClock(front,dt);const remaining=front.enemies.filter(enemy=>!enemy.dead);if(!remaining.length){front.cleared=true;if(advanceEnemies)front.clearSeconds+=dt;state.finisher=null;state.combat=null;state.attacking=false;clearSession(state);events.push({type:'front-cleared',stage:front.stage,engine:'tidebreak'});return events;}const finisherPolicy=combatFinisherRuntime(state);if(finisherPolicy.nonlethal&&remaining.every(enemy=>enemy.downed)){front.cleared=true;if(advanceEnemies)front.clearSeconds+=dt;state.finisher=null;state.combat=null;state.attacking=false;clearSession(state);events.push({type:'front-cleared',stage:front.stage,nonlethal:true,engine:'tidebreak'});return events;}if(advanceEnemies&&finisherPolicy.nonlethal)recoverDownedEnemies(state,front,events);if(advanceFinisher(state,front,dt,events))return events;if(maybeStartFinisher(state,front,events)){advanceFinisher(state,front,0,events);return events;}const allLiving=remaining.filter(enemy=>!enemy.downed);refreshAttention(allLiving,attentionStates||[state]);
  ...
  let techniqueChoice=null;
  if(state.combat&&target&&intent.mode==='attack'&&intent.stamina.allowOffense){
    techniqueChoice=resolveCapabilityTechniqueChoice(state,target);
    if(!techniqueChoice?.ok){
      const current=sessionMap(state).get(target.id);if(current){updateSessionExchange(current,{type:'capability-failure',sourceId:state.id,targetId:target.id,phase:state.combat.phase});projectPrimaryExchange(state,current,target);}
      const reason=techniqueChoice?.reason||'capability-failure',attempt=techniqueChoice?.attempts?.find(row=>row.reason===reason),stage=attempt?.capability?.stages?.find(row=>!row.allowed),remaining=['arm-injury','leg-injury'].includes(reason)?.85:.45;
      state.combat.executionBlock={reason,remaining,phase:state.combat.phase,kind:stage?.kind??null,stageIndex:stage?.index??null};state.combat.attackCooldown=Math.max(state.combat.attackCooldown||0,remaining);
      events.push({type:'execution-blocked',sourceId:state.id,targetId:target.id,...state.combat.executionBlock,engine:'tidebreak',authority:'rinne-domain'});
      intent={...intent,mode:'recover',stamina:{...intent.stamina,allowOffense:false}};
    }
  }
  if(state.combat){state.combat.bodyIntent=intent.mode;state.combat.bodyTargetId=intent.bodyTargetId;state.combat.mindVector=intent.vector;state.combat.staminaBand=intent.stamina.band;state.combat.capabilityChoice=techniqueChoice?{phase:state.combat.phase,comboId:techniqueChoice.comboId,techniqueId:techniqueChoice.techniqueId,adapted:techniqueChoice.adapted,reason:techniqueChoice.ok?null:techniqueChoice.reason}:null;state.combat.tidebreakPose=null;}
  for(const enemy of threats){if(state.down||state.ended)break;const isTarget=enemy.id===state.combat?.targetId,offense=isTarget&&intent.mode==='attack'&&intent.stamina.allowOffense,bodyAuthority=heroPositionAuthority&&enemy.id===intent.bodyTargetId,enemyCanHit=enemy.attentionTargetId===state.id||!enemy.attentionTargetId,comboIdOverride=isTarget&&offense&&techniqueChoice?.ok?techniqueChoice.comboId:null;applyTidebreakStep(state,enemy,front,dt,events,{primary:offense,bodyAuthority,enemyPositionAuthority:enemyPositionAuthority&&enemyCanHit,enemyCanHit,intent:intent.mode,sharedParticipantCount,comboIdOverride});}
