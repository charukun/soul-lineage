...
import { createFront, tickFront } from './combat.js';
...
export async function startRuntime({mode,buildInfo,name,onExit}){
  ...
  state.position=safeMuraPosition(layout,state.position);
  ...
  let alive=true,raf=0,last=performance.now(),saveElapsed=0,toastTimer=0,endDialog=null,pointer=null,keyboard={x:0,y:0},axis={x:0,y:0},portDwell=0,front=state.zone==='frontier'?createFront(state.front,state.seed):null;
  ...
  const save=async()=>{if(!alive)return false;try{await platform.storage.write(saveKey,serializeLife(state));return true;}catch(error){toast('保存できませんでした');console.error(error);return false;}};
  function syncUI(){
  ...
  function endLife(){
    if(endDialog?.open)return;endDialog=document.createElement('dialog');endDialog.className='life-end-dialog';
    const choices=state.knownSkills.filter(x=>!x.startsWith('memory:')).slice(-10);endDialog.innerHTML=`<form method="dialog"><p>100年人生</p><h2>${state.name}の生涯</h2><p>${state.generation}代目は100歳を迎えました。</p><label>次の生へ遺す記憶<select id="memento"></select></label><button value="rebirth" id="rebirth">次の人生へ</button></form>`;
    document.body.append(endDialog);const select=endDialog.querySelector('#memento');for(const id of choices.length?choices:['']){const o=document.createElement('option');o.value=id;o.textContent=id||'村で過ごした日々';select.append(o);}endDialog.addEventListener('close',async()=>{if(endDialog.returnValue==='rebirth'){state=rebirth(state,{memento:select.value||null});state.position=safeMuraPosition(layout,state.position);await save();endDialog.remove();endDialog=null;toast('また、生まれた。');}else endDialog.showModal();});endDialog.showModal();
  }
  ...
  $('talk').onclick=talk;
  $('clock-rate').onchange=e=>{try{setClockRate(state,Number(e.target.value));save();}catch(error){toast(error.message);}};
  ...
  function frame(now){
    ...
    if(state.zone==='village'&&station?.port&&canDepart(state)&&!moved){portDwell+=dt;if(portDwell>=1.5&&depart(state)){front=createFront(0,state.seed);view.syncFront(front);toast('船が出る。前線へ向かった。');portDwell=0;}}else portDwell=0;
    if(state.zone==='frontier'){front??=createFront(state.front,state.seed);const battle=tickFront(state,front,dt);handleEvents(battle);view.syncFront(front);
      if(front.cleared&&state.position.z<=-5.85&&state.front<5&&advanceFront(state)){front=createFront(state.front,state.seed);view.syncFront(front);toast(`第${state.front+1}前線へ進んだ`);}
      else if(front.cleared&&state.front>=5&&state.position.z>=4.8&&returnHome(state)){state.position=safeMuraPosition(layout,{x:166,z:0});front=null;toast('100年人生の故郷へ帰還した');}
      if(state.zone==='village'&&battle.some(e=>e.type==='rescued')){state.position=safeMuraPosition(layout,{x:0,z:0});front=null;}
    }
