...
import { installVillageWalkInput } from './village-walk-input.js';
import { installLocomotionPresentation } from './locomotion-presentation.js';
...
export async function startRuntime({mode,buildInfo,name,onExit,onLifeHome,onProgress,prepared,coop=null,deathCinematic=null,family=null,expectedSave=undefined}={}){
  ...
  const swipe=new SwipeInput(),locomotion=createLocomotion({canMoveTo:(...args)=>view.canMoveTo(...args)});
  view.clearLocomotion?.();
  ...
  const villageWalk=installVillageWalkInput({canvas,view,stations,getState:()=>state,enabled:()=>active&&!coop&&!document.hidden&&!document.querySelector('dialog[open]')&&!document.body.classList.contains('rinne-first-run-active')&&gameScreen.dataset.storybookOpen!=='true'});
  ...
  function syncMovementHint(){
    $('move-hint').textContent=birth.active()?'スワイプで母を動かせる':'タップで歩く · スワイプで移動 · 長押しで休む';
  ...
  function handleEvents(events,eventKey){view.presentCombatEvents?.(events,{state,front,eventKey});for(const event of events){
    ...
    if(event.type==='pursuit-leap')gameScreen.dispatchEvent?.(new CustomEvent('rinne:pursuit-leap',{detail:{sourceId:event.sourceId,targetId:event.targetId}}));
  ...
  function frame(now){
    ...
    if(birthStep.handled){locomotion.reset();moved=birthStep.moved;carrierMoving=birthStep.carrierMoving;}
      ...
      const enabled=!document.hidden&&!document.querySelector('dialog[open]')&&!state.ended&&!state.down;
      const direction=enabled&&mag>.08?view.cameraVector(moveAxis):null,speed=(state.interior?interiorSpeedForAge(state.ageYears):speedForAge(state.ageYears))*(state.combat?.72:1);
  ...
  function visibility(){last=performance.now();coop?.pause(document.hidden);if(document.hidden){locomotion.reset();view.clearLocomotion?.();view.clearCombatEffects?.();keys.clear();swipe.cancel();setAxis({x:0,y:0});}}
