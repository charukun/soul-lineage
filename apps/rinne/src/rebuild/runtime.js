...
export async function startRuntime({mode,buildInfo,name,onExit,onProgress,prepared,coop=null}){
  ...
  function renderCoopFrame(dt,now){
  ...
  function frame(now){
    ...
    if(coop){renderCoopFrame(dt,now);return;}
  ...
  function visibility(){last=performance.now();coop?.pause(document.hidden);if(document.hidden){keys.clear();pointer=null;flickDash=null;setAxis({x:0,y:0});}}
