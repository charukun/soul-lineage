...
export function createBirthExperience({document,canvas,gameScreen,view,stations,getState,dialogue}){
  const tour=createBirthTour(stations),status=createActorStatus({document,canvas,view,actorName:'Player'}),mother=view.scene.getObjectByName('Mother');
  ...
  return{active,step,afterRender,showIntro,release,floatStatus:status.show,dispose,tour};
}
