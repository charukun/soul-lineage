...
/** Remove obsolete XP/mutation fields without erasing validated causal technique records. */
export function stripCombatProgressionState(state){
  ...
  const causal=state.inspiration?.version===1,learned=new Set(causal?[...(state.inspiration.legacySkills||[]),...Object.keys(state.inspiration.records||{})]:[]);
  if(Array.isArray(state.knownSkills))state.knownSkills=state.knownSkills.filter(id=>BASIC_SKILL(id)||learned.has(id));
  if(Array.isArray(state.lineage))state.lineage=state.lineage.map(row=>{if(!row||typeof row!=='object')return row;const clean={...row};delete clean.combatLegacy;if(clean.experiences&&typeof clean.experiences==='object'){clean.experiences={...clean.experiences};delete clean.experiences.combat;}if(!causal&&Array.isArray(clean.skills))clean.skills=clean.skills.filter(BASIC_SKILL);return clean;});
...
export function injuryEffects(state){
  ...
  return{attackScale:clamp(1-arms*.46-head*.08,.45,1),movementScale:clamp(1-legs*.52-torso*.08,.4,1),judgmentScale:clamp(1-head*.48-torso*.08,.42,1),staminaScale:clamp(1-torso*.38-head*.16,.42,1),severity:clamp((arms+legs+head+torso)/4,0,1)};
}
...
export function applyCombatInjury(state,{damage=0,sector='front',sourceId=''}={}){
  ...
  row.severity=clamp(row.severity+gain,0,1);row.at=Number(state.ageSeconds)||0;return{part,severity:row.severity,gain};
}
