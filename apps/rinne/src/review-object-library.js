...
function runtimeObject(item){
  ...
  if(item.runtimeKind==='training-dummy'){
    ...
    mesh(new THREE.BoxGeometry(.76,.82,.28),cloth,0,1.35);
    mesh(new THREE.SphereGeometry(.28,10,8),cloth,0,2.25);
  }else if(item.runtimeKind==='armor-stand'){
    mesh(new THREE.CylinderGeometry(.08,.1,2.15,8),wood,0,1.08);
    mesh(new THREE.CapsuleGeometry(.34,.55,5,9),metal,0,1.34);
    mesh(new THREE.BoxGeometry(.95,.2,.42),metal,0,1.72);
  }else{
