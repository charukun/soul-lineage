...
export async function createReviewBattleStage({canvas,onStatus=()=>{},onInspirationCue=()=>{}}={}){
  ...
  function animateHero(state,target,time,dt,playbackTime=time){
    ...
    if(sequence?.nearMiss>0&&target){const dx=(Number(target.x)||0)-hero.actor.root.position.x,dz=(Number(target.z)||0)-hero.actor.root.position.z,len=Math.max(.001,Math.hypot(dx,dz)),side=techniquePlayback?.nearMissSide||1;hero.actor.root.position.x+=(-dz/len)*side*.27*sequence.nearMiss;hero.actor.root.position.z+=(dx/len)*side*.27*sequence.nearMiss;}
    hero.actor.sample(hero.appearance,time,(bones,sampleTime)=>{addStride(bones,sampleTime,presentation.stride);if(sequence?.nearMiss>0&&bones.spine)bones.spine.rotation.z+=(techniquePlayback?.nearMissSide||1)*.17*sequence.nearMiss;if(sequence?.stage==='reveal'){if(bones.spine)bones.spine.rotation.x-=.16;if(bones.rightUpperArm){bones.rightUpperArm.rotation.x-=.7;bones.rightUpperArm.rotation.z+=.35;}}if(sequence?.executeProgress>0&&sequence.stage!=='done'&&bones.spine)bones.spine.rotation.y+=Math.sin(sequence.executeProgress*Math.PI*2)*.22;if(!frame?.attack)addGuardPose(bones,'hero');applyTidebreakPose(bones,frame);applyReviewCombatMotion(bones,frame,sequence,sampleTime);if(time<hero.hitUntil&&bones.spine)bones.spine.rotation.z-=.13;});
  ...
  function inspirationCameraFrame(sequence){
    ...
    const close=stage==='camera'||stage==='spacing'||stage==='stagger'||stage==='silence',behind=close?(portrait?2.85:2.55):(portrait?4.4:3.8),lateral=close?(portrait?2.5:2.85):(portrait?4.0:4.65),height=close?(portrait?1.95:1.72):(portrait?2.55:2.28);
  ...
  function sync(core,dt=0,{followCamera=true,encounterMode:requestedMode='duel',cameraSystem='rinne'}={}){
    ...
    const beat=sequence?.stage||'done',heroRoot=hero.actor?.root,enemyRoot=enemies[0]?.actor?.root,fxVisible=cinematic&&['silence','execute','impact'].includes(beat);inspirationFx.visible=fxVisible;aura.visible=cinematic&&beat==='impact';
