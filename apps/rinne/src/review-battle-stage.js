...
export async function createReviewBattleStage({canvas,onStatus=()=>{},onInspirationCue=()=>{}}={}){
  ...
  function inspirationCameraFrame(sequence){
    ...
    const distance=clamp(encounterMode==='one-v-three'?8.4:7.5,7.1,8.8),backstep=Math.max(0,Math.min(1,Number(sequence?.backstepProgress)||0)),orbit=backstep*backstep*(3-2*backstep),shoulder=1.35*orbit;
    const position={x:heroPoint.x-forwardX*distance+sideX*shoulder,y:3.05,z:heroPoint.z-forwardZ*distance+sideZ*shoulder};
    const look={x:heroPoint.x+forwardX*len*.48-sideX*(.12*orbit),y:1.0,z:heroPoint.z+forwardZ*len*.48-sideZ*(.12*orbit)};
  ...
  function sync(core,dt=0,{followCamera=true,encounterMode:requestedMode='duel',cameraSystem='rinne'}={}){
    ...
    const hudStage=canvas.closest('.stage'),signHud=hudStage?.querySelector('#battle-sign'),bulbHud=hudStage?.querySelector('#battle-lightbulb'),inspirationHud=hudStage?.querySelector('#battle-inspiration'),heroRoot=hero.actor?.root;if(signHud&&heroRoot){const projected=heroRoot.position.clone();projected.y+=2.18;projected.project(camera);signHud.dataset.signAnchor='head';signHud.style.setProperty('left',`${(projected.x*.5+.5)*100}%`,'important');signHud.style.setProperty('top',`${(-projected.y*.5+.5)*100}%`,'important');}if(bulbHud&&heroRoot){const projected=heroRoot.position.clone();projected.y+=2.5;projected.project(camera);bulbHud.style.setProperty('left',`${(projected.x*.5+.5)*100}%`,'important');bulbHud.style.setProperty('top',`${(-projected.y*.5+.5)*100}%`,'important');}if(inspirationHud&&heroRoot){const projected=heroRoot.position.clone();projected.y+=2.82;projected.project(camera);const left=clamp((projected.x*.5+.5)*100,18,82),top=clamp((-projected.y*.5+.5)*100,8,44);inspirationHud.dataset.inspirationAnchor='hero-head';inspirationHud.style.setProperty('left',`${left}%`,'important');inspirationHud.style.setProperty('top',`${top}%`,'important');}
