function installTitleLiveWorld(){
  ...
  const draw=()=>{
    if(!title.hidden&&source.width>1&&source.height>1){
      const rect=title.getBoundingClientRect(),scale=title.dataset.media==='realtime'&&title.dataset.intro==='cinematic'?.78:Math.min(1.25,window.devicePixelRatio||1),w=Math.max(2,Math.round(rect.width*scale)),h=Math.max(2,Math.round(rect.height*scale)),size=`${w}x${h}`;
      ...
      try{
        ...
        if(source.dataset.titleBeat)title.dataset.cinematicBeat=source.dataset.titleBeat;if(!hasFrame&&visibleCapture(w,h)){hasFrame=true;title.dataset.liveWorld='ready';}
      }catch{}
    }
