...
const skeletonRows=REVIEW_SKELETON_EQUIPMENT
  ...
  .map(row=>{
    ...
    return freeze({
      ...
      targetFraction:row.targetFraction||targetFraction(category,row.id),
...
const kaykitBattleRows=[
  ...
  source:'KayKit Adventurers · CC0-1.0',targetFraction:fraction,recommended,
...
const objectRows=RINNE_OBJECT_REVIEW_CATALOG
  ...
  .map(item=>{
    ...
    return freeze({
      ...
      source:item.source,targetFraction:targetFraction(category,item.id),
