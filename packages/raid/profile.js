...
function learnTrait(p,key){const first=!p.unlocked.includes(key);if(first){p.unlocked.push(key);p.currentLife.powers.push(key);const empty=p.equipped.findIndex(k=>!k);if(empty>=0)p.equipped[empty]=key;else if(p.equipped.length<3)p.equipped.push(key);p.form=strongestForm(p);}return first;}
function adaptation(p,role,now){const a=p.adaptations[role]||={encounters:0,moves:[],lastSeenAt:0};a.lastSeenAt=now;return a;}
...
export class ProfileStore {
 ...
 consume(key,{learnTrait:shouldLearnTrait=false}={}){if(!Object.hasOwn(PREY,key))throw new SaveError('存在しない特能です。');return this.change(p=>{const first=shouldLearnTrait?learnTrait(p,key):false;p.totalEaten++;return first;});}
 unlock(key){return this.consume(key,{learnTrait:true});}
 recordBattle(role){if(!Object.hasOwn(PREY,role))throw new SaveError('存在しない相手です。');return this.change(p=>{const a=adaptation(p,role,this.now());a.encounters++;p.currentLife.battles++;return a.encounters;});}
 learn(role,move){if(!Object.hasOwn(PREY,role)||typeof move!=='string'||!move||move.length>80)throw new SaveError('覚えられない動きです。');return this.change(p=>{const a=adaptation(p,role,this.now());const first=!a.moves.includes(move);if(first)a.moves.push(move);if(first&&!p.currentLife.moves.includes(move))p.currentLife.moves.push(move);return first;});}
