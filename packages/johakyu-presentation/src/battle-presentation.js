...
export function presentBattleFrame(frame){return {...frame,actors:frame.actors.map(row=>({...row,combatReady:row.state?row.state.weapon==='drawn':Boolean(row.combatReady||row.action||row.phaseCue?.phase==='zanshin'),phaseCue:presentPhaseCue(row.phaseCue),action:presentBattleAction(row.action),locomotion:row.locomotion?{...row.locomotion,...resolveJohakyuLocomotion({footwork:row.locomotion.kind})}:null}))};}
