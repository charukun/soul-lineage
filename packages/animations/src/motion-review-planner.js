...
export function buildMotionReviewPlan({revision,evidence={},issues=[]}={}){
 ...
 const foot=evidence.footSliding;if(foot?.failed>0)push('foot-slide',5,{failed:foot.failed,warnings:foot.warnings,worst:foot.worst});else if(foot?.warnings>0)push('foot-slide',3,{failed:0,warnings:foot.warnings,worst:foot.worst});
 const jerk=evidence.jerk;if(jerk?.spikes?.length)push('motion-jerk',4,{count:jerk.spikes.length,worst:[...jerk.spikes].slice(0,8)});
 const device=evidence.deviceCalibration;if(device?.measuredHardware===true&&Number.isFinite(device.totalMeanMs)){const budget=evidence.deviceBudgetMs;if(Number.isFinite(budget)&&device.totalMeanMs>budget)push('device-motion-budget',4,{deviceClass:device.deviceClass,totalMeanMs:device.totalMeanMs,budgetMs:budget,cohort:device.cohort});}
...
export function motionDebugOverlayData({centerOfMass=null,supports=[],trajectory=null,interaction=null,hitDirection=null,footLocks=null,semanticEvents=[],weaponArc=[],warpWindow=null,poseRanking=[],layerPlan=null}={}){
 if(!Array.isArray(supports)||!Array.isArray(semanticEvents)||!Array.isArray(weaponArc)||!Array.isArray(poseRanking))throw Error('Invalid motion debug overlay');const point=p=>p&&Number.isFinite(p.x)&&Number.isFinite(p.z)?freeze({x:p.x,y:Number.isFinite(p.y)?p.y:0,z:p.z}):null;
 const com=point(centerOfMass),supportPoints=supports.map(point).filter(Boolean),future=(trajectory?.points??[]).map(point).filter(Boolean),hit=hitDirection&&finite(hitDirection.x,hitDirection.z)?freeze({x:hitDirection.x,z:hitDirection.z}):null,arc=weaponArc.map(point).filter(Boolean);
 return freeze({version:2,centerOfMass:com,supports:freeze(supportPoints),futureTrajectory:freeze(future),interaction:interaction?freeze({schema:interaction.schema?.id??interaction.schema??null,anchors:interaction.anchors??null,partnerId:interaction.partnerId??null}):null,hitDirection:hit,footLocks:footLocks??null,semanticEvents:freeze(semanticEvents.map(e=>freeze({name:e.name,phase:e.phase,seconds:e.seconds}))),weaponArc:freeze(arc),warpWindow:warpWindow?freeze({...warpWindow}):null,poseRanking:freeze(poseRanking.slice(0,5).map(row=>freeze({id:row.id,score:row.score}))),layerPlan:layerPlan?freeze({order:freeze([...(layerPlan.order??[])]),exclusiveOwners:layerPlan.exclusiveOwners??null}):null,diagnosticOnly:true});
}
