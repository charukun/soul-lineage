...
export function createSemanticMotionTimeline({duration,contactPhase=.5,activeStart=.42,activeEnd=.58,plantPhase=.12,handoffPhase=.9}={}){
 ...
 const events=SEMANTIC_MOTION_EVENTS.map(name=>freeze({name,phase:phases[name],seconds:phases[name]*duration,authoritative:false})).sort((a,b)=>a.phase-b.phase||a.name.localeCompare(b.name));
...
export function personalSpaceSteering({position={x:0,z:0},preferredVelocity={x:0,z:0},neighbors=[],radius=.7,maxSuggestion=.35}={}){const p=xyz2(position),v=xyz2(preferredVelocity);if(!p||!v||!Array.isArray(neighbors)||!finite(p.x,p.z,v.x,v.z,radius,maxSuggestion)||radius<=0||maxSuggestion<0)throw Error('Invalid personal space steering');let sx=0,sz=0;for(const raw of neighbors){const n=xyz2(raw);if(!n||!finite(n.x,n.z))continue;const dx=p.x-n.x,dz=p.z-n.z,d=Math.hypot(dx,dz);if(d<1e-6||d>=radius)continue;const weight=1-d/radius;sx+=dx/d*weight;sz+=dz/d*weight;}const m=Math.hypot(sx,sz);if(m>maxSuggestion&&m>1e-9){sx*=maxSuggestion/m;sz*=maxSuggestion/m;}return freeze({version:1,preferred:freeze({x:v.x,z:v.z}),suggestion:freeze({x:sx,z:sz}),advisory:true,worldAuthority:false});}
