...
export function installMusicLibrary({game,environment,mount=document.body,defaultTrack=null,autoStart=false,preferDefault=false,trigger='visible',onOpen=()=>{},onClose=()=>{},contextNote=''}){
 ...
 const root=document.createElement('div');root.className='soul-music';root.innerHTML=`<button type="button" data-open ${trigger==='hidden'?'hidden':''}>音楽室</button><dialog aria-label="三界の調べ"><form method="dialog"><button>閉じる</button></form><h2>三界の調べ</h2><p>150曲から選んで、この世界とともに。</p><p data-context hidden></p><label>検索 <input type="search" data-search placeholder="曲名・場面"></label><label>世界 <select data-world><option value="">すべて</option><option value="rinne">輪廻転焦</option><option value="village">MURAAAAAAA</option><option value="demon">尽喰廻遊</option><option value="shared">共通</option></select></label><label><input type="checkbox" data-favorites>お気に入りのみ</label><p data-count></p><div data-tracks></div><p role="status" data-state>${autoStart&&bootTrack?`最初の操作で再生：${catalog[bootTrack].title}`:'曲を選ぶと再生します'}</p><audio controls preload="none"></audio><label>BGM音量 <input type="range" data-volume min="0" max="1" step=".01"></label><button type="button" data-stop>停止</button><details><summary>試聴音源について</summary><p>DEV用の試聴版です。商用公開の権利確認は未完了です。</p></details></dialog>`;
 const style=document.createElement('style');style.textContent='.soul-music{position:fixed;right:14px;bottom:16px;z-index:90;font:14px sans-serif}.soul-music button,.soul-music input,.soul-music select{font:inherit;padding:9px;margin:4px}.soul-music>button{background:#18282d;color:#ffe8b6;border:1px solid #9b8658;border-radius:20px}.soul-music [hidden]{display:none!important}.soul-music dialog{box-sizing:border-box;width:min(680px,94vw);height:min(86dvh,720px);max-height:none;overflow:hidden;background:#142027;color:#f1ead7;border:1px solid #9b8658;border-radius:16px;padding:18px;display:grid;grid-template-columns:1fr 1fr;grid-template-rows:auto;gap:6px 12px}.soul-music dialog:not([open]){display:none!important}.soul-music dialog::backdrop{background:#0009}.soul-music dialog>form{grid-column:1/-1;text-align:right}.soul-music dialog>h2{margin:0}.soul-music dialog>h2+p{text-align:right;margin:0}.soul-music label{display:block}.soul-music [data-tracks]{grid-column:1/-1;min-height:0;overflow:auto}.soul-music [data-tracks]>div{display:flex;border-bottom:1px solid #647477}.soul-music [data-tracks] button:first-child{flex:1;text-align:left;min-width:0}.soul-music audio{grid-column:1/-1;width:100%}@media(max-width:560px){.soul-music dialog{grid-template-columns:1fr;height:90dvh;padding:13px}.soul-music dialog>*{grid-column:1!important}.soul-music dialog>h2+p{text-align:left}}';style.textContent+=`.soul-music dialog[open]{display:flex;flex-direction:column;gap:5px}.soul-music dialog>*{flex-shrink:0}.soul-music dialog>form{margin:0;align-self:flex-end}.soul-music dialog h2{font-size:20px}.soul-music dialog>p{font-size:12px;margin:0;line-height:1.5}.soul-music dialog [data-context]{color:#bed3c5}.soul-music [data-tracks]{flex:1 1 100px;min-height:64px;overflow:auto}.soul-music dialog button{min-height:44px}.soul-music dialog input[type=search]{max-width:72%;box-sizing:border-box}.soul-music dialog details{max-height:14vh;overflow:auto;font-size:11px}.soul-music dialog audio{min-height:40px;height:40px}`;root.append(style);mount.append(root);
 ...
 const context=root.querySelector('[data-context]');context.textContent=contextNote;context.hidden=!contextNote;
 const open=()=>{if(disposed||dialog.open)return;render();dialog.showModal();notifiedOpen=true;onOpen();};
