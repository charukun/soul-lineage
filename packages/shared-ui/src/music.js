...
export function installMusicLibrary({game,environment,mount=document.body,defaultTrack=null,autoStart=false,preferDefault=false,trigger='visible'}){
 ...
 const root=document.createElement('div');root.className='soul-music';root.innerHTML=`<button type="button" data-open ${trigger==='hidden'?'hidden':''}>音楽室</button><dialog aria-label="三界の調べ"><form method="dialog"><button>閉じる</button></form><h2>三界の調べ</h2><p>150曲から選んで、この世界とともに。</p><label>検索 <input type="search" data-search placeholder="曲名・場面"></label><label>世界 <select data-world><option value="">すべて</option><option value="rinne">輪廻転焦</option><option value="village">星継ぎの庭</option><option value="demon">暗い喰らいCry</option><option value="shared">共通</option></select></label><label><input type="checkbox" data-favorites>お気に入りのみ</label><p data-count></p><div data-tracks></div><p role="status" data-state>${autoStart&&bootTrack?`最初の操作で再生：${catalog[bootTrack].title}`:'曲を選ぶと再生します'}</p><audio controls preload="none"></audio><label>音量 <input type="range" data-volume min="0" max="1" step=".01"></label><button type="button" data-stop>停止</button><details><summary>試聴音源について</summary><p>DEV用の試聴版です。商用公開の権利確認は未完了です。</p></details></dialog>`;
 const style=document.createElement('style');style.textContent='.soul-music{position:fixed;right:14px;bottom:16px;z-index:90;font:14px sans-serif}.soul-music button,.soul-music input,.soul-music select{font:inherit;padding:9px;margin:4px}.soul-music>button{background:#18282d;color:#ffe8b6;border:1px solid #9b8658;border-radius:20px}.soul-music dialog{box-sizing:border-box;width:min(680px,94vw);height:min(86dvh,720px);max-height:none;overflow:hidden;background:#142027;color:#f1ead7;border:1px solid #9b8658;border-radius:16px;padding:18px;display:grid;grid-template-columns:1fr 1fr;grid-template-rows:auto auto auto minmax(0,1fr) auto;gap:6px 12px}.soul-music dialog:not([open]){display:none!important}.soul-music dialog::backdrop{background:#0009}.soul-music dialog>form{grid-column:1/-1;text-align:right}.soul-music dialog>h2{margin:0}.soul-music dialog>h2+p{text-align:right;margin:0}.soul-music label{display:block}.soul-music [data-tracks]{grid-column:1/-1;min-height:0;overflow:auto}.soul-music [data-tracks]>div{display:flex;border-bottom:1px solid #647477}.soul-music [data-tracks] button:first-child{flex:1;text-align:left;min-width:0}.soul-music audio{grid-column:1/-1;width:100%}@media(max-width:560px){.soul-music dialog{grid-template-columns:1fr;height:90dvh;padding:13px}.soul-music dialog>*{grid-column:1!important}.soul-music dialog>h2+p{text-align:left}}';root.append(style);mount.append(root);
 ...
 on($('[data-open]'),'click',()=>{render();if(!dialog.open)dialog.showModal();});on($('[data-search]'),'input',render);on($('[data-world]'),'change',render);on($('[data-favorites]'),'change',render);
 ...
 const api={root,dialog,player,open(){render();if(!dialog.open)dialog.showModal();},close(){if(dialog.open)dialog.close();},play,resume,pause,get track(){return prefs.track||bootTrack;},get playing(){return!player.paused;}};
 window.__SOUL_MUSIC__=api;window.dispatchEvent(new CustomEvent('soul-music:ready',{detail:api}));
 return()=>{disposed=true;abort.abort();serial++;player.pause();releaseURL();if(window.__SOUL_MUSIC__===api)delete window.__SOUL_MUSIC__;root.remove();};
}
