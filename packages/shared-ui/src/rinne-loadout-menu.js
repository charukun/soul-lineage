...
export function createRinneLoadoutSlotRow({documentRef=document}={}){const node=documentRef.createElement('section');node.className='loadout-slot-row';return node;}
export function createRinneLoadoutSlot({label='',value='',meta='タップして選択',selected=false,empty=false,icon='empty',onClick=null,documentRef=document}={}){const button=documentRef.createElement('button');button.type='button';button.className='loadout-slot';button.dataset.selected=String(selected);button.dataset.empty=String(empty);button.innerHTML=rinneSkillSigilMarkup(icon)+'<span></span><strong></strong><small></small>';button.querySelector('span').textContent=label;button.querySelector('strong').textContent=value||'空き';button.querySelector('small').textContent=meta;if(onClick)button.onclick=onClick;return button;}
