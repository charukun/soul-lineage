...
export function openBrandBootGate({app='rinne',ready=null}={}){
  ...
  return new Promise(resolve=>{let done=false;const enter=()=>{if(done)return;done=true;unlockAudio();el.disabled=true;el.classList.add('leave');const fade=new Promise(r=>setTimeout(r,500));Promise.all([fade,readyWait]).finally(()=>{el.remove();style.remove();document.documentElement.style.overflow=old;resolve()})};el.addEventListener('pointerup',enter,{once:true});el.addEventListener('click',e=>{if(e.detail===0)enter()},{once:true})});
}
