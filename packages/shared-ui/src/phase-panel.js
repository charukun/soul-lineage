...
export function createPhasePanel(root,{skin='jinku',brand=SKIN_LABELS[skin]||'',details=true,visible=true}={}){
  if(!root)throw new TypeError('phase panel root is required');
  root.classList.add('soul-phase-panel');
  root.setAttribute('aria-label',root.getAttribute('aria-label')||'序破急の現在状態');
  root.setAttribute('aria-live',root.getAttribute('aria-live')||'polite');
  root.innerHTML=`<div class="soul-phase-head"><span class="soul-phase-brand" data-phase-brand></span><strong class="soul-phase-current" data-phase-current>待</strong></div><div class="soul-phase-track phases" aria-label="序破急"><span class="soul-phase-step" data-phase="jo" data-active="false"><b>序</b><small>JO</small></span><i aria-hidden="true"></i><span class="soul-phase-step" data-phase="ha" data-active="false"><b>破</b><small>HA</small></span><i aria-hidden="true"></i><span class="soul-phase-step" data-phase="kyu" data-active="false"><b>急</b><small>KYU</small></span></div><div id="enemy-name" class="soul-phase-opponent" data-phase-opponent></div><div id="skill-name" class="soul-phase-skill" data-phase-skill></div><div id="enemy-health-track" class="soul-phase-health" aria-label="敵の生命"><div id="enemy-health"></div></div><small class="soul-phase-meta" data-phase-meta></small>`;
  ...
  function setDetails(next){root.dataset.details=next?'true':'false';}
  function setVisible(next){root.style.opacity=next?'1':'0';}
  ...
  setVisible(visible);
  render();
  return Object.freeze({root,setSkin,setDetails,setVisible,render,elements:Object.freeze({brand:brandNode,current:currentNode,opponent:opponentNode,skill:skillNode,meta:metaNode,healthTrack,healthFill,steps})});
}
