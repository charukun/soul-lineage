"""Candidate generation followed by a required, separate quality refinement loop."""
import argparse
import json
import re
import shutil
...
from refinement import initialize
...
def run(options):
    ...
    if target.exists() and (target/'review/refinement.json').exists() and json.loads((target/'review/refinement.json').read_text()).get('rounds'):
        raise ValueError('Refinement evidence exists; repair the same package or generate a new id, never erase its rounds')
    ...
    def stage(name):history.append({'stage':name,'status':'completed'});save_json(temp/'pipeline-state.json',{'forgeVersion':VERSION,'stages':history,'qualityRefinement':'not-yet-reviewed'})
    try:
        ...
        analysis=json.loads(Path(options.analysis).read_text()) if options.analysis else {}
        if getattr(options,'base_only',False):analysis={**analysis,'baseMeshOnly':True}
        ...
        save_json(temp/'spec/reconstruction.json',spec);initialize(temp,report['modelSha256'])
        manifest=create_manifest(spec,report,clips,model);save_json(temp/'manifest.json',manifest)
        ...
        history.append({'stage':STAGES[14],'status':'completed'});save_json(target/'pipeline-state.json',{'forgeVersion':VERSION,'stages':history,'registeredPackages':count,'qualityRefinement':{'status':'awaiting-capture','path':'review/refinement.json','required':True}})
        return {'id':options.id,'path':str(target),'mode':spec['reconstructionMode'],'reviewStatus':'review-candidate','qualityRefinement':'awaiting-capture','performance':report['performance'],'comparisons':report['comparisons']}
...
def parser():
    ...
    p.add_argument('--analysis',help='Optional Astra semantic parts/side profiles JSON; not a user coordinate UI')
    p.add_argument('--base-only',action='store_true',help='No generated hair/clothing/accessories; does not assert Golden Rig or art approval')
