...
def register_review(root):
    ...
    for path in sorted((root/'packages/assets/characters/forge').glob('*/manifest.json')):
        ...
        refinement=',qualityRefinementUrl:'+url(m['qualityRefinement']) if m.get('qualityRefinement') else ',qualityRefinementUrl:null'
        entries.append('{manifest:m'+str(i)+',modelUrl:'+url(m['model']['path'])+',references:{'+refs+'},sourceReferences:{'+raw+'},reportUrl:'+url(m['validationReport'])+refinement+'}')
