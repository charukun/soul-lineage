...
from side_profiles import apply_side_profiles
...
def reconstruction_spec(identifier,name,views,measurements,detection,provenance,analysis=None):
    ...
    # Base-only authoring never invents hair or clothes from implementation diagrams.
    base_only=bool((analysis or {}).get('baseMeshOnly',False))
    if not base_only:
    ...
    for part in (analysis or {}).get('parts',[]):
        ...
        if base_only:raise ValueError('Base-only packages cannot add hair, clothes or accessories')
    ...
    spec={'schemaVersion':'rinne.character-reconstruction/v1','forgeVersion':VERSION,'id':identifier,'displayName':name,'reconstructionMode':modes[count],
      'packageKind':'golden-base-candidate' if base_only else 'character-package',
