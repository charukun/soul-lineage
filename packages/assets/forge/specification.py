...
def reconstruction_spec(identifier,name,views,measurements,detection,provenance,analysis=None):
    ...
    def component(cid,bottom,top,bone,kind='loft',side=None,role='body'):
        ...
        for i in range(9):
            ...
            if side:
                ...
                if candidates:
                    r=max(candidates,key=lambda r:abs(sum(r)));width=r[1]-r[0];cx=sum(r)/2
                else:
    ...
    base_mesh_only=bool(analysis.get('baseMeshOnly'))
    if not base_mesh_only:
    ...
    assumptions=['Upright neutral full-body image on a flat/transparent background','Front faces +Z, up +Y, metres; target height 1.6','Side faces image-left; back is unmirrored','Joint positions are inferred, silhouette samples are observed','Raw lighting remains in projected color']
    if not base_mesh_only:assumptions.append('Rear hair/clothing are candidate semantic hypotheses')
    return {'schemaVersion':'rinne.character-reconstruction/v1','forgeVersion':VERSION,'id':identifier,'displayName':name,'reconstructionMode':modes[count],
      'assetRole':analysis.get('assetRole','character-candidate'),'rigProfile':analysis.get('rigProfile','forge-humanoid'),'baseMeshOnly':base_mesh_only,
      'referenceAuthority':analysis.get('referenceAuthority'),
      ...
      'inferredRegions':['armpits','undersides','joint locations','occluded surfaces']+([] if base_mesh_only else ['garment/hair semantic segmentation'])+(['side','back'] if count==1 else ['opposite side']),
      'unknownRegions':['hidden accessories','finger separation','true perspective camera','occluded face topology'],
      'assumptions':assumptions,'provenance':provenance,'rig':{},'morphs':[],'sockets':{},'validationMetadata':{'visualApproval':'pending','productionReady':False}}
