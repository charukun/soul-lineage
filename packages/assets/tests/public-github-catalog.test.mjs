...
const materializedRoot=new URL('assets/vendor/public-github/kaykit-medieval-hexagon/',repoRoot);
const manifest=JSON.parse(readFileSync(new URL('MANIFEST.json',materializedRoot),'utf8'));
...
test('every materialized glTF has its pinned local dependencies',()=>{
  for(const asset of Object.values(publicGithubAssetCatalog)){
    ...
    const gltfUrl=new URL(`${asset.category}/${asset.source}.gltf`,materializedRoot);
    const binUrl=new URL(`${asset.category}/${asset.source}.bin`,materializedRoot);
    const textureUrl=new URL(`${asset.category}/hexagons_medieval.png`,materializedRoot);
    assert.ok(existsSync(gltfUrl),`${asset.id}: glTF missing`);
    assert.ok(existsSync(binUrl),`${asset.id}: bin missing`);
    assert.ok(existsSync(textureUrl),`${asset.id}: texture missing`);
