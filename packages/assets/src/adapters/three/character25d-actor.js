...
export async function createCharacter25DActor(THREE,bundle,options={}) {
  ...
  const dispose=()=>{if(disposed)return;disposed=true;object.removeFromParent();for(const attachment of attachments.values())attachment.removeFromParent();attachments.clear();for(const v of views)v.rig.skeleton.dispose();bodyRig.skeleton.dispose();for(const asset of owned)asset.dispose();for(const texture of textures.values())texture.dispose();textures.clear();};
  try {
    for(const [name,appearance] of Object.entries(bundle.appearance)) {
      ...
      for(const layer of data) {
        ...
        material.onBeforeCompile=shader=>{shader.uniforms.characterLag=secondary?uniforms[secondary]:{value:0};shader.vertexShader='attribute float secondaryWeight;\nuniform float characterLag;\n'+shader.vertexShader;shader.vertexShader=shader.vertexShader.replace('#include <skinning_vertex>','#include <skinning_vertex>\ntransformed.x += characterLag * secondaryWeight;');};
        material.customProgramCacheKey=()=> 'character25d-layer-v1';
  ...
  function update(input={}) {
    ...
    object.position.set(proxy.position.x,proxy.position.y,proxy.position.z);body.rotation.y=proxy.yaw;applyPose(bodyRig);
  ...
  function snapshot(){return {schema:bundle.schema,id:bundle.id,position:{...proxy.position},yaw:proxy.yaw,velocity:{...proxy.velocity},speed:actualSpeed,grounded:proxy.grounded,blocked:proxy.blocked,groundNormal:{...proxy.groundNormal},action:motion.action,time:motion.time,view:current,transition,availableViews:views.map(v=>v.name),mirror:false,bodyBones:bodyRig.bones.length,layerMeshes:views.find(v=>v.name===current)?.meshes.length||0,rotations:[...rotations],secondary:Object.fromEntries(Object.entries(springs).map(([k,v])=>[k,v.value])),textures:textures.size,feet:{left:{...proxy.feet.left},right:{...proxy.feet.right}},disposed};}
  return {object,proxy,sockets,update,dispose,snapshot,
    play(name,opts){motion.play(name,opts);},setAction(name,opts){motion.play(name,{restart:false,...opts});},releaseAction(){motion.release();},
    ...
    setEquipment(slot,attachment){if(!sockets[slot])throw new Error('Unknown equipment socket');attachments.get(slot)?.removeFromParent();attachments.delete(slot);if(attachment){sockets[slot].add(attachment);attachments.set(slot,attachment);}},
