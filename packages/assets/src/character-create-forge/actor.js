...
export function createCharacterPackageActor(THREE,gltf,manifest){
  ...
  let action=null,actionName=null,time=0,paused=false,weapon=null,shield=null,held=null,equipment={weapon:null,shield:false};
  ...
  const setAction=name=>{
    ...
    mixer.stopAllAction();root.traverse(n=>n.isSkinnedMesh&&n.skeleton.pose());
  ...
  const api={root,manifest,sockets,helper,mixer,play:setAction,setEquipment,
    ...
    neutral(){mixer.stopAllAction();root.traverse(n=>n.isSkinnedMesh&&n.skeleton.pose());root.rotation.y=0;time=0;actionName='Bind';root.updateMatrixWorld(true);},
