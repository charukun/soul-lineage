...
test('diagnose unchanged develop fixtures against the parent source',()=>{
 const env={...process.env};delete env.NODE_TEST_CONTEXT;const root=process.cwd(),base=mkdtempSync(path.join(tmpdir(),'battle-base-'));
 try{
  ...
  const before=capture(base),after=capture(root);let count=0;console.log('BASE_PHASES',JSON.stringify([...new Set(before.flatMap(r=>r.events).filter(e=>e.type==='stage-start'&&e.source==='hero').map(e=>e.phase))]));console.log('BASE_BREAKS',JSON.stringify(before.flatMap(r=>r.events).filter(e=>e.type==='chain-break')));
  ...
  try{console.log('BASELINE_REVIEW:',execFileSync(process.execPath,['--test','apps/review/tests/johakyu-p7-review.test.mjs'],{cwd:base,env,encoding:'utf8',timeout:60000}));}
