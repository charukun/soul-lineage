...
export function createVillageAuthority({ villageId, mayorId, now = 0, timings = {} }) {
  ...
  return {
    ...
    villageId,
    mayorId,
    primaryHostId: mayorId,
    hostId: mayorId,
    phase: VILLAGE_PHASE.OPEN,
    epoch: 1,
    revision: 0,
    hostLeaseUntil: now + (timings.hostLeaseMs ?? defaults.hostLeaseMs),
    migrationStartedAt: null,
    candidateId: null,
    members: { [mayorId]: { connected: true, eligible: true, lastSeenAt: now, joinOrder: 0 } },
    nextMemberOrder: 1,
    checkpoint: null,
    reason: null,
...
function migrate(state, now, reason) {
  ...
  state.epoch += 1;
  state.candidateId = elect(state);
  if (!state.candidateId) close(state, 'no-eligible-host');
}

export function advanceVillageAuthority(current, event) {
  const state = clone(current);
  const now = event.now;
  if (!Number.isFinite(now)) throw new Error('Event time is required');
  ...
  switch (event.type) {
    case 'join': {
      ...
      const previous = member(event.playerId);
      state.members[event.playerId] = {
        connected: true,
        eligible: event.hostEligible !== false,
        lastSeenAt: now,
    ...
    case 'heartbeat':
      ...
      member(event.playerId).lastSeenAt = now;
      state.hostLeaseUntil = now + state.timings.hostLeaseMs;
      break;
    case 'disconnect':
      if (member(event.playerId)) member(event.playerId).connected = false;
      if (event.playerId === state.hostId) migrate(state, now, 'host-disconnected');
      break;
    case 'connect': {
      if (!event.playerId) throw new Error('Player ID is required');
      const previous = member(event.playerId);
      ...
      if (event.playerId === state.mayorId && state.phase === VILLAGE_PHASE.CLOSED) {
        state.phase = VILLAGE_PHASE.MIGRATING;
        state.reason = 'mayor-recovering';
        state.epoch += 1;
        state.candidateId = state.mayorId;
        state.migrationStartedAt = now;
      }
      break;
    }
    case 'checkpoint':
      ...
      state.revision += 1;
      state.checkpoint = { ...clone(event.checkpoint), revision: state.revision, epoch: state.epoch };
      break;
    case 'migration-ready':
      ...
      Object.assign(state, {
        ...
        candidateId: null,
        migrationStartedAt: null,
        hostLeaseUntil: now + state.timings.hostLeaseMs,
      });
      if (member(event.playerId)) {
        member(event.playerId).connected = true;
        member(event.playerId).lastSeenAt = now;
      }
      break;
    case 'candidate-failed':
      ...
      state.candidateId = elect(state);
      if (!state.candidateId) close(state, 'migration-exhausted');
      break;
    case 'tick':
      ...
      break;
    default:
      throw new Error(`Unknown village event: ${event.type}`);
  }
  return state;
}
