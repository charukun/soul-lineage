...
export function createCheckpointSyncedWorldNode(options={}){
 ...
 let base=null,journal=createCheckpointJournal(),activeSync=null,closed=false;
 ...
 function acceptDelta(senderId,message){
  const snap=base.snapshot();if(senderId!==snap.hostId||message.worldId!==snap.worldId||message.hostId!==snap.hostId||message.epoch!==snap.epoch||message.toRevision!==message.fromRevision+1)return false;
  try{
 ...
 function acceptCatchup(senderId,message){
  const snap=base.snapshot();if(senderId!==snap.hostId||message.worldId!==snap.worldId||message.hostId!==snap.hostId||message.epoch!==snap.epoch||!message.payload||message.payload.epoch!==message.epoch)return false;
  const started=now();
  try{
 ...
 function receive(senderId,message){
  if(closed||!message)return false;
  ...
  if(message.type==='world-sync-request'&&base.isHost){const snap=base.snapshot();if(message.worldId!==snap.worldId||!Object.hasOwn(snap.authority?.members||{},senderId)||!snap.authority.members[senderId].connected)return false;metrics.catchupRequests++;activeSync={peerId:senderId,knownRevision:Number.isSafeInteger(message.knownRevision)?message.knownRevision:0,knownDigest:typeof message.knownDigest==='string'?message.knownDigest:null,sent:false};const handled=base.syncPeer(senderId);if(handled&&!activeSync.sent)sendCatchup(senderId,activeSync.knownRevision,activeSync.knownDigest);activeSync=null;return handled;}
 ...
 function syncPeer(peerId,knownRevision=0,knownDigest=null){const snap=base.snapshot();if(closed||!base.isHost||!Object.hasOwn(snap.authority?.members||{},peerId)||!snap.authority.members[peerId].connected)return false;metrics.catchupRequests++;activeSync={peerId,knownRevision,knownDigest,sent:false};const handled=base.syncPeer(peerId);if(handled&&!activeSync.sent)sendCatchup(peerId,knownRevision,knownDigest);activeSync=null;return handled;}
 ...
 return{seedHost:base.seedHost,adoptAuthority:base.adoptAuthority,hostAdmit:base.hostAdmit,hostDisconnect:base.hostDisconnect,publishCheckpoint,receive,tick:base.tick,syncPeer,gracefulHandoff:base.gracefulHandoff,snapshot,close(){closed=true;base.close();},catchupPayload:(knownRevision,knownDigest=null)=>createCatchupPayload(journal,knownRevision,knownDigest),requestSync,get isHost(){return base.isHost;},get authority(){return base.authority;}};
}
