...
export function createCheckpointSyncedWorldNode(options={}){
 ...
 function catchupMessage(knownRevision=0){const snap=base?.snapshot()||{};const payload=createCatchupPayload(journal,knownRevision);if(payload.kind==='full')metrics.fullFallbacks++;return{type:'world-checkpoint-catchup',worldId:snap.worldId||options.worldId,hostId:snap.hostId||options.selfId,epoch:snap.epoch||journal.epoch,payload};}
 function sendCatchup(peerId,knownRevision=0){const message=catchupMessage(knownRevision);emitMeasured({to:peerId,message},'catchup');if(activeSync&&activeSync.peerId===peerId)activeSync.sent=true;return message;}
 function rawEmit(event){
  ...
  if(activeSync&&event.to===activeSync.peerId){sendCatchup(event.to,activeSync.knownRevision);return;}
 ...
 function syncPeer(peerId,knownRevision=0){metrics.catchupRequests++;activeSync={peerId,knownRevision,sent:false};const handled=base.syncPeer(peerId);if(!activeSync.sent)sendCatchup(peerId,knownRevision);activeSync=null;return handled;}
 ...
 return{seedHost:base.seedHost,adoptAuthority:base.adoptAuthority,hostAdmit:base.hostAdmit,hostDisconnect:base.hostDisconnect,publishCheckpoint,receive,tick:base.tick,syncPeer,gracefulHandoff:base.gracefulHandoff,snapshot,close:base.close,catchupPayload:knownRevision=>createCatchupPayload(journal,knownRevision),requestSync,get isHost(){return base.isHost;},get authority(){return base.authority;}};
}
