...
export function createPresenceTransport({now=()=>performance.now(),highWaterMark=128*1024}={}){
 ...
 function sendObserverSnapshot(observerId,connection,snapshot,time=now()){
  ...
  const signature=json(snapshot?.battle||null),previous=battleSignature.get(observerId);if(signature!==previous){battleSignature.set(observerId,signature);sendReliable(connection,{type:'battle',villageId:snapshot?.villageId,battle:snapshot?.battle||null,serverTime:time});}
 }
