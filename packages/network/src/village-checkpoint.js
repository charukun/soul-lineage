...
export function validateVillageCheckpoint(value, { maxBytes = 1_500_000, maxEntities = 12_000 } = {}) {
  ...
  if (value.session !== undefined && value.session !== null && (typeof value.session !== 'object' || Array.isArray(value.session))) throw new Error('Invalid checkpoint session');
  if (bytes(value) > maxBytes) throw new Error('Checkpoint byte budget exceeded');
  return value;
}

export function createVillageCheckpoint({ worldTimeMs, world, characters = [], npcs = [], randomState = null, session = null }) {
  return validateVillageCheckpoint({
    schemaVersion: 1,
    worldTimeMs,
    world: clone(world),
    characters: clone(characters),
    npcs: clone(npcs),
    randomState: clone(randomState),
