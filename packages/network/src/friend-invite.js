...
export function createFriendVillageInvite({offer,villageId,villageName,issuedAt=Date.now(),ttl=FRIEND_VILLAGE_INVITE_TTL}={}){
 ...
 if(!Number.isFinite(issuedAt)||!Number.isFinite(ttl)||ttl<60_000||ttl>60*60*1000)throw new Error('invite lifetime is invalid');
...
export function decodeFriendVillageInvite(value,{now=Date.now()}={}){
 ...
 if(!Number.isFinite(invite.issuedAt)||!Number.isFinite(invite.expiresAt)||invite.expiresAt<=invite.issuedAt||invite.expiresAt-invite.issuedAt>60*60*1000)throw new Error('友人招待の有効期限が不正です。');
