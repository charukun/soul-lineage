...
test('presence transport sends position on lossy channel and battle on reliable channel',()=>{
 const presence=[],reliable=[];const connection={sendPresence:value=>presence.push(value),send:value=>reliable.push(value),presenceBufferedAmount:()=>0};const transport=createPresenceTransport({now:()=>0});const snap={villageId:'v',players:{self:{x:0,z:0,_presence:{tier:'near',hz:20}},far:{x:80,z:0,_presence:{tier:'far',hz:2}}},battle:{demon:'d',human:'h',winner:null,core:{step(){}}}};transport.sendObserverSnapshot('self',connection,snap,0);assert.equal(presence.length,1);assert.equal(presence[0].type,'snapshot-delta');assert.equal(reliable.length,1);assert.equal(reliable[0].type,'battle');assert.equal('core' in reliable[0].battle,false);snap.battle.core={changed:true};snap.battle.time=3;transport.sendObserverSnapshot('self',connection,snap,10);assert.equal(presence.length,1);assert.equal(reliable.length,1,'internal combat progression must not spam reliable transport');snap.battle.finished=true;snap.battle.winner='demon';transport.sendObserverSnapshot('self',connection,snap,20);assert.equal(reliable.length,2);
});
