...
export function miniatureFocusBand({focusY=.5,inside=false,combat=false,clear=null,fade=null,strength=null}={}){
  const defaultStrength=combat?.42:inside?.6:1;
  return{
    center:clamp(focusY,.25,.75),
    clear:clear==null?(combat?.31:inside?.27:.20):clamp(Number(clear)||0,.04,.48),
    fade:fade==null?(combat?.18:.19):clamp(Number(fade)||0,.04,.48),
    strength:clamp(strength==null?defaultStrength:Number(strength)||0,0,1),
  };
}
...
export function createMiniatureFocus(renderer){
  ...
  function render(scene,camera,focusOptions){
    ...
    band=miniatureFocusBand(focusOptions);
    renderer.info.autoReset=false;renderer.info.reset();
    try{
      if(!supported||!budget.enabled||band.strength<=0){renderer.render(scene,camera);return;}
      const uniforms=composite.uniforms;
      uniforms.focus.value=band.center;uniforms.clearBand.value=band.clear;uniforms.fade.value=band.fade;uniforms.strength.value=band.strength*budget.strength;
  ...
  function snapshot(){return{supported,enabled:supported&&budget.enabled&&band.strength>0,level,sceneSize:[source.width,source.height],blurSize:[blurred.width,blurred.height],blurTaps:budget.taps,extraPasses:supported&&budget.enabled&&band.strength>0?2:0,focus:{...band},drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles};}
