...
export function createPerformanceRecorder({ label = 'runtime', maxSamples = 1800 } = {}) {
  const frames = [], gpu = [], calls = [], triangles = [], memory = [];
  let longFrames = 0, startedAt = Date.now();
  const trim = list => { while (list.length > maxSamples) list.shift(); };
  return {
    sample({ frameMs, gpuMs = null, drawCalls = null, triangles: tris = null, textureBytes = null } = {}) {
    ...
    reset() { frames.length = gpu.length = calls.length = triangles.length = memory.length = 0; longFrames = 0; startedAt = Date.now(); },
  };
}
