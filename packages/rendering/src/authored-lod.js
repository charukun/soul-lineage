...
import { installStylizedGeometryLOD, stylizedLodDistance } from './stylized-art.js';
...
export function installAuthoredStylizedLOD(root, profileId, { minTriangles = 96 } = {}) {
  ...
  for (const levels of groups.values()) {
    ...
    const before = base.onBeforeRender, world = new Vector3(), cameraWorld = new Vector3(), direction = new Vector3(), focus = new Vector3();
    ...
    base.onBeforeRender = function authoredStylizedLOD(renderer, scene, camera, renderGeometry, material, group) {
      ...
      const distance = stylizedLodDistance(base, camera, world, cameraWorld, direction, focus);
