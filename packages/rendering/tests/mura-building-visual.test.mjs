...
test('the default village visibly contains shared house silhouettes',()=>{
  ...
  assert.equal(resolveMuraHouseVisual(mayor.kind,mayor.material,mayor.level)?.type,'cottage');
  assert.equal(resolveMuraHouseVisual(guard.kind,guard.material,guard.level)?.type,'tallhouse');
...
test('shared house visuals fit the existing MURA footprint without changing its contract',()=>{
  ...
  for(const [kind,material,level] of [['mayor','base',1],['guardhome','base',1],['tent','base',1],['home','base',1],['home','timber',1],['home','stone',2],['lodge','base',1],['clanManor','stone',1]]){
