...
export function appendJournalEntry(journal,{revision,updatedAt,ops}){if(!Number.isSafeInteger(revision)||revision<1)throw new Error('Invalid journal revision');const next=journal&&journal.version===1?journal:{version:1,baseRevision:Math.max(0,revision-1),entries:[]};next.entries.push({revision,updatedAt,ops});return next;}
export function replayJournal(base,journal,{baseRevision=0}={}){if(!journal)return{payload:clone(base),revision:baseRevision};if(journal.version!==1||journal.baseRevision!==baseRevision)throw new Error('Journal base revision mismatch');let payload=clone(base),revision=baseRevision;for(const entry of journal.entries){if(entry.revision<=revision)continue;if(entry.revision!==revision+1)throw new Error('Journal revision gap');payload=applyIncrementalPatch(payload,entry.ops);revision=entry.revision;}return{payload,revision};}
export function journalBytes(journal){return new TextEncoder().encode(JSON.stringify(journal||null)).byteLength;}
export function shouldCompactJournal(journal,{maxEntries=24,maxBytes=900_000}={}){return(journal?.entries?.length||0)>=maxEntries||journalBytes(journal)>=maxBytes;}
