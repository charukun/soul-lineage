...
Use metadata → changed filenames / failed job → necessary patch / range. CI log retrieval is capped by the shared context ledger: 3 unique excerpts or 96 KiB total. On budget exhaustion, do not switch ranges/jobs; summarize the evidence and hand off instead of mining more logs or polling.
