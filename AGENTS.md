...
- Routine Fast DEV work must not add task-specific test files merely to exercise them in GitHub Actions, and must not retain one-off validation tests solely for future Actions reuse. Prefer existing lightweight checks; when no task-specific hosted work is materially necessary, use `Astra-Validation: none` so the merge-owning run performs only the Fast DEV contract/freshness path. Add or select tests/builds only when they are materially required for correctness or explicitly requested by the user.
