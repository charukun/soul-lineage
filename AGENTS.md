...
- Source bloat and structural debt follow `docs/CODE_HEALTH.md`. Fast validation rejects material new size/complexity regressions without retroactively blocking existing hotspots; the reusable Code Health maintenance job is invoked from the trusted develop Integration/Rescue control plane and may bootstrap at most one focused RINNE Dispatch refactor PR at a time, with a bounded cooldown. Do not rely on a develop-only scheduled workflow while `main` is the default branch, and do not weaken thresholds, tests, browser assertions, or Integration gates merely to make a change pass.
...
- The `RINNE Dispatch Handoff` GitHub workflow validates the eligible Draft PR and records `dispatch/handoff=success`; it does not invoke a paid Platform API implementation worker. A configured ChatGPT Work GitHub PR event task owns implementation, affected fast validation, push and Ready transition on the same branch/PR. The Draft PR / branch / commit remain the recovery source of truth; do not add a separate task ID, task database, or second Integration queue.
