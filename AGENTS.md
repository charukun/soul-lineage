...
Ready ends the interactive implementation work. Running / Queued / Pending CI, browser checks, merge automation, and DEV publication must not keep the worker alive. Do not watch, sleep, or poll for completion. The repository automation continues from the same Ready PR: exact-head CI success immediately enters the serialized expected-head merge lane, with repair only for stale/conflicting heads. The detailed boundary and mandatory screenshot/video presentation in completion reports are canonical in [`docs/RINNE_PROJECT_EXECUTION_POLICY.md`](docs/RINNE_PROJECT_EXECUTION_POLICY.md); do not fetch the full policy unconditionally. Read the relevant evidence section for final delivery.
