...
- For GitHub Actions result inspection, use `npm run actions:summary -- --repo <owner/repo> --sha <exact-sha> [--pr <number>]` as the first source of truth. Its default mode is status-first: read the exact-head `astra/fast-dev-contract`, `astra/focused-validation`, and `astra/merge-freshness` commit statuses plus minimal PR/freshness metadata, and do not enumerate Actions runs/jobs/artifacts or fetch logs when those canonical statuses are healthy. If a canonical status is `error`/`failure`, inspect only the single run referenced by that status and fetch logs only for failed jobs. Use `--full` only when the task explicitly needs DEV Publish, Browser Review, artifacts/evidence, historical run detail, or a failure remains ambiguous. Never poll DEV publication through `actions:summary` after merge.
