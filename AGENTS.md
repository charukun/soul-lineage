...
2. **Validate once, merge safely** — when implementation is coherent, re-read current `develop`, but do not reconcile solely because its SHA advanced. Routine Fast DEV uses a connector-native exact-bytes proof: validate the final source bytes before blob creation, write those same bytes through Git Data, then verify the resulting PR head, mergeability, changed scope, and latest `develop` through the GitHub Connector. No GitHub Actions runner is started for routine `Astra-Validation: none` / `Astra-Check` work. Explicit test/build/browser/DCC/asset/specialist work uses `[astra-heavy-validation]` and the hosted exact-head runner. Independent mergeable drift reuses the proof; conflict or impact overlap requires reconciliation and a new proof. Then mark Ready and merge to `develop` in the same task/session.
...
- **Routine Connector round-trip budget:** organize Connector work into phases, not a fetch/edit/check loop. Default phases are: (1) one batched source/context acquisition, (2) validate final source bytes and perform one coherent Git Data write (`blobs -> tree -> commit -> ref`), and (3) one connector-native Ready/merge race-guard phase that verifies PR head, current `develop`, mergeability, and affected-scope freshness before immediate merge. Independent calls inside one orchestrated batch are one phase. Routine work must not start or inspect Actions.
...
- Routine Fast DEV has no Actions result to inspect. Use one connector-native merge-window orchestration after Ready: read PR head + current `develop` + mergeability/freshness coordinates and immediately merge with `expected_head_sha`; then fetch only the resulting merge commit once to confirm its parents. Heavy/specialist validation remains status-first through `actions:summary`.
