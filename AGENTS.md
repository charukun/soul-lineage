...
- Integration Control Plane v2 is defined in `docs/INTEGRATION_CONTROL_PLANE_V2.md`. The registered `deploy.yml` is the compatibility gateway and DEV/Production publisher; develop-side merge/Rescue logic lives in the reusable `integration-controller.yml`. Do not directly dispatch a develop-only workflow that is absent from default branch `main`. Wakeups must coalesce before controller work, and trusted control-plane Fast Lane applies only when every changed path is classified as control-plane scope.
