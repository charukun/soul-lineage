...
- The Fast DEV Actions workload is a hard ceiling and the normal `develop` push workload is **zero**. No workflow may automatically run install/build/test/publish/browser/artifact/notification work from a `develop` push. The only merge-owning Actions path is the explicitly armed final-head `Astra Work Validation` on the work branch. Manual distribution and Production workflows remain outside Fast DEV. The existing Fast DEV Actions workload is a ceiling, not a template to extend. Without an explicit user request to change the Fast DEV contract itself, never add an Actions job/step, increase the number of test cases executed by Actions, change a Fast DEV lifecycle command (`predev`, `prebuild`, `build`, `postbuild`), or add network/materialization work to the validation path.
