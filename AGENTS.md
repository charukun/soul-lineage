...
For a `develop` implementation task, **Ready for review is transient, not a success terminal**. Never finish or report success as `READY_FOR_INTEGRATION`, “Ready handoff”, or “Integration wait”. Normal success requires the same task worker to merge the exact validated PR head and record the resulting merge commit (`MERGED_TO_DEVELOP`). If a real hold, permission failure, unresolved dependency, or other documented blocker prevents merge, report `FAILED` with repository / branch / exact head / PR / reason instead of inventing an intermediate terminal state.
...
Before claiming that a task cannot be completed in the current session/environment, follow the **実行不能宣言の禁止と代替経路の強制** section in [`docs/RINNE_PROJECT_EXECUTION_POLICY.md`](docs/RINNE_PROJECT_EXECUTION_POLICY.md). A missing direct tool or one failed transport is not a capability verdict; exhaust the applicable repository/API/browser/artifact/Codespaces route first, and only then report a concrete `FAILED` blocker with evidence.
