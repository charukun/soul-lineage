...
1. **Implement** — start from latest `develop` and construct the coherent final tree through the connected GitHub Connector before creating a routine branch. For routine Fast DEV, create the final commit detached from refs, then create the dedicated work branch directly at that commit and open the PR non-draft. Do not create an empty/preflight branch, do not move the branch twice, and do not use Draft/Ready choreography unless recovery or heavy/specialist work actually needs it.
