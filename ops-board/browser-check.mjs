...
const fixture = {
  ...
  pullRequests: { normal: ['Draft', 'Ready', 'Merged', 'Closed'].map((state, i) => ({ number: i + 1, title: `検証用タスク ${i + 1}`, detail: '変更概要と対象アプリの確認', state, updatedAt: rawDate, url: `https://github.com/charukun/soul-lineage/pull/${i+1}`, targets: [{ id: 'rinne', label: '百年転生' }], targetsStatus: 'ready', targetsComplete: true })), visualReview: [] },
  applications: ['rinne','village','demon','portal','ops-board','visual-review'].map((id,i) => ({ id, name: ['百年転生','叡智豊満','魔物側','WAYFINDER','開発状況ボード','Visual Review Lab'][i], kind: i < 3 ? 'game' : 'tool', targets: [{ id, label: '開発版', state: i === 3 ? 'unknown' : 'success', commit: 'a'.repeat(40), deployedAt: rawDate, url: base, source: '検証データ' }] })),
