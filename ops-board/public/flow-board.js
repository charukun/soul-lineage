...
function reconciliationBlock(plan){
  ...
  const headline=node('p','rs-note',`Reconcile: Ready ${plan.totalReady||counts.ready||0} · writer ${counts.writer||0} · Train ${counts.trains||0}/${counts.trainMembers||0} PR · repair ${counts.repair||0} · active ${counts.active||0} · blocked ${counts.blocked||0} · deferred ${counts.deferred||0}`);
