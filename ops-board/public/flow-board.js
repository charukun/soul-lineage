...
function render(state){
  ...
  if(!state){root.append(node('p','empty','Integrationの状態を取得できていません'));return;}
  ...
  const status=node('div'); status.append(node('span','rs-flow-kicker','INTEGRATION'),node('strong','',headline));
  head.append(status,node('span','rs-flow-waiting',`Ready ${readyCount}件`));
  overview.append(head);

  const facts=node('div','rs-flow-facts');
  facts.append(
    fact('EXACT HEAD',`${fastCheck.length}件`,failedChecks?`失敗 ${failedChecks}件 · 対象PRだけWORKINGへ戻します`:fastCheck.length?'Ready exact headを確認中':'待ちなし',failedChecks?'attention':''),
    fact('MERGE',`${mergeLane.length}件`,mergeLane.length?'single writerが順次developへ反映':'待ちなし'),
    fact('DEVELOP → DEV',`${devAhead} commit`,devFollowing?'最新developだけを公開へ追従':'DEVはdevelopに追従済み'),
    fact('REPAIR',`${repairWaiting}件`,`${repairActive} worker active · READY内の後処理`,humanRequired?'attention':'')
  );
  overview.append(facts);
  const blockedNote=holds.length?` 明示保留は ${holds.length}件あり、そのPRだけ停止しています。`:'';
  overview.append(node('p','rs-flow-explain',`ここはREADY以降の機械処理です。browser・DEV公開・Repairは後追いし、独立したReady PRのmergeを止めません。${blockedNote}`));
