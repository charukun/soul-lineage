...
export function rescueView(state, now = Date.now()) {
  ...
  return { available: true, status, generatedAt: state.updatedAt, staleMs: config.staleMs,
    ...
    waves: state.waves.slice(-5).reverse().map(w => ({ ...w, repaired: w.returned?.length ?? w.rescueIds.filter(id => recent.some(r => r.rescueId === id)).length,
    ...
    throughput: { rescued: count(['RETURNED_TO_INTEGRATION']), merged: count(['MERGED']), manual: count(['FAILED_MANUAL']), retrying: counts.retry, windowHours: 24, retainedEvents: activity.length },
    activity: activity.slice(-30).reverse().map(e => ({ at: e.at, pr: e.pr, workerId: e.workerId, type: e.type, action: safeString(e.action) })),
