import { REPOSITORY, ACTIVE, RETURNED, SEMANTIC_WORK, rescueConfig } from '../scripts/integration-rescue-policy.mjs';
...
export function rescueView(state, now = Date.now()) {
  if (!state || state.schema !== 1 || state.repository !== REPOSITORY) return { available: false, status: 'UNAVAILABLE', reason: 'Rescue状態をまだ取得していません', workers: [], semantic: [], queue: [], recent: [], manual: [], waves: [], activity: [] };
  ...
  const records = Object.values(state.records).map(r => ({
    ...
    deliveryKind: repairEvidence(r) ? 'repaired' : r.state === 'AWAITING_PUSH' ? 'staged' : SEMANTIC_WORK.has(r.state) ? 'semantic' : r.mode === 'reevaluate' && r.returnedAt ? 'reevaluated' : 'observed',
    rescueId: r.rescueId || null, workerId: r.claimedBy || null, semanticWorkerId: safeString(r.semanticWorkerId), wave: r.wave || null, lease: Boolean(r.lease), semanticLease: Boolean(r.semanticLease),
    ...
    semanticReason: safeString(r.semanticReason, 400), semanticRequestedAt: r.semanticRequestedAt || null, semanticResolvedAt: r.semanticResolvedAt || null,
    ...
    resolution: safeString(r.resolution), semanticResolution: safeString(r.semanticResolution), failureReason: safeString(r.failureReason, 400), failures: (r.failures || []).slice(-3),
  ...
  const workers = records.filter(r => r.lease), semantic = records.filter(r => SEMANTIC_WORK.has(r.state));
  const queue = records.filter(r => ['DETECTED', 'QUEUED', 'BLOCKED_BY_RESCUE', 'FAILED_RETRYABLE'].includes(r.state));
  ...
  const counts = { active: workers.filter(r => !r.heartbeatStale && ACTIVE.has(r.state)).length, reserved: workers.length, max: config.maxConcurrency,
    ...
    semantic: semantic.length, semanticWorking: semantic.filter(r => r.state === 'SEMANTIC_WORKING').length,
  ...
  const status = !state.coordinator?.configured ? 'CONFIGURATION_REQUIRED' : counts.manual || counts.stale ? 'ATTENTION' : counts.active || counts.semanticWorking ? 'ACTIVE' : counts.queued || counts.blocked || counts.returned || counts.awaitingPush || counts.semantic ? 'WAITING' : 'ALL_CLEAR';
  return { available: true, status, generatedAt: state.updatedAt, staleMs: config.staleMs,
    ...
    counts, workers, semantic: semantic.sort((a, b) => Date.parse(a.semanticRequestedAt || a.updatedAt) - Date.parse(b.semanticRequestedAt || b.updatedAt)),
    queue: queue.sort((a, b) => (b.priority?.score || 0) - (a.priority?.score || 0)), manual, recent,
    ...
    throughput: { rescued: count(['RETURNED_TO_INTEGRATION'], repairedEvent), merged: count(['MERGED'], repairedEvent), semantic: count(['SEMANTIC_WORK_CLAIMED']), manual: count(['FAILED_MANUAL']), retrying: counts.retry, windowHours: 24, retainedEvents: activity.length },
