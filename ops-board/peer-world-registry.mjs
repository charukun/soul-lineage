...
export function joinPeerWorldRoom(value,id,input={},now=Date.now()){
  ...
  const peerId=text(input.peerId,128);if(!peerId)throw new Error('invalid_peer');const joinId=roomId(),guestToken=token();room.joins[joinId]={joinId,peerId,app:text(input.app||'village',24),guestToken,capability:clone(input.capability||null),events:[],createdAt:new Date(now).toISOString()};event(room,'join',{joinId,peerId,app:room.joins[joinId].app,capability:room.joins[joinId].capability});touch(room,now);return{state,result:{joinId,guestToken,room:publicRoom(room)}};
}
export function readHostEvents(value,id,auth,after=0,now=Date.now()){
  const state=prunePeerWorldRegistry(value,now),room=getRoom(state,id,now);assertHost(room,auth);touch(room,now);const events=room.hostEvents.filter(row=>row.seq>integer(after));return{state,result:{events,next:events.at(-1)?.seq??integer(after)}};
}
export function postPeerWorldOffer(value,id,joinId,auth,offer,now=Date.now()){
  const state=prunePeerWorldRegistry(value,now),room=getRoom(state,id,now);assertHost(room,auth);const join=room.joins[text(joinId,32)];if(!join)throw new Error('join_not_found');guestEvent(room,join,'offer',{offer:validSignal(offer)});touch(room,now);return{state,result:{accepted:true}};
}
...
export function postPeerWorldAnswer(value,id,joinId,auth,answer,now=Date.now()){
  const state=prunePeerWorldRegistry(value,now),room=getRoom(state,id,now),join=room.joins[text(joinId,32)];if(!join)throw new Error('join_not_found');assertGuest(join,auth);event(room,'answer',{joinId:join.joinId,peerId:join.peerId,answer:validSignal(answer)});touch(room,now);return{state,result:{accepted:true}};
}
export function updatePeerWorldTelemetry(value,id,auth,input={},now=Date.now()){
  const state=prunePeerWorldRegistry(value,now),room=getRoom(state,id,now);assertHost(room,auth);const phase=['open','migrating','closed'].includes(input.phase)?input.phase:'open';
  room.telemetry={phase,hostRef:text(input.hostRef||room.hostId,12),hostScore:Number.isFinite(input.hostScore)?integer(input.hostScore,0,110):null,peers:integer(input.peers,1,999),checkpointRevision:integer(input.checkpointRevision,0),epoch:integer(input.epoch,1),quorum:input.quorum&&Number.isFinite(input.quorum.acked)&&Number.isFinite(input.quorum.total)?{acked:integer(input.quorum.acked,0,999),total:integer(input.quorum.total,0,999)}:null,migrationMs:Number.isFinite(input.migrationMs)?integer(input.migrationMs,0,120000):null,detectionMs:Number.isFinite(input.detectionMs)?integer(input.detectionMs,0,120000):null,sloPass:input.sloPass!==false,splitBrainPrevented:integer(input.splitBrainPrevented,0,1_000_000)};touch(room,now);return{state,result:{room:publicRoom(room)}};
}
