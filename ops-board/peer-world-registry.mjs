...
function capability(value){const result={};for(const key of ['foreground','charging','saveData','stable'])if(typeof value?.[key]==='boolean')result[key]=value[key];for(const key of ['score','batteryLevel','hardwareConcurrency','deviceMemory','rttMs','downlinkMbps','frameP95Ms','gpuP95Ms'])if(Number.isFinite(value?.[key]))result[key]=Math.min(5000,Math.max(0,value[key]));return result;}
...
export function publicPeerWorldSnapshot(value,now=Date.now()){
  const state=prunePeerWorldRegistry(value,now),rooms=Object.values(state.rooms).map(room=>{const {roomId,worldId,label,...telemetry}=publicRoom(room);return{...telemetry,label:'共有セッション'};}).sort((a,b)=>Date.parse(b.updatedAt)-Date.parse(a.updatedAt));
...
export function createPeerWorldRoom(value,input={},now=Date.now()){
  ...
  const id=roomId(),hostToken=token(),inviteToken=token(),worldId=text(input.worldId,96),hostId=text(input.hostId,128);if(!worldId||!hostId)throw new Error('invalid_room');
  const room={roomId:id,worldId,inviteToken,purpose:input.purpose==='standby'?'standby':'visitor',label:text(input.label||'共通村',64),hostId,hostToken,capability:capability(input.capability),createdAt:new Date(now).toISOString(),updatedAt:new Date(now).toISOString(),expiresAt:now+PEER_WORLD_TTL_MS,nextSeq:0,hostEvents:[],joins:{},telemetry:{phase:'open',peers:1,epoch:1,checkpointRevision:0,sloPass:true,splitBrainPrevented:0}};state.rooms[id]=room;return{state,result:{room:publicRoom(room),hostToken,inviteToken}};
}
...
export function listPeerWorldRooms(value,now=Date.now()){return{state:prunePeerWorldRegistry(value,now),result:{rooms:[]}};}
export function joinPeerWorldRoom(value,id,input={},now=Date.now()){
  const state=prunePeerWorldRegistry(value,now),room=getRoom(state,id,now);if(!input.inviteToken||input.inviteToken!==room.inviteToken)throw new Error('unauthorized');if(input.hostEligible===true&&(room.purpose!=='standby'||input.app!=='village'))throw new Error('host_eligibility_forbidden');if(Object.keys(room.joins).length>=MAX_JOINS)throw new Error('join_limit');
  const peerId=text(input.peerId,128);if(!peerId)throw new Error('invalid_peer');const joinId=roomId(),guestToken=token();room.joins[joinId]={joinId,peerId,app:text(input.app||'village',24),hostEligible:input.hostEligible===true,guestToken,capability:capability(input.capability),events:[],createdAt:new Date(now).toISOString(),offeredAt:null,answeredAt:null};event(room,'join',{joinId,peerId,app:room.joins[joinId].app,hostEligible:room.joins[joinId].hostEligible,capability:room.joins[joinId].capability});touch(room,now);return{state,result:{joinId,guestToken,room:publicRoom(room)}};
}
...
export function postPeerWorldOffer(value,id,joinId,auth,offer,now=Date.now()){
  const state=prunePeerWorldRegistry(value,now),room=getRoom(state,id,now);assertController(room,auth);const join=room.joins[text(joinId,32)];if(!join)throw new Error('join_not_found');const signal=validSignal(offer);guestEvent(room,join,'offer',{offer:signal});join.offeredAt=new Date(now).toISOString();touch(room,now);return{state,result:{accepted:true}};
}
...
export function postPeerWorldAnswer(value,id,joinId,auth,answer,now=Date.now()){
  const state=prunePeerWorldRegistry(value,now),room=getRoom(state,id,now),join=room.joins[text(joinId,32)];if(!join)throw new Error('join_not_found');assertGuest(join,auth);if(!join.offeredAt)throw new Error('offer_required');const signal=validSignal(answer);join.answeredAt=new Date(now).toISOString();event(room,'answer',{joinId:join.joinId,peerId:join.peerId,answer:signal});touch(room,now);return{state,result:{accepted:true,hostCursor:room.nextSeq,controller:join.hostEligible===true}};
}
