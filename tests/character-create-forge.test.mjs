...
test('Forge keeps observed views, exports real skin/clips, and registers only validated candidates',{timeout:180000},()=>{
  ...
  try{
    ...
    for(const view of ['front','side','back']){assert.ok(multi.spec.textureProjection.sampleContributions[view]>100);assert.ok(multi.report.comparisons[view]);assert.ok(multi.report.comparisons[view].silhouetteMismatch<.8);}
