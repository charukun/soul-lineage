...
test('signaling registry exchanges offer and answer without exposing secrets publicly',()=>{
  ...
  step=joinPeerWorldRoom(state,room.roomId,{peerId:'peer-B',app:'village',hostEligible:true,capability:{score:81}},t+1);state=step.state;const {joinId,guestToken}=step.result;
...
test('answered host-eligible standby can control signaling after authority migration',()=>{
  const t=3_000_000;let state=emptyPeerWorldRegistry();let step=createPeerWorldRoom(state,{worldId:'village-3',hostId:'host-A'},t);state=step.state;const {room,hostToken}=step.result;
  step=joinPeerWorldRoom(state,room.roomId,{peerId:'peer-B-abcdef',app:'village',hostEligible:true},t+1);state=step.state;const {joinId,guestToken}=step.result;
  ...
  let guest=joinPeerWorldRoom(state,room.roomId,{peerId:'peer-C',app:'village',hostEligible:true},t+7);state=guest.state;
...
test('non-host-eligible guest never receives signaling controller authority',()=>{
  const t=4_000_000;let state=emptyPeerWorldRegistry();let step=createPeerWorldRoom(state,{worldId:'village-4',hostId:'host-A'},t);state=step.state;const {room,hostToken}=step.result;
  step=joinPeerWorldRoom(state,room.roomId,{peerId:'rinne-client',app:'rinne',hostEligible:false},t+1);state=step.state;const {joinId,guestToken}=step.result;
  step=postPeerWorldOffer(state,room.roomId,joinId,hostToken,'offer-abcdefghijklmnopqrstuvwxyz',t+2);state=step.state;
