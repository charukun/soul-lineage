...
test('coalesced DEV publish returns every merged develop PR between previous and current public snapshots', async () => {
  const calls = [];
  const request = async (url, init = {}) => {
    ...
    if (url.includes(`/compare/${previous}...${current}`)) return response({
      ...
      commits: [
        ...
        { sha: merge874, commit: { message: 'Merge pull request #874 from charukun/feat/a\n\nfeat: a' } },
