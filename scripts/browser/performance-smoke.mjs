...
try{
 ...
 const snapshot=await page.evaluate(name=>window[name].snapshot(),globalName);const report={app,viewport:{width:390,height:844},capturedAt:new Date().toISOString(),...snapshot};
 writeFileSync(resolve(output,`${app}.json`),JSON.stringify(report,null,2));console.log('PERFORMANCE LAB CAPTURED',JSON.stringify({app,samples:report.performance.samples,frameP95:report.performance.frame.p95Ms,gpuP95:report.performance.gpu.p95Ms,drawCalls:report.performance.drawCalls.p95}));
 await context.close();
