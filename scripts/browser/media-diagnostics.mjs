...
async function capturePerformanceSnapshot(page){
  ...
  const result=await page.evaluate(()=>{
    ...
    return source?.snapshot?{app,snapshot:source.snapshot(),resilience:resilience?.snapshot?.()||null}:null;
  }).catch(()=>null);
  ...
  writeFileSync(resolve(dir,`${result.app}-performance.json`),JSON.stringify({app:result.app,capturedAt:new Date().toISOString(),...result.snapshot,resilience:result.resilience},null,2));
