...
export async function dedupeSite(root, { minBytes = 4096 } = {}) {
  ...
  let linkedFiles = 0, bytesSaved = 0;
  ...
  for (const bucket of sizeBuckets.values()) {
    ...
    for (const file of bucket) {
      ...
      const current = await lstat(file.path), source = await lstat(first.path);
      if (current.ino === source.ino && current.dev === source.dev) continue;
      await unlink(file.path);
      await link(first.path, file.path);
      linkedFiles++; bytesSaved += file.size; hashes.set(file.path, first.path);
    }
  }
...
async function main() {
  ...
  console.log(JSON.stringify(await dedupeSite(root), null, 2));
}
