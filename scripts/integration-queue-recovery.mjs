...
export function exactDevelopAncestor(comparison, head, develop) {
  return comparison?.base_commit?.sha === head &&
    comparison?.merge_base_commit?.sha === head &&
    comparison?.head_commit?.sha === develop &&
...
export function rescueRuntimeAudit(state, develop) {
  if (!state || state.schema !== 1 || state.repository !== REPOSITORY) {
    return {
      ok: false,
      pending: true,
      reason: 'Rescue state unavailable',
      expected: RESCUE_RUNTIME_TARGET,
      actual: null,
      mismatches: [],
    };
  }
  ...
  const actual = Object.fromEntries(
    Object.keys(RESCUE_RUNTIME_TARGET).map(key => [key, state.config?.[key] ?? null]));
  const mismatches = [];

  for (const [key, spec] of Object.entries(RESCUE_RUNTIME_TARGET)) {
    const value = actual[key];
    const ok = spec.exact !== undefined
      ? value === spec.exact
  ...
  if (observedDevelop !== develop) {
    return {
      ok: false,
      pending: true,
      reason: 'Waiting for Rescue coordinator on current develop',
      expected: RESCUE_RUNTIME_TARGET,
      actual,
      mismatches,
      observedDevelop,
  ...
  return {
    ...
    reason: mismatches.length
      ? `Runtime config outside safe adaptive bounds: ${mismatches.map(item => `${item.key}=${item.actual}`).join(', ')}`
      : 'Runtime config is inside adaptive safety bounds',
    expected: RESCUE_RUNTIME_TARGET,
    actual,
    mismatches,
    observedDevelop,
...
async function recordRuntimeAudit(c, develop, audit) {
  ...
  const description = audit.pending
    ? 'Waiting for Rescue runtime config on current develop'
    : audit.ok
...
async function supersessionSafety(c, expected) {
  ...
  for (const number of dependencies(pr.body || '')) {
    ...
    if (!(dependency.merged || dependency.merged_at) ||
        dependency.base?.ref !== 'develop' ||
        dependency.base?.repo?.full_name !== REPOSITORY) return false;
  }
  return true;
}
...
async function closeSuperseded(c, pr, develop, description) {
  ...
  const finalPr = await c.api('GET', `${c.root}/pulls/${pr.number}`);
  const finalBranch = await c.api('GET', `${c.root}/branches/develop`);
  if (finalPr.head.sha !== pr.head.sha ||
      finalBranch.commit.sha !== develop ||
      finalPr.state !== 'open' ||
      finalPr.draft ||
      finalPr.body !== fresh.body ||
      JSON.stringify(finalPr.labels) !== JSON.stringify(fresh.labels)) return false;

  await c.api('POST', `${c.root}/statuses/${pr.head.sha}`, {
    state: 'success',
    context: 'integration/queue',
    description,
    target_url: finalPr.html_url,
  });
...
async function treeEntries(c, commitSha) {
  ...
  if (commit?.sha !== commitSha || !/^[0-9a-f]{40}$/.test(commit.tree?.sha || '')) {
    throw new Error('SUPERSESSION_TREE_COMMIT_INVALID');
  }
  ...
  if (tree?.sha !== commit.tree.sha || tree.truncated !== false || !Array.isArray(tree.tree)) {
    throw new Error('SUPERSESSION_TREE_INCOMPLETE');
  }

  const entries = new Map();
  for (const item of tree.tree) {
    if (item.type !== 'tree') entries.set(item.path, `${item.mode}:${item.type}:${item.sha}`);
  }
  return entries;
}
...
export async function retireContentSuperseded(c, pr, develop) {
  ...
  const [headEntries, developEntries] = await Promise.all([
    treeEntries(c, pr.head.sha),
    treeEntries(c, develop),
  ]);
...
export async function reconcileStack(c, pr, develop, { write = false } = {}) {
  ...
  if (!deps.length || pr.head.repo?.full_name !== REPOSITORY || pr.base?.ref !== 'develop') {
    return { state: 'not-stacked' };
  }

  const depStates = await Promise.all(deps.map(number => c.api('GET', `${c.root}/pulls/${number}`)));
  if (!depStates.every(dep =>
    (dep.merged || dep.merged_at) &&
    dep.base?.ref === 'develop' &&
    dep.base?.repo?.full_name === REPOSITORY)) {
...
function quarantineSet(state) {
  return new Set(Object.values(state?.records || {})
    .filter(record => quarantineDecision(record).quarantined)
...
export async function recoverQueue(c, { now = Date.now(), limit = 24, budgetMs = 150000 } = {}) {
  ...
  const state = await readRescueState(c).catch(error => {
    report.errors.push({ pr: null, reason: `RESCUE_STATE: ${error.message}` });
    return null;
  });
  ...
  const start = report.flowControl.mode === 'BURN_DOWN'
    ? 0
    : (Math.floor(now / 600000) % windows) * limit;
  let semanticBudget = 4;
  let stackBudget = 4;

  for (const snapshot of ready.slice(start, start + limit)) {
    if (Date.now() - started > budgetMs ||
        (c.metrics?.().requests || 0) > 140 ||
    ...
    try {
      ...
      if (stale) {
        report.staleReview.push({
          pr: pr.number,
          head: pr.head.sha,
          createdAt: pr.created_at || null,
          reason: 'STALE_READY_AI_REVIEW_CANDIDATE',
        });
      }
      if (develop && stale && semanticBudget > 0) {
        semanticBudget--;
        if (await retireContentSuperseded(c, pr, develop)) {
          report.semanticSuperseded.push({ pr: pr.number, head: pr.head.sha, develop });
          continue;
        }
      }
      ...
      if (develop && stackBudget > 0 && dependencies(pr.body || '').length) {
        ...
        if (candidate.state === 'ready-to-reconcile') {
          report.stackCandidates.push({ pr: pr.number, ...candidate });
        }
      }
      ...
      if (await fastGate(c, pr)) {
        ...
        const missed = !queue ||
          /fast gate|another check|cancelled.*CI|develop verification is running|deferred|time budget|HTTP (429|5\d\d)/i.test(queue.description || '');
      ...
      report.errors.push({ pr: snapshot.number, reason: error.message });
    }
  }
  ...
  if (develop) {
    ...
    report.configAudit = {
      ok: false,
      pending: true,
      reason: 'Current develop snapshot unavailable; audit deferred',
      expected: RESCUE_RUNTIME_TARGET,
      actual: null,
      mismatches: [],
    };
...
async function main() {
  if (process.env.GITHUB_REPOSITORY !== REPOSITORY || process.env.GITHUB_REF !== 'refs/heads/develop') {
    throw new Error('QUEUE_RECOVERY_TRUSTED_DEVELOP_ONLY');
  }
