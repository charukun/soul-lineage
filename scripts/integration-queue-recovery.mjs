...
import { pullEvidence } from './integration-rescue-store.mjs';
...
export const RESCUE_RUNTIME_TARGET = Object.freeze({
  maxConcurrency: { min: 3, max: 6 },
  maxEvaluations: { min: 12, max: 24 },
  scanMinMs: { exact: 60_000 },
  queueStallMs: { exact: 300_000 },
  retryMs: { exact: 120_000 },
});
...
export function rescueRuntimeAudit(state, develop) {
  ...
  return { ok: mismatches.length === 0, pending: false,
    reason: mismatches.length ? `Runtime config outside safe adaptive bounds: ${mismatches.map(item => `${item.key}=${item.actual}`).join(', ')}` : 'Runtime config is inside adaptive safety bounds',
...
async function recordRuntimeAudit(c, develop, audit) {
  ...
  const description = audit.pending ? 'Waiting for Rescue runtime config on current develop' : audit.ok
    ? 'Rescue runtime inside adaptive bounds: 3-6 workers / 12-24 eval'
    : 'Rescue runtime config outside adaptive safety bounds';
  await c.api('POST', `${c.root}/statuses/${develop}`, { state, context: 'integration-rescue/config', description, target_url: `https://github.com/${REPOSITORY}/actions/workflows/deploy.yml` });
}

async function supersessionSafety(c, expected) {
  const evidence = await pullEvidence(c, expected.number);
  const pr = evidence.pr;
  if (pr.head.sha !== expected.head.sha || manualReason(pr, evidence)) return false;
  for (const number of dependencies(pr.body || '')) {
    const dependency = await c.api('GET', `${c.root}/pulls/${number}`);
    if (!(dependency.merged || dependency.merged_at) || dependency.base?.ref !== 'develop' || dependency.base?.repo?.full_name !== REPOSITORY) return false;
  }
  return true;
}

async function closeSuperseded(c, pr, develop, description) {
  const fresh = await c.api('GET', `${c.root}/pulls/${pr.number}`), branch = await c.api('GET', `${c.root}/branches/develop`);
  if (fresh.head.sha !== pr.head.sha || branch.commit.sha !== develop || manualReason(fresh)) return false;
  if (!await supersessionSafety(c, fresh)) return false;
  const finalPr = await c.api('GET', `${c.root}/pulls/${pr.number}`), finalBranch = await c.api('GET', `${c.root}/branches/develop`);
  if (finalPr.head.sha !== pr.head.sha || finalBranch.commit.sha !== develop || finalPr.state !== 'open' || finalPr.draft ||
      finalPr.body !== fresh.body || JSON.stringify(finalPr.labels) !== JSON.stringify(fresh.labels)) return false;
  await c.api('POST', `${c.root}/statuses/${pr.head.sha}`, { state: 'success', context: 'integration/queue', description, target_url: finalPr.html_url });
...
export async function retireContentSuperseded(c, pr, develop) {
  ...
  if (!contentSupersessionProof(paths, headEntries, developEntries).equivalent) return false;
...
export async function reconcileStack(c, pr, develop, { write = false } = {}) {
  ...
  try {
    const result = await c.api('POST', `${c.root}/merges`, { base: pr.head.ref, head: develop, commit_message: `Merge develop into ${pr.head.ref} after dependencies ${deps.map(number => `#${number}`).join(', ')}` });
...
export async function recoverQueue(c, { now = Date.now(), limit = 24, budgetMs = 150000 } = {}) {
  ...
  const report = { checked: [], ciRecovery: [], wake: [], superseded: [], semanticSuperseded: [], stackCandidates: [], quarantine: [], staleReview: [], errors: [], dispatched: false, configAudit: null, flowControl: null };
  let develop = null;
  ...
  for (const snapshot of ready.slice(start, start + limit)) {
    ...
    try {
      ...
      if (develop && stale && semanticBudget > 0) {
        semanticBudget--;
        if (await retireContentSuperseded(c, pr, develop)) { report.semanticSuperseded.push({ pr: pr.number, head: pr.head.sha, develop }); continue; }
      }
      const record = state?.records?.[pr.number];
      if (quarantined.has(Number(pr.number)) && (!record?.headSha || record.headSha === pr.head.sha)) {
        const decision = quarantineDecision(record);
        report.quarantine.push({ pr: pr.number, head: pr.head.sha, reason: decision.reason });
        await c.api('POST', `${c.root}/statuses/${pr.head.sha}`, { state: 'pending', context: 'integration/quarantine', description: 'QUARANTINE: repeated failures isolated from normal Integration train; AI deep repair owns recovery', target_url: pr.html_url });
        continue;
      }
      if (develop && stackBudget > 0 && dependencies(pr.body || '').length) {
        stackBudget--;
        const candidate = await reconcileStack(c, pr, develop);
        if (candidate.state === 'ready-to-reconcile') report.stackCandidates.push({ pr: pr.number, ...candidate });
      }
  ...
  if (develop) {
    try { report.configAudit = rescueRuntimeAudit(state, develop); await recordRuntimeAudit(c, develop, report.configAudit); }
