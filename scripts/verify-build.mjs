...
assert.ok(['game','dev-tool'].includes(info.kind || 'game'), 'Unknown app kind');
if ((info.kind || 'game') === 'game') assert.ok(html.includes('id="game"'), 'Game canvas is missing');
else assert.ok(/data-dev-tool=/.test(html), 'Developer tool marker is missing');
