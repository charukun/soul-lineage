...
const scope = full ? 'full' : deploy ? 'deploy' : fastGateScope(plan);
const narrowFast = !full && !deploy && scope !== 'broad';
const selected = narrowFast ? [...new Set([
  ...plan.apps.map(id => appNode(nodes, id).name),
  ...plan.packages,
])] : closureSelected;
console.log(JSON.stringify({ mode: full ? 'full' : deploy ? 'deploy' : 'fast', scope, workspaces: selected, transitiveWorkspaces: narrowFast ? closureSelected : undefined, plan }, null, 2));
...
console.log(JSON.stringify({ fastGate: summarizeFastGate({ scope, checkedWorkspaces: selected, ...split }) }, null, 2));
