...
"""Author inferred expression poses on the frozen, reference-projected head.
...
def author(w,cache):
    ...
    for index,point in enumerate(base):
        ...
        blink=sum(falloff(x-ex,7)*falloff(y-ey,7)*(y-ey)*.95 for ex,ey in eyes)*scale*facing
        mouth=falloff(x-center,14)*falloff(y-mouth_row,7)*facing
        smile=mouth*(min(1.,abs(x-center)/9)**2*3.-.5)*scale
    ...
    write_json(w/'build/morph/authoring.json',{'status':'inferred poses; actual browser semantics/neutral parity pending','neutralChanged':False,'observed':['front eye centres','front mouth row','reference-projected surface identity'],'inferred':['expression amplitude','smooth influence radii','smile corner movement','mouth opening deformation'],'notClaimed':['observed expression reference','measured facial muscle anatomy','observed mouth cavity'],'upstreamProjector':'forge.stage1_intake.camera_fitting_math.project_landmark','movedVertexCounts':moved})
    return moved
