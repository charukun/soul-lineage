...
try{
  ...
  await expect(page.locator('.forge-advanced')).not.toHaveAttribute('open','');
  ...
  await page.locator('.forge-advanced > summary').click();
  ...
  await page.locator('.forge-advanced > summary').click();
  await page.setViewportSize({width:390,height:844});await page.waitForTimeout(250);const columns=await page.locator('.forge-candidates').evaluate(e=>getComputedStyle(e).gridTemplateColumns.split(' ').length);assert.equal(columns,5);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true,'no horizontal overflow on phone');await expect(page.locator('[data-forge-wire]')).toBeHidden();await capture('mobile',page.locator('.forge-page'));
