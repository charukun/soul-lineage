...
function isQuarantinedHead(returnedHeads, item) { return returnedHeads.quarantineHeads?.get(Number(item?.number)) === item?.head?.sha; }
...
export function prioritizeReturnedReady(items, returnedHeads, now = Date.now()) {
  ...
  const pinnedIds = new Set(pinned.map(item => item.number)), byNumber = new Map(sorted.map(item => [Number(item.number), item]));
