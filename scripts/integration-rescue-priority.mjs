...
function labelNames(item) {
  return new Set((item?.labels || [])
    .map(label => typeof label === 'string' ? label : label?.name)
    .filter(Boolean));
}

function isQuarantinedHead(returnedHeads, item) {
  return returnedHeads.quarantineHeads?.get(Number(item?.number)) === item?.head?.sha;
}
...
export function readyPriorityScore(item, returnedHeads, now = Date.now(), graph = null) {
  ...
  const quarantined = isQuarantinedHead(returnedHeads, item);
...
export async function returnedPriorityHeads(c) {
  ...
  result.quarantineHeads = new Map();
  result.latency = deliveryLatencyMetrics([]);
  result.provenTrain = null;

  try {
    const file = await c.api(
      'GET',
      `${c.root}/contents/rescue-state.json?ref=${encodeURIComponent(STATE_BRANCH)}`,
      null,
      { cache: true },
    );
    ...
    result.latency = deliveryLatencyMetrics(deliveries.length ? deliveries : records);

    for (const record of records) {
      if (record.scope?.files?.length) result.scopeByPr.set(record.pr, record.scope);
      if (RETURNED.has(record.state) && record.state !== 'AWAITING_PUSH' && record.returnedAt) {
        result.set(record.pr, record.pushedSha || record.headSha);
      }
      if (quarantineDecision(record).quarantined && record.headSha) {
        result.quarantineHeads.set(Number(record.pr), record.headSha);
      }
    }
...
export function prioritizeReturnedReady(items, returnedHeads, now = Date.now()) {
  returnedHeads.scopeByPr ||= new Map();
  returnedHeads.quarantineHeads ||= new Map();
  returnedHeads.latency ||= deliveryLatencyMetrics([]);
  ...
  const proven = (returnedHeads.provenTrain?.candidates || [])
    .map(entry => byNumber.get(Number(entry.pr)))
    .filter(item => item && !pinnedIds.has(item.number) && !isQuarantinedHead(returnedHeads, item))
    .filter(item => returnedHeads.provenTrain.candidates.some(entry =>
      Number(entry.pr) === Number(item.number) && entry.head === item.head?.sha));
  const provenIds = new Set(proven.map(item => item.number));

  const ordinary = sorted.filter(item =>
    !pinnedIds.has(item.number) &&
    !provenIds.has(item.number) &&
    !isQuarantinedHead(returnedHeads, item));
  const quarantined = sorted.filter(item =>
    !pinnedIds.has(item.number) && isQuarantinedHead(returnedHeads, item));

  const trained = pressure.mode === 'NORMAL'
    ? ordinary
    : prioritizeIntegrationTrain(ordinary, returnedHeads.scopeByPr, { max: tuning.trainSize });

  const result = [...pinned, ...proven, ...trained, ...quarantined];
  result.flowControl = {
    ...pressure,
    ...tuning,
    provenTrain: proven.map(item => item.number),
    train: trained.slice(0, tuning.trainSize).map(item => item.number),
    quarantined: quarantined.map(item => item.number),
    criticalPath: [...graph.unblockCount.entries()]
