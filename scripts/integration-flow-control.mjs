...
export function dependencyGraph(items = []) {
  ...
  for (const item of items) {
    let parsed = [];
    try { parsed = dependencies(item.body || '').filter(number => byNumber.has(number)); }
    catch { parsed = []; }
    direct.set(Number(item.number), parsed);
    for (const dependency of parsed) reverse.set(dependency, [...(reverse.get(dependency) || []), Number(item.number)]);
  }
  ...
  return { byNumber, direct, reverse, unblockCount: new Map([...byNumber.keys()].map(number => [number, downstream(number).size])) };
}
...
export function planIntegrationTrain(items, scopeByPr = new Map(), { max = FLOW_THRESHOLDS.trainSize } = {}) {
  const selected = [], deferred = [], ordered = criticalPathOrder(items), graph = dependencyGraph(ordered);
  for (const item of ordered) {
    ...
    if (directDependencies.some(number => !selected.some(entry => Number(entry.number) === number))) { deferred.push(item); continue; }
    const independent = selected.every(existing => directDependencies.includes(Number(existing.number)) || compareScopes(scope, scopeByPr.get(existing.number)).risk === 'GREEN');
...
function finiteDurations(values) { return values.filter(value => Number.isFinite(value) && value >= 0).sort((a, b) => a - b); }
...
export function deliveryLatencyMetrics(records = []) {
  ...
  const metric = values => ({ samples: values.length, p50Ms: percentile(values, .5), p95Ms: percentile(values, .95) });
...
export function adaptiveFlowTuning({ ready = 0, recoverableManual = 0, latency = {}, failureRate = 0, rateRemaining = null } = {}) {
  ...
  const unstable = Number(failureRate) >= .25;
...
export function contentSupersessionProof(paths = [], headEntries = new Map(), developEntries = new Map()) {
  ...
  for (const path of paths) if ((headEntries.get(path) ?? null) !== (developEntries.get(path) ?? null)) return { equivalent: false, reason: `CONTENT_DIFF:${path}` };
...
export function staleReadyCandidate(pr, now = Date.now()) {
  ...
  return Number.isFinite(created) && !pr?.draft && pr?.state === 'open' && now - created >= FLOW_THRESHOLDS.staleReadyHours * HOUR_MS;
}

export function compactRescueState(state, now = Date.now(), { terminalRetentionMs = 48 * HOUR_MS, maxWaves = 48, maxOutbox = 120, maxDeliveries = 120, deliveryRetentionMs = 7 * 24 * HOUR_MS } = {}) {
