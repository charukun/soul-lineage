...
export const FLOW_LIMITS = Object.freeze({
  minTrain: 2,
  maxTrain: 5,
  minRescueConcurrency: 3,
  maxRescueConcurrency: 6,
  minEvaluations: 12,
  maxEvaluations: 24,
...
export function flowPressure({ ready = 0, recoverableManual = 0 } = {}) {
  ...
  const mode = demand >= FLOW_THRESHOLDS.burnDownReady
    ? 'BURN_DOWN'
    : demand >= FLOW_THRESHOLDS.busyReady
      ? 'BUSY'
      : 'NORMAL';
  ...
  return {
    ...
    reason: mode === 'BURN_DOWN'
      ? `Ready/AI-repair backlog ${demand} >= ${FLOW_THRESHOLDS.burnDownReady}`
      : mode === 'BUSY'
        ? `Ready/AI-repair backlog ${demand} >= ${FLOW_THRESHOLDS.busyReady}`
        : 'Queue pressure is normal',
  };
}
...
export function dependencyGraph(items = []) {
  ...
  for (const item of items) {
    let parsed = [];
    try {
      parsed = dependencies(item.body || '').filter(number => byNumber.has(number));
    } catch {
      parsed = [];
    }
    ...
    for (const dependency of parsed) {
      reverse.set(dependency, [...(reverse.get(dependency) || []), Number(item.number)]);
    }
  }
  ...
  return {
    ...
    direct,
    reverse,
    unblockCount: new Map([...byNumber.keys()].map(number => [number, downstream(number).size])),
  };
}
...
export function criticalPathOrder(items = [], { now = Date.now() } = {}) {
  ...
  return [...items].sort((a, b) => {
    const unblock = (graph.unblockCount.get(Number(b.number)) || 0) -
      (graph.unblockCount.get(Number(a.number)) || 0);
...
export function planIntegrationTrain(items, scopeByPr = new Map(), { max = FLOW_THRESHOLDS.trainSize } = {}) {
  ...
  const graph = dependencyGraph(ordered);
  ...
  for (const item of ordered) {
    ...
    const directDependencies = graph.direct.get(Number(item.number)) || [];
    if (directDependencies.some(number => !selected.some(entry => Number(entry.number) === number))) {
      deferred.push(item);
      continue;
    }

    const independent = selected.every(existing =>
      directDependencies.includes(Number(existing.number)) ||
      compareScopes(scope, scopeByPr.get(existing.number)).risk === 'GREEN');

    if (independent) selected.push(item);
...
export function deliveryLatencyMetrics(records = []) {
  const implementationToReady = records
    .map(record => duration(record.implementationStartedAt || record.createdAt, record.readyAt || record.detectedAt))
    .filter(Number.isFinite);
  const readyToMerge = records
    .map(record => duration(record.readyAt || record.detectedAt, record.mergedAt))
    .filter(Number.isFinite);
  const mergeToDev = records
    .map(record => duration(record.mergedAt, record.devAt))
    .filter(Number.isFinite);
  const readyToDev = records
    .map(record => duration(record.readyAt || record.detectedAt, record.devAt))
    .filter(Number.isFinite);
  const implementationToDev = records
    .map(record => duration(record.implementationStartedAt || record.createdAt, record.devAt))
    .filter(Number.isFinite);
  ...
  return {
    implementationToReady: metric(implementationToReady),
    readyToMerge: metric(readyToMerge),
    mergeToDev: metric(mergeToDev),
    readyToDev: metric(readyToDev),
    implementationToDev: metric(implementationToDev),
  };
}

export function quarantineDecision(record) {
  if (!record || record.state === 'DEV' || record.state === 'CLOSED' || record.workRepair?.status === 'human-required') {
    return { quarantined: false, failures: 0 };
  }

  const meaningful = (record.failures || []).filter(item =>
    !observationOnlyReason(String(item?.reason || '')));
  const recent = meaningful.slice(-FLOW_LIMITS.quarantineFailures);
  const distinct = new Set(recent.map(item =>
    String(item.reason || '')
  ...
  return {
    quarantined,
    failures: meaningful.length,
    reason: quarantined
      ? `Repeated repair failures (${meaningful.length}); isolate from normal train and use deep AI repair`
      : null,
  };
}

export function adaptiveFlowTuning({
  ready = 0,
  recoverableManual = 0,
  latency = {},
  failureRate = 0,
  ...
  if (apiTight || unstable) {
    return {
      pressure,
      trainSize: 2,
      rescueConcurrency: 3,
      maxEvaluations: 12,
      reason: apiTight ? 'GitHub API reserve is tight' : 'Recent repair failure rate is high',
    };
  }
  if (pressure.mode === 'BURN_DOWN' && p95 >= 20 * 60000) {
    return {
      pressure,
      trainSize: 5,
      rescueConcurrency: 6,
      maxEvaluations: 24,
      reason: 'Backlog and p95 latency are high; use bounded maximum throughput',
    };
  }
  if (pressure.mode === 'BUSY' || p95 >= 10 * 60000) {
    return {
      pressure,
      trainSize: 4,
      rescueConcurrency: 5,
      maxEvaluations: 20,
      reason: 'Queue is busy; raise bounded throughput',
    };
  }
  return {
    pressure,
    ...
    rescueConcurrency: 4,
    maxEvaluations: 16,
    reason: 'Normal bounded operating point',
  };
}
...
export function contentSupersessionProof(paths = [], headEntries = new Map(), developEntries = new Map()) {
  ...
  for (const path of paths) {
    if ((headEntries.get(path) ?? null) !== (developEntries.get(path) ?? null)) {
      return { equivalent: false, reason: `CONTENT_DIFF:${path}` };
    }
  }
...
export function staleReadyCandidate(pr, now = Date.now()) {
  ...
  return Number.isFinite(created) && !pr?.draft && pr?.state === 'open' &&
    now - created >= FLOW_THRESHOLDS.staleReadyHours * HOUR_MS;
}

export function compactRescueState(state, now = Date.now(), {
  terminalRetentionMs = 48 * HOUR_MS,
  maxWaves = 48,
  maxOutbox = 120,
  maxDeliveries = 120,
  deliveryRetentionMs = 7 * 24 * HOUR_MS,
} = {}) {
  ...
  const completed = (state.waves || []).filter(wave => wave.completedAt).slice(-maxWaves);
  const active = (state.waves || []).filter(wave => !wave.completedAt);
  state.waves = [...completed, ...active].slice(-maxWaves);
  state.activity = (state.activity || []).filter(entry => now - Date.parse(entry.at) < 24 * HOUR_MS).slice(-400);
  state.outbox = (state.outbox || [])
    .filter(entry => !entry.sentAt || now - Date.parse(entry.sentAt) < 24 * HOUR_MS)
    .slice(-maxOutbox);
  ...
  const deliveries = Object.values(state.flowControl.deliveries || {})
    ...
    .sort((a, b) =>
      Date.parse(b.devAt || b.mergedAt || b.readyAt || 0) -
      Date.parse(a.devAt || a.mergedAt || a.readyAt || 0))
