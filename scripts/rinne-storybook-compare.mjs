...
const calibration='apps/rinne/src/storybook-calibration.css',originalCss=await readFile(calibration,'utf8');
const harness='apps/rinne/storybook-review.html',index=await readFile('apps/rinne/index.html','utf8');
...
let state;
function fixture(age=20){const s=createLife({seed:491});s.name='旅人';s.phase='living';s.ageYears=age;s.ageSeconds=age*60;ensureInspiration(s);s.inspiration.legacySkills=[...SUPPORT_SKILLS.slice(0,12).map(v=>v.id),...ACTION_SKILLS.filter(v=>!v.id.startsWith('spark.')).map(v=>v.id)];s.knownSkills=[...new Set([...s.knownSkills,...s.inspiration.legacySkills])];s.equipment={weapon:age>=7?'sword':'fist',armor:'cloth',shield:false};s.inventory={weapons:age>=7?['fist','sword','dagger','great','spear','axe','staff']:['fist'],armors:age>=7?['cloth','light','heavy']:['cloth'],shields:age>=7?[false,true]:[false]};ensureCombatLoadout(s);for(let i=0;i<3;i++)setHeartSlot(s,i,SUPPORT_SKILLS[i].id);return s;}
...
window.bookReview={ui,fixture,get state(){return state},set(next){state=next;ui.bindState(next)},snapshot(){return structuredClone(state.combatLoadout)},model(page){return buildStorybookModel(state,page)}};
...
const params=Object.fromEntries(pages.map(p=>[p,{top:15.3,bottom:13,heading:12,loadout:25,library:p==='body'?53:p==='items'?51.5:49,...input.initialParams?.[p]}]));
const cssFor=()=>`/* Retained after explicit 100-round browser comparison. */\n@media (min-height:701px) and (orientation:portrait){\n${pages.map(p=>`.rb-page[data-book-page="${p}"]{--rb-top:${params[p].top}%;--rb-bottom:${params[p].bottom}%;--rb-heading:${params[p].heading}%;--rb-loadout:${params[p].loadout}%;--rb-library-width:${params[p].library}%}`).join('\n')}\n}\n`;
...
async function settle(){await page.evaluate(async()=>{await document.fonts.ready;await Promise.all([...document.querySelectorAll('.rb-page img')].map(im=>im.complete?Promise.resolve():new Promise(r=>{im.onload=im.onerror=r})));});await page.waitForTimeout(100);}
...
try{
  ...
  if(Number(input.rounds)>0){
    ...
    await writeFile(calibration,cssFor());await page.waitForTimeout(250);
    for(const p of pages){
      await select(p);
      for(let local=0;local<25;local++){
        const iteration=rounds.length+1,field=['top','bottom','heading','loadout','library'][local%5],prior=params[p][field],step=[1,.6,.35,.2,.1][Math.floor(local/5)],direction=Math.floor(local/5)%2?-1:1;
        const beforePath=`${out}/${String(iteration).padStart(3,'0')}-${p}-before.jpg`,afterPath=`${out}/${String(iteration).padStart(3,'0')}-${p}-after.jpg`;
        ...
        assert.notEqual(after.sha256,before.sha256,'A comparison round must produce a real rendered change');
        if(!accepted){params[p][field]=prior;await writeFile(calibration,cssFor());await page.waitForTimeout(160);await settle();}
        const verified=await geometry();assert.equal(verified.missing.length,0);assert.ok(verified.nav.bottom<=verified.view.h-15);
        rounds.push({index:iteration,page:p,exactHead:head,parameter:field,beforeValue:prior,candidateValue:Math.round((prior+step*direction)*1000)/1000,accepted,before,after,sourceCssSha256:hash(candidateCss),screenshots:{before:beforePath.split('/').pop(),after:afterPath.split('/').pop()}});
        console.log(`round ${iteration}/100 ${p} ${field}: ${before.score.toFixed(5)} -> ${after.score.toFixed(5)} ${accepted?'retained':'reverted'}`);
      }
    }
  }
  ...
  await probe.locator('[data-heart]').click();await probe.locator('.rb-page[data-book-page="heart"]').waitFor();assert.equal(await probe.locator('.rinne-primary-four button').count(),4);
  const before=await probe.evaluate(()=>window.bookReview.snapshot());
  const candidate=probe.locator('.rb-tile').nth(4),id=await candidate.getAttribute('data-book-id');await candidate.click();
  ...
  await probe.locator('[data-book-action="remove"]').click();assert.notEqual(await probe.evaluate(()=>window.bookReview.state.combatLoadout.heart.active[0]),id);functional.push('heart-remove');
  ...
  const bounds=await probe.locator('.rinne-primary-four').boundingBox();assert.ok(bounds&&bounds.x>=0&&bounds.x+bounds.width<=360&&bounds.y+bounds.height<=505);functional.push('small-viewport-core-navigation');
  await probe.setViewportSize({width:740,height:360});await probe.screenshot({path:`${out}/landscape-body.png`});const landscape=await probe.locator('.rinne-primary-four').boundingBox();assert.ok(landscape.x>=0&&landscape.x+landscape.width<=740&&landscape.y+landscape.height<=350);functional.push('landscape-core-navigation');
  await probeContext.close();assert.equal(errors.length,0,errors.join('\n'));
  const receipt={schemaVersion:1,kind:'approved-ui-component-authoring',exactHead:head,referenceKind:'user-approved-original-images',roundsRequested:Number(input.rounds)||0,roundsExecuted:rounds.length,retained:rounds.filter(r=>r.accepted).length,reverted:rounds.filter(r=>!r.accepted).length,metric:'55% blurred-reference MAE + 45% normalized layout-anchor distance; not a claim of complete pixel identity',functional,params,rounds,calibrationCss:Number(input.rounds)>0?cssFor():originalCss,createdAt:new Date().toISOString(),scope:'Production menu controller and canonical game-state setters under isolated UI fixtures; this is not a no-injection full-life gameplay test.'};
  await writeFile(`${out}/receipt.json`,JSON.stringify(receipt,null,2));await writeFile(`${out}/proposed-calibration.css`,receipt.calibrationCss);
  ...
  const rows=[];
  for(const name of ['receipt.json','proposed-calibration.css',...pages.flatMap(p=>[`initial-${p}.png`,`final-${p}.png`]),'mobile-technique.png','mobile-body.png','mobile-items.png','newborn-equipment.png','small-body.png','landscape-body.png']){
