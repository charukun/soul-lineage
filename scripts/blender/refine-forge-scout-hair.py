...
def main():
    ...
    payload=w/'review/material-pass/mesh-buffers.json';rows=json.loads(payload.read_text())
    ...
    modifier=hair.modifiers.new('Reference rim relaxation','SMOOTH');modifier.vertex_group=group.name;modifier.factor=.35;modifier.iterations=4
    ...
    if displacement>8*scale:raise RuntimeError('DCC correction exceeds the declared observed-band displacement budget')
