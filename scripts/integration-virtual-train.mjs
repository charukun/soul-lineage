...
import { adaptiveFlowTuning, deliveryLatencyMetrics, planIntegrationTrain, quarantineDecision } from './integration-flow-control.mjs';
...
export async function cleanupVirtualTrain(c, branch) {
  ...
  try { await c.api('DELETE', `${c.root}/git/refs/heads/${branch}`); return true; }
  catch (error) { if (/HTTP 404\b/.test(error.message)) return false; throw error; }
}
...
export async function buildVirtualTrain(c, state, develop, { runId = Date.now(), max = 5, keepRef = false } = {}) {
  ...
  try {
    ...
    for (const pr of candidates) {
      ...
      if (fresh.head.sha !== pr.head.sha || liveDevelop !== develop) return { status:'stale', base:develop, branch, candidates:merged, reason:'develop or candidate head changed during virtual train' };
      try {
        ...
        if (/HTTP 409\b/.test(error.message)) return { status:'conflict', base:develop, branch, candidates:merged, failed:{pr:pr.number,head:pr.head.sha}, reason:'virtual merge conflict' };
        throw error;
      }
    }
    ...
    return { status:'assembled', base:develop, branch, candidates:merged, syntheticCommit:current, syntheticTree:commit.tree.sha, createdAt:new Date().toISOString() };
  } finally {
...
export async function planVirtualTrain(c, store, { runId = Date.now() } = {}) {
  ...
  const latency = deliveryLatencyMetrics(records);
  const failureRate = records.filter(r => (r.failures || []).length).length / Math.max(1, records.length);
  const tuning = adaptiveFlowTuning({ ready:records.filter(r=>!['DEV','CLOSED'].includes(r.state)).length, latency, failureRate, rateRemaining:c.metrics?.().rateRemaining });
...
export async function finalizeVirtualTrain(c, store, proof, { fast, browser, now = Date.now() } = {}) {
  ...
  const liveDevelop = (await c.api('GET', `${c.root}/branches/develop`)).commit.sha;
  if (proof.status === 'assembled' && proof.base === liveDevelop && fast === 'success' && browser === 'success') result = { ...result, status:'validated', validatedAt:new Date(now).toISOString() };
  else if (proof.status === 'assembled') result = { ...result, status:proof.base === liveDevelop ? 'failed-validation' : 'stale', validatedAt:new Date(now).toISOString() };
...
async function main() {
  ...
  if(mode==='plan'){
    const proof=await planVirtualTrain(c,store,{runId:process.env.GITHUB_RUN_ID});writeFileSync(file,JSON.stringify(proof,null,2));
  ...
  if(mode==='finalize'){
    const proof=JSON.parse(readFileSync(file,'utf8'));const result=await finalizeVirtualTrain(c,store,proof,{fast:process.env.TRAIN_FAST,browser:process.env.TRAIN_BROWSER});writeFileSync(file,JSON.stringify(result,null,2));console.log(JSON.stringify(result));return;
  }
