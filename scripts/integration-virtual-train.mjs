...
export async function cleanupOrphanedVirtualTrains(c, { limit = 8 } = {}) {
  let refs;
  try { refs = await c.api('GET', `${c.root}/git/matching-refs/heads/${TRAIN_PREFIX}`); }
  catch (error) { if (/HTTP 404\b/.test(error.message)) return []; throw error; }
...
export async function recordVirtualTrain(store, proof, now = Date.now()) {
  await store.mutate(state => { state.flowControl ||= {}; state.flowControl.trainProof = { ...proof, recordedAt: new Date(now).toISOString() }; });
  return proof;
}
...
async function main() {
  ...
  if (mode === 'finalize') {
    ...
    const result = await finalizeVirtualTrain(c, store, proof, { fast: process.env.TRAIN_FAST, browser: process.env.TRAIN_BROWSER });
    writeFileSync(file, JSON.stringify(result, null, 2)); console.log(JSON.stringify(result)); return;
  }
