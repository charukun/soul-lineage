...
import { graph, affected, apps, toolingPath, workspaceManifestPath } from './workspaces.mjs';
...
const infrastructure = globalInfrastructure || dependencyGraphChanged;
const packages = [...nodes.values()].filter(n => n.group === 'packages' && (globalInfrastructure || paths.some(p => p.startsWith(`${n.dir}/`) && !p.endsWith('/README.md')))).map(n => n.name);
const plan = { apps: selected, packages, allApps: apps(nodes).map(n => n.id), infrastructure, dependencyGraphChanged, paths };
