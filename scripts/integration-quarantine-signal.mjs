...
async function main(){
  ...
  const config=rescueConfig(process.env),c=rescueClient(process.env.GH_TOKEN,config),store=new RescueStore(c,config);console.log(JSON.stringify({signaled:await signalQuarantine(c,store)}));
}
