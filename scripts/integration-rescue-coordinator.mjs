...
import { adaptiveFlowTuning, deliveryLatencyMetrics, quarantineDecision } from './integration-flow-control.mjs';
...
function mergeObserved(state, observation, now) {
  ...
  if (previous?.state === 'FAILED_MANUAL') return;
...
export async function coordinate(c, store, { now = Date.now(), runId, runAttempt = 1, consumers = {}, configured = true, scan = true } = {}) {
  ...
  if (now - Date.parse(initial.coordinator?.heartbeatAt || 0) < store.config.scanMinMs ||
    ...
    return { state: initial, result: [], errors: [{ reason: 'RESCUE_SCAN_COALESCED_OR_API_RESERVED' }], develop: initial.coordinator?.develop, tuning:initial.flowControl?.tuning || null };
  }
  ...
  const latency = deliveryLatencyMetrics(historical);
  ...
  const recoverableManual = historical.filter(record => record.state === 'FAILED_MANUAL' && record.workRepair?.status !== 'human-required').length;
  ...
  const runStates = {};
  for (const run of new Set(historical.filter(r => r.lease).map(r => r.runId))) runStates[run] = (await c.api('GET', `${c.root}/actions/runs/${run}`)).status;
  const lifecycle = [];
  const observedRecord = r => ({ state: r.state, headSha: r.headSha, pushedSha: r.pushedSha, stagedSha: r.stagedSha });
  for (const r of historical.filter(r => !r.lease && ((!TERMINAL.has(r.state) && (!live.has(r.pr) || live.get(r.pr).draft)) || (r.state === 'FAILED_MANUAL' && !live.has(r.pr)))).slice(0, 4)) {
  ...
  for (const r of historical.filter(r => RETURNED.has(r.state) || r.state === 'MERGED').slice(0, 8)) {
    const pr = live.get(r.pr) || await c.api('GET', `${c.root}/pulls/${r.pr}`);
    let timedOut = false;
  ...
  for (const pr of selected) {
    try {
      if ((c.metrics?.().requests || 0) >= store.config.maxRequests - 20 || (c.metrics?.().rateRemaining != null && c.metrics().rateRemaining < store.config.apiReserve)) { errors.push({ pr: pr.number, reason: 'RESCUE_API_BUDGET_DEFERRED' }); break; }
      ...
      if (excluded) {
        observations.push({ excluded, record: { pr: pr.number, title: pr.title, branch: pr.head.ref, headSha: pr.head.sha, developSha: develop,
          implementationStartedAt: previous?.implementationStartedAt || pr.created_at || null, readyAt: previous?.readyAt || new Date(now).toISOString(),
      ...
      observations.push({ excluded: depError || browserRepair?.manual, record: { pr: fresh.number, title: fresh.title, detail: (fresh.body || '').split('\n')[1]?.slice(0, 240),
        ...
        implementationStartedAt: previous?.implementationStartedAt || fresh.created_at || null, readyAt: previous?.readyAt || new Date(now).toISOString(),
  ...
  const result = await store.mutate(state => {
    state.config = { ...store.config, maxConcurrency:tuning.rescueConcurrency, maxEvaluations:tuning.maxEvaluations };
    state.flowControl ||= {};
    ...
    for (const { pr, expectedId, observed, excluded, timedOut } of lifecycle) {
      ...
      if (pr.merged || pr.merged_at) {
        if (!['MERGED', 'DEV'].includes(r.state)) { r.mergedAt = pr.merged_at || new Date(now).toISOString(); r.mergeCommit = pr.merge_commit_sha; transition(state, r, 'MERGED', 'Normal Integration merged this PR into develop', now); }
      ...
      else if (pr.head.sha !== r.headSha && pr.head.sha !== r.pushedSha && !(r.state === 'AWAITING_PUSH' && pr.head.sha === r.stagedSha)) failure(state, r, 'HEAD_CHANGED_AFTER_RESCUE', now);
      else if (timedOut && !r.lease && !(r.pushLease && now - Date.parse(r.pushLease.at) <= 600000)) failure(state, r, r.state === 'AWAITING_PUSH' ? 'WORK_PUSH_RELAY_STALLED' : 'INTEGRATION_RETURN_STALLED', now);
    }
    ...
    function visit(n, chain = []) {
      ...
      visiting.add(n); for (const p of state.records[n]?.dependencies || []) if (!state.records[n]?.mergedDependencies?.includes(p)) visit(p, [...chain, n]); visiting.delete(n); visited.add(n);
    }
    ...
    state.coordinator = { runId:String(runId), heartbeatAt:new Date(now).toISOString(), develop, configured, phase:configured?'observing':'CONFIGURATION_REQUIRED', errors, backend:'actions-no-api + chatgpt-work-push', configurationReason:configured?null:'Rescue explicitly disabled; no worker started' };
    const freshIds = new Set(observations.filter(o => !o.excluded).map(o => o.record.pr));
    const deferred = [];
    for (const r of Object.values(state.records)) if (!freshIds.has(r.pr) && ['DETECTED', 'QUEUED', 'BLOCKED_BY_RESCUE', 'FAILED_RETRYABLE'].includes(r.state)) { deferred.push([r, r.nextAttemptAt]); r.nextAttemptAt = new Date(now + 1).toISOString(); }
...
async function main() {
  ...
  const report = await coordinate(c, store, { runId:process.env.GITHUB_RUN_ID, runAttempt:process.env.GITHUB_RUN_ATTEMPT, configured:process.env.RESCUE_CONFIGURED==='true', consumers:workspaceConsumers(), scan:process.env.RESCUE_OBSERVE_ONLY!=='true' });
  mkdirSync('.deploy-state', { recursive:true });
  writeFileSync('.deploy-state/rescue-coordinator.json', JSON.stringify({ workers:report.result, errors:report.errors, tuning:report.tuning, api:c.metrics() }, null, 2));
  ...
  console.log(JSON.stringify({ workers:report.result.length, scannedErrors:report.errors, configured:report.state.coordinator.configured, tuning:report.tuning }));
}
