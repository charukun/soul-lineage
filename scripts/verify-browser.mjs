...
const run = args => execFileSync('npm', args, { cwd: root, stdio: 'inherit', env: { ...process.env,
  BROWSER_SITE_URL: siteUrl,
  BROWSER_SITE_ROOT: siteRoot,
  BROWSER_SITE_MOUNT: new URL(siteUrl).pathname,
  BROWSER_TARGETS: JSON.stringify(targets.map(({ app, path, legacy, version }) => ({ app, path, legacy, version }))),
} });
