...
export async function requestDevelopPublication(c, report, { targetUrl } = {}) {
  ...
  // An idle pass repairs the missing wake left by the old Controller, once per SHA.
  // Existing delivery failures remain owned by repair; this is not an unbounded retry loop.
  if (!report.merged.length && (status(publicationWakeContext) ||
      (status('integration/develop')?.state === 'success' && status('ops-board/public')?.state === 'success'))) {
  ...
  try {
    ...
    await record('pending', 'Requesting DEV/PULSE publication; this is not public verification');
    ...
    await record('success', 'DEV/PULSE publication requested; public verification pending');
