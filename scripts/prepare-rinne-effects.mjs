...
import {EFFECT_SOURCE,EFFECT_RUNTIME,EFFECT_ASSETS,RUNTIME_ASSETS,EFFECT_PUBLIC_PATH} from '../apps/rinne/src/rebuild/authored-effect-manifest.js';
...
export function effectDependencies(bytes){
  ...
  for(let p=8;p+8<=b.length;){
    ...
    if(tag==='INFO'){
      ...
      const result=[];
      // color/normal/distortion textures, models, sounds, materials.
...
export async function prepareRinneEffects({outputRoot=path.join(root,'apps/rinne/public',EFFECT_PUBLIC_PATH),fetchImpl=globalThis.fetch}={}){
  const rows=[];
  // Small fixed batch: never download a complete pack, FBX source, or unused effect.
