...
async function collectRunDetails(repo,run,token,{includeArtifacts=true,logMode='auto'}={}){
  const jobsRaw=await paged('/repos/'+repo+'/actions/runs/'+run.id+'/jobs?filter=latest','jobs',token);
  const artifactsRaw=includeArtifacts?await paged('/repos/'+repo+'/actions/runs/'+run.id+'/artifacts','artifacts',token):[];
  const kind=runKind(run);
  const jobs=await Promise.all(jobsRaw.map(async job=>{
    const failed=['failure','cancelled','timed_out','action_required'].includes(job.conclusion);
    const needsLog=logMode==='failures'?failed:
      kind==='dev_publish'||kind==='browser_review'||/browser|playwright|chromium/i.test(job.name||'')||failed;
...
async function collectRuns(repo,sha,token){
  ...
  return Promise.all(runsRaw.map(run=>collectRunDetails(repo,run,token)));
}

async function collectRunById(repo,runId,token){
  const run=await request('/repos/'+repo+'/actions/runs/'+runId,{token});
  return collectRunDetails(repo,run,token,{includeArtifacts:false,logMode:'failures'});
}
...
export async function collectFastDevSession({repo,sha=null,prNumber=null,token,full=false}={}){
  ...
  const headStatuses=headStatusBundle.statuses||[];
  const inspection=actionsInspectionPlan(headStatuses,{full});
  let runs=[],statuses=headStatuses;
  if(full){
    ...
    runs=uniqueRuns((await Promise.all([...new Set(relatedShas)].map(value=>collectRuns(repo,value,token)))).flat());
    const statusBundles=await Promise.all([...new Set(relatedShas)].map(value=>request('/repos/'+repo+'/commits/'+value+'/status',{token})));
    statuses=statusBundles.flatMap(bundle=>bundle.statuses||[]);
  }else if(inspection.run_id){
    runs=[await collectRunById(repo,inspection.run_id,token)];
  }
