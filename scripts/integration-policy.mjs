...
export function eligibility({ pr, repository, files, reviews, unresolved, dependenciesMerged, dependenciesCurrent = false, checksPassed, baseChanges = [], recovery = false }) {
  ...
  if (pr.mergeable_state === 'behind') {
    ...
    if (!dependenciesCurrent) return 'dependency merge-forward required';
  } else if (!['clean', 'unstable', 'has_hooks'].includes(pr.mergeable_state)) {
    return 'mergeability/protection requires attention';
  }
